...
- [ ] 阶段 A（A-1~A-9, A-T）→ 门 R01/R02
- [ ] 阶段 B（B-1~B-5, B-T）→ 门 R03
- [ ] 阶段 C（C-1~C-4, C-T）→ 门 R04~R06
- [ ] 阶段 D（D-1~D-8, D-T）→ 门 R07~R13
- [ ] 阶段 E（E-1~E-4, E-T）→ 门 R14~R17
- [ ] 阶段 F（F-1~F-6, F-T）→ 门 R18~R22
- [ ] 与 2号位 Snapshot 联调记录（交付物 11）
- [ ] 与 1号位 Solver Strategy 联调记录（交付物 12）
