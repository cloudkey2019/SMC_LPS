-- =============================================
-- APS 数据库表结构设计 DDL v5.2.5 (金牌 DDL 模板)
-- 版本: v5.2.5
-- v5.2.5更新（2026-07-31 1号位纯内存接口边界澄清）：
--   1) 不新增或修改任何表、列、索引、约束、视图投影或存储过程逻辑。
--   2) TaskDraft/FinalTaskDraft/ScheduledTaskDraft/AllocationShare均为应用层内存对象，不存在物理表。
--   3) Stage路径、Routing、进度快照、资源和参数均由2号位查询并装入DomainSolveRequest；1号位不得直接访问数据库。
--   4) 1号位仅返回内存FinalTaskDraft/AllocationShares；正式[Task]及Ledger/PSA/Pegging由2号位统一事务落库。
--
-- v5.2.4更新（2026-07-31 库存净量契约与Pegging最终编码语义）：
--   1) 不新增表/列/索引/约束；ERP库存ODS契约字段集合不变，Quantity由5号位定义为max(0, ERP总量-WasterQty)的APS净可用量。
--   2) ext_ERP_Inventory_View由SELECT *改为显式7列包装，并在注释中锁定Quantity净量语义；APS下游不得再次扣减WasterQty。
--   3) AllocationSequence由应用层在每PlanVersion的Pegging局部计数器中、成功双边扣减后生成；数据库唯一约束仅作兜底。
--   4) FinalTaskId/TaskComponentQty在单Domain事务内通过FinalTaskDraftKey→TaskId映射后完整写入Ledger；不先写半成品Ledger。
--   5) SupplyBusinessKey格式、身份来源及Task→Ledger→PSA→物理Pegging统一事务顺序由配套文档冻结；DDL结构保持v5.2.3。
--
-- v5.2.3更新（2026-07-30 定点补丁：MES快照服务边界与ExecutionLock数量语义）：
--   1) INightlyBatchOrchestrator只创建ScheduleRun并冻结ScheduleRunId+DataCutoffTime；三个MES快照SP由独立定时Job调用，ISchedulingOrchestrator只校验和消费快照。
--   2) ExecutionLock删除人工短量差额字段；RemainingExecutionQty改为2号位显式维护的现实MES工单未来Stage产出承诺上限。
--   3) ExecutionLock数量约束改为CompletedQty+CancelledQty+RemainingExecutionQty<=OriginalExecutionQty；终态允许未完成、未取消差额返回PI未承诺剩余池。
--
-- v5.2.2更新（2026-07-29 第三轮定点修订：MES双向视图、HardLock部分解除与文档审计收口）：
--   1) 明确APS↔MES采用双向视图集成：新增MESPlanRelease发布快照表与APS_MES_PlanRelease_View；MES按ReleaseItemKey幂等建单，TaskNo仅展示诊断。
--   2) Task新增MESPlanReleaseId；同PI+同Stage的一条发布单元可关联多个小工序Task；发布数量取Stage级执行批次单一流转量，不累加串行Task数量；MES建单后ExecutionLock接替跨版本现实执行。
--   3) MESWorkOrderSnapshot新增ReleaseItemKey，按ScheduleRun.DataCutoffTime从MES实时权威视图形成运行快照；不建设MES_Actual_Staging事件累计链。
--   4) ExecutionLock新增MESPlanReleaseId；MES工序状态4仅保留为小工序人工完结来源事实，不改变PI总剩余边界。
--   5) DemandSupplyHardLock允许ACTIVE状态下部分ReleasedQty，剩余数量继续保持硬归属。
--   6) PlanVersion分区改为每100个版本一个分区，预建边界至100000；归档/失败数据保留与清理由部署作业按文档策略执行。
--   7) APS_Auth由独立脚本《APS_Auth数据库DDL_v1.0.sql》部署，不复制进APS_Production主DDL。
--
-- v5.2.1更新（2026-07-29 六份文档全局机械审计与场景反向审计修正）：
--   1) Task新增ProductionInstructionNo/StageCode，补齐MES下发与ExecutionLock创建的执行身份；MTS_InstructionNo仅兼容。
--   2) Ledger新增AllocationSequence与TaskComponentQty；视图只汇总TaskComponentQty，避免AllocatedQty在多层/多来源场景重复累计。
--   3) ExecutionLock/HardLock生命周期CHECK补齐；DemandSupplyHardLock新增RowVersion。
--   4) PeggingSupplyAllocation增加PlanVersionId+LedgerId唯一约束；Ledger FinalTaskId建立复合FK到Task。
--   5) 旧sp_SyncPipelineSupply空跑过程改为安全阻断，避免部署中断后遗留TRUNCATE实现；正式实现仍由Batch 5覆盖。
--
-- v5.2.0更新（2026-07-29 Pegging软硬锁、PI位置快照与跨版本执行闭环）：
--   1) 新增 ProductionInstructionSupplySnapshot / ProductionInstructionPositionSlice：按 ScheduleRunId+DomainKey 保存PI总量边界与互斥位置切片；
--      AvailableProductionQty = max(0, InstructionQty-ReceivedQty)，MES/XC/在途仅定位，不二次扣减总量。
--   2) 新增 ExecutionLock：以 SourceSystem+MESWorkOrderNo 为现实执行稳定锚点；RELEASED固定执行过程，不自动固定原需求归属。
--   3) 新增 DemandSupplyHardLock：承载特殊出荷指示类型、客户专属、质量/环保资格、人工审批等硬归属；与ExecutionLock严格分离。
--   4) 新增 PeggingAllocationLedger：按PlanVersion保存需求—供给数量血缘、SOFT/HARD模式、位置切片、逻辑块及最终Task组成份额。
--   5) 补建 PeggingSupplyAllocation 正式表：只承接库存/在途/Received等非Task现实供给分配；物理Pegging继续仅Task-to-Task。
--   6) Task新增 ExecutionLockId / IsVirtual；Task.OrderId仅为代表订单，需求数量归属以Ledger及vw_TaskDemandAllocation为权威；虚拟Task禁止进入执行态。
--   7) 新增 vw_TaskDemandAllocation；V1不建SoftAllocation、DemandPromiseConstraint、ExecutionLockTaskLink、TaskDemandAllocation物理表。
--   8) V1不建设动态插件平台或完整事件溯源；普通计算返回Result/Decision，只有人工审批或正式状态变化使用Voucher。
--   9) 管道供给从“固定空跑”升级为统一输入视图按来源启用：补采购在途/VMI/已到厂未入库同构占位契约，sp_SyncPipelineSupply按BatchNo真实装载已启用来源；未命中规则默认按全局基础口径纳入，明确排除规则优先。
...
-- 日期: 2026-07-31
-- 基于: 《APS数据架构与防腐层设计方案 v1.41》+《APS各类基础数据分层承接与演变总表 v3.37》+《APS核心排产全流程走查 V3.22》+《APS集成接口设计 v1.31》
-- 数据库: SQL Server 2019+
...
--   - V1简化：DefaultLeadTimeDays硬编码=2天；V2可新增DomainLogisticsConfig配置表精确化
...
--     边界：仅 ODS 内部 & 委外 ShippingTask 生成器 & 运维诊断使用；**不是 APS 下游消费入口**
...
--   sp_SyncPipelineSupply V1 空跑骨架确认：TRUNCATE SupplyFact_Pipeline + 写 SUCCESS 日志；
...
--   ERP_InterplantInTransit_View V1 空链路预留框架
...
--   ▸ sp_SyncPipelineSupply：正式签名 +@DataCutoffTime（与ScheduleRun同值）；V1传值但空跑阶段不使用
...
--   ▸ ODS库 MES_Integration 新增 ERP_InterplantInTransit_View 空契约视图（14字段 + WHERE 1=0 返回0行）
...
--     消费边界：仅 ODS 内部消费（5 号位 material_stage_chain 推导、委外 ShippingTask 生成器、运维诊断）；APS_Production 永不查此表
...
--
-- 4. 【分区扩展】
--    - v5.2.2按每100个PlanVersion一个分区，已预建至PlanVersionId=100000
--    - 接近100000前由DBA评估扩边界或迁移方案，禁止长期落入尾部分区
--
-- 5. 【索引优化】
--    - 脚本中的索引为基础配置
...
-- 2号位装载：按StageScopeType区分查询并读取StageSeq+StageCode，串接RoutingOperation形成内存TaskDraft；再通过方法参数传给1号位，1号位不直接查询本表
...
--   - 仅 ODS 内部（委外 ShippingTask 生成器、运维诊断、BI 报表）使用
...
-- =============================================
-- §1.10b ERP_InterplantInTransit_View（ODS 契约视图 — v5.0.42 新增 2026-06-15）
-- =============================================
-- 所属层：ODS 层
-- 所属库：MES_Integration
-- 来源系统：ERP
-- 维护责任人：5号位
-- 定位：暴露 ERP 厂间物流运输在途数据；当前为管道供给链的唯一 ODS 来源视图
-- ⚠️ 契约锁定规则：ODS 契约视图字段结构为强契约。
--    V1.1/V2 启用真实数据时，仅允许替换视图内部的 FROM 和 JOIN 逻辑，
--    禁止修改字段顺序、字段类型、字段名称。
-- ⚠️ V1 状态：WHERE 1=0 返回 0 行（空契约骨架）
--    V1.1/V2：5号位替换内部的 SELECT 和 ERP 源表 JOIN，不改变对外契约
-- 字段语义红线：
...
--   视图行范围：尚未全部收货 / 尚未关闭 / 尚未取消 / 剩余数量 > 0 / 能明确目的工厂 / 有效厂间调拨数据
CREATE OR ALTER VIEW dbo.ERP_InterplantInTransit_View
AS
SELECT
    CAST(NULL AS INT)             AS MasterID,              -- ERP物料主数据物理ID（通过 MaterialMapping.SourceID 映射 APS 物料）
    CAST(NULL AS NVARCHAR(100))   AS MaterialCode,          -- ERP物料编码（业务追溯字段；不能替代 MasterID 的权威映射作用）
    CAST(NULL AS NVARCHAR(50))    AS SourceFactoryCode,     -- 发出工厂编码（仅用于物流追溯）
    CAST(NULL AS NVARCHAR(50))    AS FactoryCode,           -- 目的工厂编码（收货工厂；映射 SupplyFact_Pipeline.FactoryId）
    CAST('INTERPLANT_IN_TRANSIT'
         AS NVARCHAR(50))         AS SupplyType,            -- 供给类型（本视图固定为 INTERPLANT_IN_TRANSIT）
    CAST('OWNED' AS NVARCHAR(20)) AS OwnershipType,         -- 所有权类型（厂间在途默认 OWNED）
    CAST('AVAILABLE'
         AS NVARCHAR(20))         AS QualityStatus,         -- 质量状态（默认 AVAILABLE）
    CAST(NULL AS DECIMAL(18,4))   AS Quantity,              -- 在途剩余数量（尚未收货、尚未关闭的剩余在途数量）
    CAST(NULL AS DATETIME2)       AS ETA,                   -- ERP原始预计到达时间（ODS不得加入APS提前期偏移）
    CAST(NULL AS NVARCHAR(50))    AS StorageCode,           -- 目的仓库编码（预计收货仓库/库存点）
    CAST(NULL AS NVARCHAR(50))    AS SupplierCode,          -- 供应方编码（厂间在途可为空）
    CAST(NULL AS NVARCHAR(100))   AS SourceDocumentNo,      -- ERP来源单据号（调拨单/出库单/运输单）
    CAST(NULL AS NVARCHAR(50))    AS SourceDocumentLineNo,  -- ERP来源单据行号
    CAST(NULL AS DATETIME2)       AS SourceUpdatedAt        -- ERP来源更新时间（用于增量同步/数据新鲜度检查）
WHERE 1 = 0;
GO
...
-- 7. 指向ODS的ERP库存视图（v5.2.4净可用量契约）
-- ODS字段集合保持：MaterialCode, MasterID, WarehouseCode, Quantity, FactoryCode, SnapshotTime, IsActive。
-- Quantity由5号位在APS专用ODS视图中定义为：
--   CASE WHEN TotalQty-WasterQty<0 THEN 0 ELSE TotalQty-WasterQty END
-- 即APS净可用量，不等于ERP账面总量；WasterQty不向APS透传，下游不得再次扣减。
CREATE OR ALTER VIEW dbo.ext_ERP_Inventory_View AS
SELECT
    MaterialCode,
    MasterID,
    WarehouseCode,
...
--    禁止修改字段顺序、字段类型、字段名称。
-- 尚未接入真实来源时允许返回0行；一旦ODS视图实现真实SELECT，即进入v5.2.0统一管道主链
CREATE OR ALTER VIEW dbo.ext_ERP_InterplantInTransit_View AS
SELECT
    MasterID,
    MaterialCode,
    SourceFactoryCode,
...
-- =============================================
-- 9a. 历史多来源统一输入骨架（v5.2.0 Batch 5 末尾正式覆盖）
-- =============================================
-- 所属层：APS 层
-- 所属库：APS_Production
-- 维护责任人：2号位
-- 定位：将 ODS 层所有管道供给来源视图 UNION ALL 收敛为统一输入
-- 优势：未来新增管道供给来源（采购在途/VMI/已到厂未入库/供应商锁定）时，
--       只需在 ODS 层新增对应契约视图 + 在本视图追加 UNION ALL 分支，
--       sp_SyncPipelineSupply 主流程零改动。
-- ⚠️ 本处保留v5.0.42历史placeholder实现；当前权威视图位于Batch 5，统一引用四个单来源包装视图。
CREATE OR ALTER VIEW dbo.ext_PipelineSupply_Source_View
AS
-- ⚠️ 本视图输出 15 列：14字段 ODS 契约 + 1列派生 SourceSystem
--    ODS 单来源契约视图保持 14 字段不变；APS 统一输入视图额外派生 SourceSystem
--    供 sp_SyncPipelineSupply 的 SourceRowKey 和写入 SupplyFact_Pipeline.SourceSystem 使用
-- 分支1：厂间在途（V1 已建，返回0行空契约）
SELECT
    MasterID, MaterialCode, SourceFactoryCode, FactoryCode,
    SupplyType, OwnershipType, QualityStatus, Quantity,
    ETA, StorageCode, SupplierCode,
    SourceDocumentNo, SourceDocumentLineNo, SourceUpdatedAt,
    'ERP'                     AS SourceSystem      -- 派生列：来源系统标识
FROM dbo.ext_ERP_InterplantInTransit_View

UNION ALL

-- 分支2：采购在途（V1.1/V2 5号位新建 ODS 契约视图后启用）
SELECT
    ...
    CAST('PURCHASE_IN_TRANSIT'
         AS NVARCHAR(50))        AS SupplyType,
    CAST('OWNED'  AS NVARCHAR(20)) AS OwnershipType,
    CAST('AVAILABLE'
         AS NVARCHAR(20))        AS QualityStatus,
    ...
    'PROCUREMENT'                AS SourceSystem      -- 派生列

WHERE 1 = 0          -- V1.1/V2：替换为 ext_ERP_PurchaseInTransit_View

UNION ALL

-- 分支3：VMI 在途（V1.1/V2 预留）
SELECT
    CAST(NULL AS INT)            AS MasterID,
    CAST(NULL AS NVARCHAR(100))  AS MaterialCode,
    CAST(NULL AS NVARCHAR(50))   AS SourceFactoryCode,
    ...
    CAST('VMI_ONSITE'
         AS NVARCHAR(50))        AS SupplyType,
    CAST('CONSIGNMENT'
         AS NVARCHAR(20))        AS OwnershipType,
    CAST('AVAILABLE'
         AS NVARCHAR(20))        AS QualityStatus,
    CAST(NULL AS DECIMAL(18,4))  AS Quantity,
    CAST(NULL AS DATETIME2)      AS ETA,
    ...
    CAST(NULL AS NVARCHAR(50))   AS SourceDocumentLineNo,
    CAST(NULL AS DATETIME2)      AS SourceUpdatedAt,
    'VMI'                        AS SourceSystem      -- 派生列

WHERE 1 = 0          -- V1.1/V2：替换为 ext_ERP_VMI_View

UNION ALL

-- 分支4：已到厂未入库（V1.1/V2 预留）
SELECT
    CAST(NULL AS INT)            AS MasterID,
    CAST(NULL AS NVARCHAR(100))  AS MaterialCode,
    CAST(NULL AS NVARCHAR(50))   AS SourceFactoryCode,
    CAST(NULL AS NVARCHAR(50))   AS FactoryCode,
    CAST('ARRIVED_NOT_RECEIVED'
         AS NVARCHAR(50))        AS SupplyType,
    CAST('OWNED'  AS NVARCHAR(20)) AS OwnershipType,
    CAST('PENDING_INSPECTION'
         AS NVARCHAR(20))        AS QualityStatus,
    CAST(NULL AS DECIMAL(18,4))  AS Quantity,
    CAST(NULL AS DATETIME2)      AS ETA,
    CAST(NULL AS NVARCHAR(50))   AS StorageCode,
    ...
    CAST(NULL AS NVARCHAR(50))   AS SourceDocumentLineNo,
    CAST(NULL AS DATETIME2)      AS SourceUpdatedAt,
    'ERP'                     AS SourceSystem      -- 派生列（已到厂未入库同属 ERP 范畴）

WHERE 1 = 0          -- V1.1/V2：替换为 ext_ERP_ArrivedNotReceived_View
;
GO
...
-- =============================================
-- v5.2.2：每100个PlanVersion共用一个分区，预建至PlanVersionId=100000。
-- 这样避免“每版本一个分区”在多Domain每日运行下迅速耗尽边界；后续扩容由DBA在接近100000前评估SPLIT RANGE。
...
AS RANGE RIGHT FOR VALUES (
    ...
    37100, 37200, 37300, 37400, 37500, 37600, 37700, 37800, 37900, 38000,
    38100, 38200, 38300, 38400, 38500, 38600, 38700, 38800, 38900, 39000,
    39100, 39200, 39300, 39400, 39500, 39600, 39700, 39800, 39900, 40000,
    40100, 40200, 40300, 40400, 40500, 40600, 40700, 40800, 40900, 41000,
    41100, 41200, 41300, 41400, 41500, 41600, 41700, 41800, 41900, 42000,
    42100, 42200, 42300, 42400, 42500, 42600, 42700, 42800, 42900, 43000,
    43100, 43200, 43300, 43400, 43500, 43600, 43700, 43800, 43900, 44000,
    44100, 44200, 44300, 44400, 44500, 44600, 44700, 44800, 44900, 45000,
    45100, 45200, 45300, 45400, 45500, 45600, 45700, 45800, 45900, 46000,
    46100, 46200, 46300, 46400, 46500, 46600, 46700, 46800, 46900, 47000,
    47100, 47200, 47300, 47400, 47500, 47600, 47700, 47800, 47900, 48000,
    48100, 48200, 48300, 48400, 48500, 48600, 48700, 48800, 48900, 49000,
    49100, 49200, 49300, 49400, 49500, 49600, 49700, 49800, 49900, 50000,
    ...
    61100, 61200, 61300, 61400, 61500, 61600, 61700, 61800, 61900, 62000,
    62100, 62200, 62300, 62400, 62500, 62600, 62700, 62800, 62900, 63000,
    63100, 63200, 63300, 63400, 63500, 63600, 63700, 63800, 63900, 64000,
    64100, 64200, 64300, 64400, 64500, 64600, 64700, 64800, 64900, 65000,
    65100, 65200, 65300, 65400, 65500, 65600, 65700, 65800, 65900, 66000,
    66100, 66200, 66300, 66400, 66500, 66600, 66700, 66800, 66900, 67000,
    67100, 67200, 67300, 67400, 67500, 67600, 67700, 67800, 67900, 68000,
    68100, 68200, 68300, 68400, 68500, 68600, 68700, 68800, 68900, 69000,
    69100, 69200, 69300, 69400, 69500, 69600, 69700, 69800, 69900, 70000,
    70100, 70200, 70300, 70400, 70500, 70600, 70700, 70800, 70900, 71000,
    71100, 71200, 71300, 71400, 71500, 71600, 71700, 71800, 71900, 72000,
    72100, 72200, 72300, 72400, 72500, 72600, 72700, 72800, 72900, 73000,
    73100, 73200, 73300, 73400, 73500, 73600, 73700, 73800, 73900, 74000,
    74100, 74200, 74300, 74400, 74500, 74600, 74700, 74800, 74900, 75000,
    75100, 75200, 75300, 75400, 75500, 75600, 75700, 75800, 75900, 76000,
    76100, 76200, 76300, 76400, 76500, 76600, 76700, 76800, 76900, 77000,
    77100, 77200, 77300, 77400, 77500, 77600, 77700, 77800, 77900, 78000,
    78100, 78200, 78300, 78400, 78500, 78600, 78700, 78800, 78900, 79000,
    79100, 79200, 79300, 79400, 79500, 79600, 79700, 79800, 79900, 80000,
    80100, 80200, 80300, 80400, 80500, 80600, 80700, 80800, 80900, 81000,
    81100, 81200, 81300, 81400, 81500, 81600, 81700, 81800, 81900, 82000,
    82100, 82200, 82300, 82400, 82500, 82600, 82700, 82800, 82900, 83000,
    83100, 83200, 83300, 83400, 83500, 83600, 83700, 83800, 83900, 84000,
    84100, 84200, 84300, 84400, 84500, 84600, 84700, 84800, 84900, 85000,
    85100, 85200, 85300, 85400, 85500, 85600, 85700, 85800, 85900, 86000,
    86100, 86200, 86300, 86400, 86500, 86600, 86700, 86800, 86900, 87000,
    87100, 87200, 87300, 87400, 87500, 87600, 87700, 87800, 87900, 88000,
    88100, 88200, 88300, 88400, 88500, 88600, 88700, 88800, 88900, 89000,
    89100, 89200, 89300, 89400, 89500, 89600, 89700, 89800, 89900, 90000,
    90100, 90200, 90300, 90400, 90500, 90600, 90700, 90800, 90900, 91000,
    91100, 91200, 91300, 91400, 91500, 91600, 91700, 91800, 91900, 92000,
    92100, 92200, 92300, 92400, 92500, 92600, 92700, 92800, 92900, 93000,
    93100, 93200, 93300, 93400, 93500, 93600, 93700, 93800, 93900, 94000,
    94100, 94200, 94300, 94400, 94500, 94600, 94700, 94800, 94900, 95000,
    95100, 95200, 95300, 95400, 95500, 95600, 95700, 95800, 95900, 96000,
    96100, 96200, 96300, 96400, 96500, 96600, 96700, 96800, 96900, 97000,
    97100, 97200, 97300, 97400, 97500, 97600, 97700, 97800, 97900, 98000,
    98100, 98200, 98300, 98400, 98500, 98600, 98700, 98800, 98900, 99000,
    99100, 99200, 99300, 99400, 99500, 99600, 99700, 99800, 99900, 100000
);
GO

-- V1仍使用PRIMARY文件组；生产环境可由DBA在容量评估后迁移到独立文件组，不改变表与索引契约。
...
CREATE TABLE InventoryBalance (
    ...
    AllocatedQty DECIMAL(18,4) NOT NULL DEFAULT 0,   -- V1兼容列：快照同步固定写0，排程分配不回写；占用权威在内存余额/Ledger/PSA
...
-- 【设计决策】InventoryBalance 定义不变；本链路结果为空时不影响现有排程
...
-- 【V1 空链路预留】ERP_InterplantInTransit_View 建空框架；sp_SyncPipelineSupply 空运行；
--                  SupplyFact_Pipeline 结果为空，不影响现有排程；V1.1 再填入真实 ERP 数据
...
CREATE TABLE SupplyFact_Pipeline (
    ...
    -- 当前已定义属性值：INTERPLANT_IN_TRANSIT / PURCHASE_IN_TRANSIT /
    ...
    AvailableTime     DATETIME2 NULL,                  -- 本地派生字段：= ETA + SupplyAvailabilityRule.LeadTimeOffset（小时）；sp_SyncPipelineSupply 装载时计算落库
...
-- v5.2.0当前口径：同步过程调用本表；未命中规则时采用基础纳入口径，命中IncludeFlag=0时明确排除。
...
-- =============================================
-- 2.8.3 历史空跑骨架（仅供追溯；v5.2.0 Batch 5 末尾以 CREATE OR ALTER 正式覆盖）
-- =============================================
-- ⚠️ 当前权威实现位于本脚本 Batch 5：
--   读取 ext_PipelineSupply_Source_View；按 @DataCutoffTime 截断；
--   执行物料/工厂映射、来源业务键去重与规则裁决；
--   只替换本次 @BatchNo 切片，禁止 TRUNCATE 全表。
-- 下列旧过程体保留是为了展示 v5.0.42 的历史演变，部署本脚本时会被后文正式实现覆盖。
CREATE OR ALTER PROCEDURE sp_SyncPipelineSupply
    @BatchNo          NVARCHAR(50),
    @DataCutoffTime   DATETIME2,          -- 历史骨架参数；正式实现见 Batch 5
    @RowsAffected     INT OUTPUT,
    @ErrorMessage     NVARCHAR(MAX) OUTPUT
AS
BEGIN
    ...
    BEGIN TRY
        -- ================================================================
        -- V1.1/V2 真实同步 TODO（V1 不执行；V1 始终执行最后的 TRUNCATE + SUCCESS）
        -- ================================================================
        --
        -- 时间标准：全链统一使用中国工厂本地时间（UTC+8）。
        --          ETA、AvailableTime、DataCutoffTime、SyncedAt、RuleEvaluatedAt
        --          禁止混用 UTC 与本地时间。
        -- 异常分类：
        --   · 技术失败（源不可访问/事务崩溃）= APS_ETL_Log FAILED + THROW
        --     禁止伪装成空集合继续排程
        --   · 业务异常（单行映射失败/数据质量）= 逐行跳过 + WARN 日志（限50条）
        -- ================================================================
        --
        -- Step 0  前置校验
        --         IF @DataCutoffTime IS NULL
        --             RAISERROR('@DataCutoffTime must not be NULL', 16, 1);
        --             RETURN;
        --         IF OBJECT_ID('dbo.ext_PipelineSupply_Source_View') IS NULL
        --             RAISERROR('ext_PipelineSupply_Source_View not found', 16, 1);
        --             RETURN;
        --
        -- ================================================================
        ...
        --       SourceSystem      -- ⚠️ 第15列（APS派生）；输入视图共15列
        --   FROM ext_PipelineSupply_Source_View    -- 🔒 仅读统一输入视图（15列）
        --   WHERE (SourceUpdatedAt IS NULL OR SourceUpdatedAt <= @DataCutoffTime)
        --     AND Quantity > 0                       -- 质量门禁
        --     AND FactoryCode IS NOT NULL            -- 目的工厂必须存在
        --     AND ((SupplyType <> 'INTERPLANT_IN_TRANSIT')
        --          OR (SupplyType = 'INTERPLANT_IN_TRANSIT'
        --              AND SourceDocumentNo IS NOT NULL
        --              AND SourceDocumentLineNo IS NOT NULL
        --              AND StorageCode IS NOT NULL))  -- 厂间在途幂等键+P0-4门禁
        --   ;   -- SourceUpdatedAt IS NULL 策略：保留兼容，登记
        --       -- PIPELINE_SOURCE_TIME_MISSING WARN（限 50 条）
        --   -- 缺失 SourceDocumentLineNo 的 INTERPLANT_IN_TRANSIT 记录：
        --       -- 登记 PIPELINE_SOURCE_KEY_MISSING → 跳过不写入
        --       -- ODS 须用 ERP 源表主键生成稳定行标识
        --
        -- ================================================================
        -- Step 2  物理物料身份映射（一物多仓正确映射）
        -- ================================================================
        --   映射链（复用现货库存 ETL 的仓库级匹配设计）：
        --
        --     #Raw.MasterID
        --   + #Raw.StorageCode
        --   + Source='ERP'
        --   + IsCurrent=1
        --     ↓
        --     MaterialMapping.SourceID
        --   + MaterialMapping.Warehouse_Norm = ISNULL(#Raw.StorageCode, 'N/A')
        --     ↓
        --     MaterialMapping.MaterialCode
        --     ↓
        --     Material.MaterialCode
        --     ↓
        --     Material.Id / Material.ProductFamilyId
        --
        --   INSERT INTO #MappedPipelineSupply_Material (...) SELECT ...
        --   FROM #RawPipelineSupply v
        ...
        --   ;
        --
        --   映射验证（P0-4）：在 Step1 临时表阶段按组成字段生成同口径 SourceRowKey（见 Step1），
        --    然后 GROUP BY SourceRowKey HAVING COUNT(*)>1 → 登记 PIPELINE_MATERIAL_MAPPING_AMBIGUOUS
        --    若不在 #Raw 阶段预生成 SourceRowKey，也可直接按组成字段分组：
        --     GROUP BY SourceSystem, SupplyType,
        --              SourceDocumentNo, SourceDocumentLineNo,
        --              MasterID, StorageCode, FactoryCode
        --     HAVING COUNT(*) > 1
        --   → MappingCount=0 → PIPELINE_MATERIAL_MAPPING_NOT_FOUND
        --   → MappingCount=1 → 正常继续
        --   → MappingCount>1 → PIPELINE_MATERIAL_MAPPING_AMBIGUOUS → 跳过不写入
        --
        --   MaterialCode 交叉校验：
        --     正常：v.MaterialCode = mm.MaterialCode = mat.MaterialCode
        --     不一致：登记 PIPELINE_MATERIALCODE_MISMATCH WARN
        --     写入 SupplyFact_Pipeline.MaterialCode 时使用：
        --     mat.MaterialCode（APS 标准编码），禁止用 v.MaterialCode 覆盖
        --
        --   StorageCode 必填规则（P0-3）：
        --     SupplyType = INTERPLANT_IN_TRANSIT 且 StorageCode IS NULL：
        --       跳过，登记 PIPELINE_STORAGECODE_MISSING
        --     （其他 SupplyType 按各自业务规则单独定义）
        --
        -- ================================================================
        -- Step 3  工厂映射（INNER JOIN，失败整行跳过）
        -- ================================================================
        --   INNER JOIN Factory f
        --       ON f.Code       = v.FactoryCode
        --      AND f.IsActive   = 1
        --   映射失败 → 登记 PIPELINE_FACTORY_NOT_FOUND → 跳过
        --
        -- ================================================================
        -- Step 4  规则唯一胜出裁决（OUTER APPLY TOP 1）
        -- ================================================================
        -- ⚠️ 使用 OUTER APPLY (SELECT TOP(1) ... ORDER BY) 确保唯一胜出规则
        --   禁止普通 JOIN 后直接 INSERT（会因规则匹配产生重复供给）
        --
        --   OUTER APPLY
        --   (
        --       SELECT TOP (1)
        ...
        --   规则行为：
        --     NULL（无命中）：
        --       不写入，登记 NoRuleMatch，IsAvailable=0
        --     winner.IncludeFlag = 0：
        --       不写入
        --     winner.IncludeFlag = 1：
        --       写入 SupplyFact_Pipeline；
        --       AvailableTime = DATEADD(HOUR, winner.LeadTimeOffset, v.ETA)
        --       SupplyAvailabilityRuleId = winner.RuleId
        --       AppliedLeadTimeOffset   = winner.LeadTimeOffset
        --       RulePriority            = 规则 Priority 值
        --       RuleEvaluatedAt         = @DataCutoffTime
        --
        -- ================================================================
        -- Step 5  写入 SupplyFact_Pipeline（事务 + 批次原子替换）
        -- ================================================================
        --   SET XACT_ABORT ON;
        --   BEGIN TRY
        --       BEGIN TRANSACTION;
        --
        --       DELETE FROM SupplyFact_Pipeline
        ...
        --       )
        --       SELECT
        --           mat.MaterialCode                AS MaterialCode,
        --           mat.Id                          AS MaterialId,
        --           f.Code                          AS FactoryCode,
        --           f.Id                            AS FactoryId,
        --           mat.ProductFamilyId             AS ProductFamilyId,
        --           v.SupplyType,
        --           v.OwnershipType,
        --           v.QualityStatus,
        --           v.Quantity,
        --           v.ETA,
        --           DATEADD(HOUR, ISNULL(winner.LeadTimeOffset, 0), v.ETA)
        --                                            AS AvailableTime,
        --           v.StorageCode,
        --           v.SupplierCode,
        ...
        --           1                               AS IsActive,
        --           GETDATE()                       AS SyncedAt
        --       FROM #MappedPipelineSupply_Material v
        --       INNER JOIN Material mat
        --           ON mat.MaterialCode = v.MaterialCode AND mat.IsActive = 1
        --       INNER JOIN Factory f
        --           ON f.Code = v.FactoryCode AND f.IsActive = 1
        --       OUTER APPLY (...) winner             -- Step 4 胜出规则
        --       WHERE 1=1
        --         AND winner.IncludeFlag = 1
        --         AND v.ETA IS NOT NULL              -- V1.1/V2 ETA=NULL处理策略待业务确认；当前无ETA字段授权机制
        --       ;
        --
        --       SET @RowsAffected = @@ROWCOUNT;
        --
        ...
        --               CONCAT('管道供给同步完成: ', @RowsAffected, ' 行写入 SupplyFact_Pipeline'),
        --               'SUCCESS', GETDATE());
        --
        --       COMMIT TRANSACTION;
        --   END TRY
        ...
        --       IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
        --       SET @ErrorMessage =
        --           CONCAT('sp_SyncPipelineSupply 技术失败: ', ERROR_MESSAGE());
        --       INSERT INTO APS_ETL_Log (BatchNo, Step, Message, Status, CreatedAt)
        --       VALUES (@BatchNo, 'sp_SyncPipelineSupply',
        --               @ErrorMessage, 'FAILED', GETDATE());
        --       THROW;    -- ⚠️ 技术失败必须向上传播，禁止伪装成空集合
        --   END CATCH
        --
        -- ================================================================
        -- 禁止实现清单（P0）
        -- ================================================================
        --   ❌ 只按 MasterID 查 MaterialMapping（忽略 StorageCode 仓库维度）
        --   ❌ TOP 1 随机取多仓库映射
        --   ❌ 按 MaterialCode 对在途事实去重
        --   ❌ 普通 LEFT JOIN SupplyAvailabilityRule（可能产生重复供给）
        --   ❌ 使用 GETDATE() 判断规则有效期与源数据时间
        --   ❌ FactoryId=NULL 写入 SupplyFact_Pipeline
        --   ❌ DELETE 后 INSERT 失败留下空/半批次
        --   ❌ SP 失败后写 SUCCESS 日志
        --   ❌ 采购在途 UPDATE Inventory
        --   ❌ 修改14字段 ODS 契约结构
        -- ----------------------------------------------------------------
        -- ----------------------------------------------------------------

        -- v5.2.1安全修正：历史骨架不得再执行TRUNCATE。
        -- 完整脚本后文Batch 5会CREATE OR ALTER为正式实现；若部署中断在此处，调用必须明确失败而不是清空供给。
        THROW 51001, 'sp_SyncPipelineSupply deprecated skeleton: complete deployment through Batch 5 before execution', 1;

    END TRY
    BEGIN CATCH
        SET @ErrorMessage = ERROR_MESSAGE();
        INSERT INTO APS_ETL_Log (BatchNo, Step, Message, Status, CreatedAt)
        VALUES (@BatchNo, 'sp_SyncPipelineSupply', @ErrorMessage, 'FAILED', GETDATE());
        THROW;   -- ⚠️ 技术失败必须向上传播；调度器不得以空集合继续排程
    END CATCH
END;
GO
...
-- 消费方：1号位排程引擎（全量重算前扣减已完成量）；ScheduleContext 装载
...
CREATE TABLE MESWorkOrderSnapshot (
    ...
    ReleaseItemKey           NVARCHAR(50)  NULL,                        -- v5.2.2：APS发布幂等键；MES读取APS发布视图后原样保存并在工单视图回传；历史MES工单可为空
...
CREATE INDEX IX_MESWorkOrderSnapshot_ReleaseItemKey
    ON MESWorkOrderSnapshot (ScheduleRunId, ReleaseItemKey)
    INCLUDE (MESWorkOrderNo, ProductionInstructionNo, WorkOrderStatus, SourceUpdatedAt)
    WHERE ReleaseItemKey IS NOT NULL;
GO
...
-- 2号位装载：按OperationName+StageCode联合识别工序，用GoodQty裁剪内存TaskDraft后传给1号位；1号位不直接查询本表
-- 注意：StageScopeType（EDGE / ROOT）是BOM侧APS_BOM_STAGE_PATH_RAW字段，不属于本表；由2号位完成关联和裁剪后传入1号位，1号位不直接消费数据库快照
...
-- 调用边界：由00:40独立Hangfire Job调用；不由INightlyBatchOrchestrator或ISchedulingOrchestrator直接调用。
-- =============================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO MESWorkOrderSnapshot (
            ScheduleRunId, ReleaseItemKey, ProductionInstructionNo, MESWorkOrderNo, MaterialCode,
    ...
    BEGIN CATCH
        ...
        THROW;  -- MES现实工单快照是执行锁与发布对账的权威输入；技术失败不得以空快照继续排程
    END CATCH
END;
GO
...
-- 调用边界：由00:45独立Hangfire Job调用；不由INightlyBatchOrchestrator或ISchedulingOrchestrator直接调用。
-- =============================================
...
-- 调用边界：由00:50独立Hangfire Job调用；不由INightlyBatchOrchestrator或ISchedulingOrchestrator直接调用。
-- =============================================
...
BEGIN
    ...
    BEGIN CATCH
        ...
        THROW;  -- Stage累计事实是PI位置权威输入，失败必须阻断对应运行
    END CATCH
END;
GO
...
CREATE TABLE [Order] (
    ...
    Priority INT NOT NULL DEFAULT 50,
    PriorityScore DECIMAL(10,2) NULL,
    Status NVARCHAR(50) NOT NULL,
...
CREATE TABLE FenceConfig (
    ...
    FirmDays INT NOT NULL,                            -- 锁定区天数：需要审批才能修改
    EffectiveFrom DATE NOT NULL,
...
-- =============================================
-- 2.14 TaskSplitRuleConfig（拆批规则配置表）⚠️ 2026-04-03审计补充
-- =============================================
...
-- 2号位装载：读取StageDetail阶段顺序，并对无RoutingOperation的阶段查询本表生成内存标准TaskDraft；再通过方法参数传给1号位
-- 命中顺序（从细到粗）：
...
BEGIN
    ...
    SELECT 
        ...
        oc.Priority,
        NULL AS PriorityScore,  -- ⚠️ TODO: 由1号位排程引擎根据规则计算
        oc.Status,
...
-- 3号位/INightlyBatchOrchestrator 最晚00:38前只创建记录（Status=RUNNING）并确定DataCutoffTime；
-- 00:40/00:45/00:50三个独立定时Job分别调用MES快照SP，并使用同一ScheduleRunId+DataCutoffTime；
...
ALTER TABLE PlanVersion
    ADD SourceScheduleRunId   INT           NOT NULL;               -- FK→ScheduleRun.Id；流程保证 ScheduleRun 先创建（00:38），PlanVersion 后创建（02:00）
ALTER TABLE PlanVersion
...
CREATE TABLE OrderScheduleSummary (
    ...
    OrderId         INT           NOT NULL,              -- 逻辑关联 Order.Id；数据库未建立物理外键
...
-- ⚠️ V1：白天实时评估中的正式业务容器，业务适用时创建（INSERT_ORDER_WHATIF 可使用 Scenario 保存假设和目标）；不强制每个运行创建（ScheduleRun.ScenarioId 可空）。
...
CREATE OR ALTER VIEW dbo.ERP_Received_ByDocument_View
AS
SELECT
    CAST(NULL AS NVARCHAR(50))   AS FactoryCode,
    ...
    CAST(NULL AS INT)            AS MasterID,
    CAST(NULL AS NVARCHAR(100))  AS MaterialCode,
    CAST(NULL AS NVARCHAR(50))   AS DocumentType,       -- SHIPPING_INSTRUCTION / PRODUCTION_INSTRUCTION / UNKNOWN
    CAST(NULL AS NVARCHAR(100))  AS DocumentNo,          -- ERP Received 原始单据号
    CAST(NULL AS DECIMAL(18,4))  AS ReceivedQty,         -- 按单据汇总入库数量
    CAST(NULL AS DATETIME2)      AS LastReceivedAt,
    CAST(NULL AS DATETIME2)      AS SourceUpdatedAt,
    CAST(NULL AS BIT)            AS IsActive
WHERE 1 = 0;   -- V1 空契约骨架；V1.1/V2 5号位替换为实际 ERP Received 汇总逻辑
GO
...
AS
SELECT
    FactoryCode, WarehouseCode, MasterID, MaterialCode,
...
-- =============================================
-- Batch 4：Pegging软硬锁、PI位置快照、MES发布承诺与跨版本执行闭环（v5.2.2，2026-07-29）
-- 对齐：核心走查 V3.19 / 演变总表 v3.34 / 防腐层 v1.38 / 集成接口 v1.28 / 字段说明 v5.2.2
-- =============================================

-- -----------------------------------------------
-- 4.1 MESPlanRelease（MES拉取模式的稳定发布快照）
-- -----------------------------------------------
-- APS与MES采用双向视图集成：APS不主动调用MES建单接口。
-- 一条MESPlanRelease对应一张未来MES现实工单，可关联同一生产指示、同一Stage下的多个APS小工序Task。
-- Quantity取Stage级执行批次的单一流转数量，禁止SUM关联小工序Task.Quantity；数量不一致且无显式换算闭合时不得组装发布单元。
-- ReleaseItemKey由APS生成并永久不变，是跨系统唯一幂等键；TaskNo仅用于展示和诊断。
CREATE TABLE MESPlanRelease (
    Id                          BIGINT IDENTITY(1,1) PRIMARY KEY,
    ReleaseItemKey              NVARCHAR(50)  NOT NULL,
    SourcePlanVersionId         INT           NOT NULL,
    ProductionInstructionNo     NVARCHAR(100) NOT NULL,
    MaterialId                  INT           NOT NULL,
    FactoryId                   INT           NOT NULL,
    StageCode                   NVARCHAR(50)  NOT NULL,
    Quantity                    DECIMAL(18,4) NOT NULL,                    -- Stage级执行批次单一流转数量；禁止对关联串行小工序Task.Quantity求和
    PlannedStartTime            DATETIME2     NULL,
    PlannedEndTime              DATETIME2     NULL,
    PublishStatus               NVARCHAR(20)  NOT NULL DEFAULT 'PUBLISHED',
    PublishedAt                 DATETIME2     NOT NULL DEFAULT GETDATE(),
    ConsumedAt                  DATETIME2     NULL,
    MESWorkOrderNo              NVARCHAR(100) NULL,
    CancelledAt                 DATETIME2     NULL,
    CreatedBy                   NVARCHAR(100) NULL,
    CreatedAt                   DATETIME2     NOT NULL DEFAULT GETDATE(),
    UpdatedAt                   DATETIME2     NOT NULL DEFAULT GETDATE(),
    RowVersion                  ROWVERSION,
    CONSTRAINT UQ_MESPlanRelease_Key UNIQUE (ReleaseItemKey),
    CONSTRAINT FK_MESPlanRelease_PlanVersion FOREIGN KEY (SourcePlanVersionId) REFERENCES PlanVersion(Id),
    CONSTRAINT FK_MESPlanRelease_Material FOREIGN KEY (MaterialId) REFERENCES Material(Id),
    CONSTRAINT FK_MESPlanRelease_Factory FOREIGN KEY (FactoryId) REFERENCES Factory(Id),
    CONSTRAINT CK_MESPlanRelease_Qty CHECK (Quantity > 0),
    CONSTRAINT CK_MESPlanRelease_Status CHECK (PublishStatus IN ('PUBLISHED','CONSUMED','CANCELLED')),
    CONSTRAINT CK_MESPlanRelease_Time CHECK (
        PlannedStartTime IS NULL OR PlannedEndTime IS NULL OR PlannedEndTime >= PlannedStartTime
    ),
    CONSTRAINT CK_MESPlanRelease_Lifecycle CHECK (
        (PublishStatus = 'PUBLISHED' AND ConsumedAt IS NULL AND MESWorkOrderNo IS NULL AND CancelledAt IS NULL)
        OR (PublishStatus = 'CONSUMED' AND ConsumedAt IS NOT NULL AND MESWorkOrderNo IS NOT NULL AND CancelledAt IS NULL)
        OR (PublishStatus = 'CANCELLED' AND CancelledAt IS NOT NULL AND ConsumedAt IS NULL AND MESWorkOrderNo IS NULL)
    )
);
GO

CREATE INDEX IX_MESPlanRelease_Status
    ON MESPlanRelease(PublishStatus, PublishedAt)
    INCLUDE (ReleaseItemKey, ProductionInstructionNo, StageCode, Quantity, PlannedStartTime, PlannedEndTime);
CREATE INDEX IX_MESPlanRelease_InstructionStage
    ON MESPlanRelease(ProductionInstructionNo, StageCode, PublishStatus)
    INCLUDE (ReleaseItemKey, Quantity, MESWorkOrderNo);
GO

-- MES读取本视图；同一ReleaseItemKey无论被轮询多少次，MES最多创建一张工单。
-- 跨系统契约只暴露MaterialCode/FactoryCode等稳定业务编码，APS内部MaterialId/FactoryId不对MES公开。
-- 视图保留CANCELLED行，MES在真正落工单前必须再次读取当前状态，避免“已读取但尚未建单”竞态。
CREATE OR ALTER VIEW dbo.APS_MES_PlanRelease_View
AS
SELECT
    r.ReleaseItemKey,
    r.ProductionInstructionNo,
    r.StageCode,
    m.MaterialCode,
    f.Code AS FactoryCode,
    r.Quantity,
    ...
    r.PublishedAt,
    r.UpdatedAt
FROM dbo.MESPlanRelease AS r
INNER JOIN dbo.Material AS m ON m.Id = r.MaterialId
INNER JOIN dbo.Factory  AS f ON f.Id = r.FactoryId;
GO

-- -----------------------------------------------
-- 4.2 ExecutionLock（跨版本现实执行锁）
-- -----------------------------------------------
-- 定位：MES实时工单视图确认ReleaseItemKey已被建单后形成的现实执行事实，以MESWorkOrderNo为跨PlanVersion稳定锚点。
-- 红线：RELEASED只自动固定生产指示+Stage+MES工单+剩余执行量；普通通用产出归属次日仍可重新Pegging。
-- V1不建设完整事件溯源；本表保存当前权威状态，关键人工操作进入既有审计日志。
CREATE TABLE ExecutionLock (
    Id                                  BIGINT IDENTITY(1,1) PRIMARY KEY,
    ExecutionLockNo                     NVARCHAR(50)  NOT NULL,
    SourceSystem                        NVARCHAR(50)  NOT NULL DEFAULT 'MES',
    MESWorkOrderNo                      NVARCHAR(100) NOT NULL,
    MESPlanReleaseId                     BIGINT        NULL, -- v5.2.2：新APS发布工单必须关联；历史/外部MES工单允许为空
    ProductionInstructionNo             NVARCHAR(100) NOT NULL,
    MaterialId                          INT           NOT NULL,
    FactoryId                           INT           NOT NULL,
    StageCode                           NVARCHAR(50)  NOT NULL,
    OriginalExecutionQty                DECIMAL(18,4) NOT NULL,
    CompletedQty                        DECIMAL(18,4) NOT NULL DEFAULT 0,
    CancelledQty                        DECIMAL(18,4) NOT NULL DEFAULT 0,
    RemainingExecutionQty               DECIMAL(18,4) NOT NULL, -- 2号位按MES现实工单状态维护；未来Stage产出承诺上限，不用于小工序Task数量
    AvailableTime                       DATETIME2     NULL,
    Status                              NVARCHAR(20)  NOT NULL DEFAULT 'ACTIVE',
    OriginalTriggerDemandOrderCanonicalId BIGINT     NULL, -- 仅追溯，不表示永久需求归属
    LastMESUpdatedAt                    DATETIME2     NULL,
    CreatedAt                           DATETIME2     NOT NULL DEFAULT GETDATE(),
    UpdatedAt                           DATETIME2     NOT NULL DEFAULT GETDATE(),
    RowVersion                          ROWVERSION,
    CONSTRAINT UQ_ExecutionLock_No UNIQUE (ExecutionLockNo),
    CONSTRAINT UQ_ExecutionLock_MESWorkOrder UNIQUE (SourceSystem, MESWorkOrderNo),
    CONSTRAINT FK_ExecutionLock_MESPlanRelease FOREIGN KEY (MESPlanReleaseId) REFERENCES MESPlanRelease(Id),
    CONSTRAINT FK_ExecutionLock_Material FOREIGN KEY (MaterialId) REFERENCES Material(Id),
    CONSTRAINT FK_ExecutionLock_Factory FOREIGN KEY (FactoryId) REFERENCES Factory(Id),
    CONSTRAINT FK_ExecutionLock_OriginalDemand FOREIGN KEY (OriginalTriggerDemandOrderCanonicalId) REFERENCES Order_Canonical(Id),
    CONSTRAINT CK_ExecutionLock_Qty CHECK (
        OriginalExecutionQty > 0
        AND CompletedQty >= 0 AND CancelledQty >= 0 AND RemainingExecutionQty >= 0
        AND CompletedQty + CancelledQty + RemainingExecutionQty <= OriginalExecutionQty
    ),
    CONSTRAINT CK_ExecutionLock_Status CHECK (Status IN ('ACTIVE','COMPLETED','CANCELLED')),
    CONSTRAINT CK_ExecutionLock_Lifecycle CHECK (
        (Status = 'ACTIVE' AND RemainingExecutionQty > 0)
        OR (Status = 'COMPLETED' AND RemainingExecutionQty = 0)
        OR (Status = 'CANCELLED' AND RemainingExecutionQty = 0 AND CancelledQty > 0)
    )
);
GO

CREATE UNIQUE INDEX UQ_ExecutionLock_MESPlanRelease
    ON ExecutionLock(MESPlanReleaseId)
    WHERE MESPlanReleaseId IS NOT NULL;
CREATE INDEX IX_ExecutionLock_InstructionStage
    ON ExecutionLock(ProductionInstructionNo, StageCode, Status)
    INCLUDE (MESWorkOrderNo, MESPlanReleaseId, CompletedQty, CancelledQty, RemainingExecutionQty, AvailableTime);
CREATE INDEX IX_ExecutionLock_StatusAvailableTime
    ON ExecutionLock(Status, AvailableTime)
    INCLUDE (MaterialId, FactoryId, RemainingExecutionQty);
GO

-- -----------------------------------------------
-- 4.3 DemandSupplyHardLock（跨版本需求—供给硬归属）
-- -----------------------------------------------
-- 来源：特殊出荷指示类型、客户专属、质量/环保/认证资格、专属BOM/标签/包装、不可逆出货准备、人工审批冻结。
-- 普通RELEASED任务不自动写本表；未命中HardLock的ExecutionLock未来产出仍按最新优先级重新Pegging。
CREATE TABLE DemandSupplyHardLock (
    Id                          BIGINT IDENTITY(1,1) PRIMARY KEY,
    HardLockNo                  NVARCHAR(50)  NOT NULL,
    DemandOrderCanonicalId      BIGINT        NOT NULL,
    SupplyType                  NVARCHAR(50)  NOT NULL,
    SupplyBusinessKey           NVARCHAR(300) NOT NULL,
    ExecutionLockId             BIGINT        NULL,
    LockedQty                   DECIMAL(18,4) NOT NULL,
    FulfilledQty                DECIMAL(18,4) NOT NULL DEFAULT 0,
    ReleasedQty                 DECIMAL(18,4) NOT NULL DEFAULT 0,
    RemainingLockedQty AS (
        CASE
            WHEN LockedQty - FulfilledQty - ReleasedQty < 0 THEN CONVERT(DECIMAL(18,4), 0)
            ELSE LockedQty - FulfilledQty - ReleasedQty
        END
    ) PERSISTED,
    LockReason                  NVARCHAR(100) NOT NULL,
    RuleSetVersionId            BIGINT        NULL,
    EffectiveAt                 DATETIME2     NOT NULL DEFAULT GETDATE(),
    ...
    UpdatedAt                   DATETIME2     NOT NULL DEFAULT GETDATE(),
    RowVersion                  ROWVERSION,
    CONSTRAINT UQ_DemandSupplyHardLock_No UNIQUE (HardLockNo),
    CONSTRAINT FK_DSHL_Demand FOREIGN KEY (DemandOrderCanonicalId) REFERENCES Order_Canonical(Id),
    CONSTRAINT FK_DSHL_ExecutionLock FOREIGN KEY (ExecutionLockId) REFERENCES ExecutionLock(Id),
    CONSTRAINT FK_DSHL_RuleSetVersion FOREIGN KEY (RuleSetVersionId) REFERENCES RuleSetVersion(Id),
    CONSTRAINT CK_DSHL_Qty CHECK (
        LockedQty > 0 AND FulfilledQty >= 0 AND ReleasedQty >= 0
        AND FulfilledQty + ReleasedQty <= LockedQty
    ),
    CONSTRAINT CK_DSHL_Status CHECK (Status IN ('ACTIVE','COMPLETED','RELEASED','CANCELLED')),
    CONSTRAINT CK_DSHL_Lifecycle CHECK (
        -- ACTIVE允许部分履约与部分解除；只要RemainingLockedQty>0，剩余数量继续保持硬归属。
        (Status = 'ACTIVE' AND FulfilledQty + ReleasedQty < LockedQty
            AND CompletedAt IS NULL
            AND (ReleasedQty = 0 OR ReleasedAt IS NOT NULL))
        OR (Status = 'COMPLETED' AND FulfilledQty = LockedQty
            AND ReleasedQty = 0 AND CompletedAt IS NOT NULL)
        OR (Status = 'RELEASED' AND FulfilledQty + ReleasedQty = LockedQty
            AND ReleasedQty > 0 AND ReleasedAt IS NOT NULL)
        OR (Status = 'CANCELLED' AND FulfilledQty = 0
            AND ReleasedQty = LockedQty AND ReleasedAt IS NOT NULL)
    )
);
GO

CREATE INDEX IX_DSHL_ActiveDemand
    ON DemandSupplyHardLock(DemandOrderCanonicalId, Status)
    INCLUDE (SupplyType, SupplyBusinessKey, LockedQty, RemainingLockedQty, ExecutionLockId)
    WHERE Status = 'ACTIVE';
CREATE INDEX IX_DSHL_ActiveSupply
    ON DemandSupplyHardLock(SupplyType, SupplyBusinessKey, Status)
    INCLUDE (DemandOrderCanonicalId, LockedQty, RemainingLockedQty, ExecutionLockId)
    WHERE Status = 'ACTIVE';
CREATE INDEX IX_DSHL_ExecutionLock
    ON DemandSupplyHardLock(ExecutionLockId, Status)
    WHERE ExecutionLockId IS NOT NULL;
GO

-- -----------------------------------------------
-- 4.4 Task追加发布锚点、跨版本执行锚点与虚拟任务标识
-- -----------------------------------------------
-- Task.OrderId继续NOT NULL，但只表示代表订单/兼容查询入口；跨需求数量归属以PeggingAllocationLedger为权威。
-- ProductionInstructionNo+StageCode是正式执行身份；MTS_InstructionNo仅历史兼容。IsLocked仅为计划冻结，不等于ExecutionLock。
ALTER TABLE Task
ADD MESPlanReleaseId      BIGINT NULL,
    ExecutionLockId       BIGINT NULL,
    IsVirtual             BIT NOT NULL CONSTRAINT DF_Task_IsVirtual DEFAULT 0,
    ProductionInstructionNo NVARCHAR(100) NULL,
    StageCode             NVARCHAR(50) NULL;
GO

ALTER TABLE Task
ADD CONSTRAINT FK_Task_MESPlanRelease
    FOREIGN KEY (MESPlanReleaseId) REFERENCES MESPlanRelease(Id),
    CONSTRAINT FK_Task_ExecutionLock
    FOREIGN KEY (ExecutionLockId) REFERENCES ExecutionLock(Id);
GO

ALTER TABLE Task
ADD CONSTRAINT CK_Task_Status_v520
    CHECK (Status IN ('PLANNED','RELEASED','IN_PROGRESS','COMPLETED','CANCELLED'));
GO

ALTER TABLE Task
ADD CONSTRAINT CK_Task_VirtualExecution
    CHECK (
        IsVirtual = 0
        OR (MESPlanReleaseId IS NULL AND ExecutionLockId IS NULL AND ProductionInstructionNo IS NULL AND Status IN ('PLANNED','CANCELLED'))
    );
GO

ALTER TABLE Task
ADD CONSTRAINT CK_Task_ExecutionIdentity
    CHECK (
        (MESPlanReleaseId IS NULL AND ExecutionLockId IS NULL)
        OR (IsVirtual = 0 AND ProductionInstructionNo IS NOT NULL AND StageCode IS NOT NULL)
    );
GO

CREATE INDEX IX_Task_MESPlanRelease
    ON Task(MESPlanReleaseId, PlanVersionId)
    INCLUDE (TaskNo, Status, Quantity, ProductionInstructionNo, StageCode, PlannedStartTime, PlannedEndTime)
    WHERE MESPlanReleaseId IS NOT NULL
    ON PS_PlanVersion(PlanVersionId);
CREATE INDEX IX_Task_ExecutionLock
    ON Task(ExecutionLockId, PlanVersionId)
    INCLUDE (TaskNo, Status, Quantity, ProductionInstructionNo, StageCode, PlannedStartTime, PlannedEndTime)
    WHERE ExecutionLockId IS NOT NULL
    ON PS_PlanVersion(PlanVersionId);
CREATE INDEX IX_Task_Virtual
    ON Task(PlanVersionId, IsVirtual, Status)
    WHERE IsVirtual = 1
    ON PS_PlanVersion(PlanVersionId);
GO

-- -----------------------------------------------
-- 4.5 ProductionInstructionSupplySnapshot（PI总量闭合快照）
-- -----------------------------------------------
-- 隔离键：ScheduleRunId + DomainKey + ProductionInstructionNo。
-- 总量唯一口径：AvailableProductionQty=max(0,InstructionQty-ReceivedQty)。MES/XC/在途只定位，不再次扣减。
CREATE TABLE ProductionInstructionSupplySnapshot (
    Id                          BIGINT IDENTITY(1,1) PRIMARY KEY,
    ScheduleRunId               INT           NOT NULL,
    DomainKey                   NVARCHAR(100) NOT NULL,
    PlanVersionId               INT           NULL,
    DataCutoffTime              DATETIME2     NOT NULL,
    OrderId                     BIGINT        NULL, -- 当前PlanVersion的Order.Id，逻辑关联
    OrderCanonicalId            BIGINT        NOT NULL,
    ProductionInstructionNo     NVARCHAR(100) NOT NULL,
    MaterialId                  INT           NOT NULL,
    FactoryId                   INT           NOT NULL,
    InstructionQty              DECIMAL(18,4) NOT NULL,
    ReceivedQty                 DECIMAL(18,4) NOT NULL DEFAULT 0,
    AvailableProductionQty AS (
        CASE
            WHEN InstructionQty - ReceivedQty < 0 THEN CONVERT(DECIMAL(18,4), 0)
            ELSE InstructionQty - ReceivedQty
        END
    ) PERSISTED,
    LocatedQty                  DECIMAL(18,4) NOT NULL DEFAULT 0,
    UnlocatedQty                DECIMAL(18,4) NOT NULL DEFAULT 0,
    SnapshotStatus              NVARCHAR(20)  NOT NULL DEFAULT 'READY',
    IssueCode                   NVARCHAR(100) NULL,
    SourceUpdatedAt             DATETIME2     NULL,
    CreatedAt                   DATETIME2     NOT NULL DEFAULT GETDATE(),
    CONSTRAINT FK_PISS_ScheduleRun FOREIGN KEY (ScheduleRunId) REFERENCES ScheduleRun(Id),
    CONSTRAINT FK_PISS_PlanVersion FOREIGN KEY (PlanVersionId) REFERENCES PlanVersion(Id),
    CONSTRAINT FK_PISS_OrderCanonical FOREIGN KEY (OrderCanonicalId) REFERENCES Order_Canonical(Id),
    CONSTRAINT FK_PISS_Material FOREIGN KEY (MaterialId) REFERENCES Material(Id),
    CONSTRAINT FK_PISS_Factory FOREIGN KEY (FactoryId) REFERENCES Factory(Id),
    CONSTRAINT UQ_PISS_RunDomainInstruction UNIQUE (ScheduleRunId, DomainKey, ProductionInstructionNo),
    CONSTRAINT UQ_PISS_RunDomainCanonical UNIQUE (ScheduleRunId, DomainKey, OrderCanonicalId),
    CONSTRAINT CK_PISS_Qty CHECK (
        InstructionQty >= 0 AND ReceivedQty >= 0 AND LocatedQty >= 0 AND UnlocatedQty >= 0
        AND LocatedQty + UnlocatedQty =
            CASE WHEN InstructionQty - ReceivedQty < 0 THEN CONVERT(DECIMAL(18,4), 0)
                 ELSE InstructionQty - ReceivedQty END
    ),
    CONSTRAINT CK_PISS_Status CHECK (SnapshotStatus IN ('READY','WITH_ISSUE'))
);
GO

CREATE INDEX IX_PISS_RunDomain
    ON ProductionInstructionSupplySnapshot(ScheduleRunId, DomainKey)
    INCLUDE (ProductionInstructionNo, MaterialId, FactoryId, AvailableProductionQty, SnapshotStatus);
CREATE INDEX IX_PISS_Instruction
    ON ProductionInstructionSupplySnapshot(ProductionInstructionNo, DataCutoffTime DESC);
GO

-- -----------------------------------------------
-- 4.6 ProductionInstructionPositionSlice（PI互斥位置切片）
-- -----------------------------------------------
CREATE TABLE ProductionInstructionPositionSlice (
    Id                              BIGINT IDENTITY(1,1) PRIMARY KEY,
    SupplySnapshotId                BIGINT        NOT NULL,
    PositionSliceKey                NVARCHAR(100) NOT NULL,
    PositionType                    NVARCHAR(30)  NOT NULL, -- STAGE / XC / PIPELINE / UNLOCATED
    Quantity                        DECIMAL(18,4) NOT NULL,
    ...
    AvailableTime                   DATETIME2     NULL,
    SupplyRepresentationType        NVARCHAR(50)  NOT NULL,
    SourceSystem                    NVARCHAR(50)  NULL,
    SourceRowKey                    NVARCHAR(500) NULL,
    SourceDocumentNo                NVARCHAR(100) NULL,
    InventoryAvailableSupplyDetailId BIGINT       NULL, -- 逻辑关联；来源表会TRUNCATE，不建FK
    PipelineSupplyId                BIGINT        NULL, -- 逻辑关联SupplyFact_Pipeline.Id
    IsEstimated                     BIT           NOT NULL DEFAULT 0,
    IssueCode                       NVARCHAR(100) NULL,
    CreatedAt                       DATETIME2     NOT NULL DEFAULT GETDATE(),
    CONSTRAINT FK_PIPS_Snapshot FOREIGN KEY (SupplySnapshotId) REFERENCES ProductionInstructionSupplySnapshot(Id),
    CONSTRAINT UQ_PIPS_SnapshotSlice UNIQUE (SupplySnapshotId, PositionSliceKey),
    CONSTRAINT CK_PIPS_Qty CHECK (Quantity > 0),
    CONSTRAINT CK_PIPS_PositionType CHECK (PositionType IN ('STAGE','XC','PIPELINE','UNLOCATED')),
    CONSTRAINT CK_PIPS_PathJson CHECK (RemainingStagePathJson IS NULL OR ISJSON(RemainingStagePathJson) = 1)
);
GO

CREATE INDEX IX_PIPS_SnapshotPosition
    ON ProductionInstructionPositionSlice(SupplySnapshotId, PositionType)
    INCLUDE (Quantity, CurrentStageCode, NextRequiredStageCode, AvailableTime);
CREATE INDEX IX_PIPS_SourceRow
    ON ProductionInstructionPositionSlice(SourceSystem, SourceRowKey)
    WHERE SourceRowKey IS NOT NULL;
GO

-- -----------------------------------------------
-- 4.7 PeggingAllocationLedger（需求—供给数量血缘权威账本）
-- -----------------------------------------------
-- 每笔成功分配必须同时完成需求/供给余额扣减并在内存追加Entry；AllocationSequence由每PlanVersion局部计数器在成功后生成。
-- 正式Ledger在1号位返回最终Task份额后，于单Domain事务中批量写入；数据库唯一约束仅作最终兜底。
-- SOFT/HARD都在本表表达；V1不另建SoftAllocation或TaskDemandAllocation物理表。
CREATE TABLE PeggingAllocationLedger (
    Id                          BIGINT IDENTITY(1,1) NOT NULL,
    PlanVersionId               INT            NOT NULL,
    ScheduleRunId               INT            NOT NULL,
    DomainKey                   NVARCHAR(100)  NOT NULL,
    AllocationSequence          BIGINT         NOT NULL,
    FlowNodeCode                NVARCHAR(100)  NULL,
    RootDemandOrderCanonicalId  BIGINT         NOT NULL,
    DemandOrderCanonicalId      BIGINT         NOT NULL,
    DemandOrderId               BIGINT         NULL, -- 当前PlanVersion Order.Id，逻辑关联
    DemandMaterialId            INT            NOT NULL,
    RequiredStageCode           NVARCHAR(50)   NULL,
    SupplyType                  NVARCHAR(50)   NOT NULL,
    SupplyBusinessKey           NVARCHAR(300)  NOT NULL,
    ParentProductionInstructionNo NVARCHAR(100) NULL,
    PositionSliceId             BIGINT         NULL,
    PositionSliceKey            NVARCHAR(100)  NULL,
    AllocatedQty                DECIMAL(18,4)  NOT NULL,
    UOM                         NVARCHAR(20)   NOT NULL,
    AvailableTime               DATETIME2      NULL,
    PriorityRank                INT            NULL,
    PriorityScore               DECIMAL(18,6) NULL,
    PriorityReason              NVARCHAR(500)  NULL,
    RuleSetVersionId            BIGINT         NULL,
    AllocationMode              NVARCHAR(10)   NOT NULL DEFAULT 'SOFT',
    HardLockId                  BIGINT         NULL,
    ExecutionLockId             BIGINT         NULL,
    LogicalBlockKey             NVARCHAR(100)  NULL,
    FinalTaskId                 BIGINT         NULL, -- 当前PlanVersion Task.Id；同一Domain事务先写Task并解析DraftKey→TaskId后写入
    TaskComponentQty            DECIMAL(18,4) NULL, -- 最终Task对该需求的组成份额；与AllocatedQty分离
    SourceRowKey                NVARCHAR(500)  NULL,
    SourceDocumentNo            NVARCHAR(100)  NULL,
    CreatedAt                   DATETIME2      NOT NULL DEFAULT GETDATE(),
    CONSTRAINT PK_PeggingAllocationLedger PRIMARY KEY (Id, PlanVersionId),
    CONSTRAINT FK_PAL_ScheduleRun FOREIGN KEY (ScheduleRunId) REFERENCES ScheduleRun(Id),
    CONSTRAINT FK_PAL_RootDemand FOREIGN KEY (RootDemandOrderCanonicalId) REFERENCES Order_Canonical(Id),
    CONSTRAINT FK_PAL_Demand FOREIGN KEY (DemandOrderCanonicalId) REFERENCES Order_Canonical(Id),
    CONSTRAINT FK_PAL_DemandMaterial FOREIGN KEY (DemandMaterialId) REFERENCES Material(Id),
    CONSTRAINT FK_PAL_PositionSlice FOREIGN KEY (PositionSliceId) REFERENCES ProductionInstructionPositionSlice(Id),
    CONSTRAINT FK_PAL_RuleSetVersion FOREIGN KEY (RuleSetVersionId) REFERENCES RuleSetVersion(Id),
    CONSTRAINT FK_PAL_HardLock FOREIGN KEY (HardLockId) REFERENCES DemandSupplyHardLock(Id),
    CONSTRAINT FK_PAL_ExecutionLock FOREIGN KEY (ExecutionLockId) REFERENCES ExecutionLock(Id),
    CONSTRAINT FK_PAL_FinalTask FOREIGN KEY (FinalTaskId, PlanVersionId) REFERENCES Task(Id, PlanVersionId),
    CONSTRAINT UQ_PAL_PlanAllocationSequence UNIQUE (PlanVersionId, AllocationSequence),
    CONSTRAINT CK_PAL_Qty CHECK (AllocatedQty > 0),
    CONSTRAINT CK_PAL_AllocationMode CHECK (AllocationMode IN ('SOFT','HARD')),
    CONSTRAINT CK_PAL_HardMode CHECK (
        (AllocationMode = 'SOFT' AND HardLockId IS NULL)
        OR (AllocationMode = 'HARD' AND HardLockId IS NOT NULL)
    ),
    CONSTRAINT CK_PAL_TaskComponent CHECK (
        (FinalTaskId IS NULL AND TaskComponentQty IS NULL)
        OR (FinalTaskId IS NOT NULL AND TaskComponentQty > 0)
    )
) ON PS_PlanVersion(PlanVersionId);
GO

CREATE INDEX IX_PAL_Demand
    ON PeggingAllocationLedger(PlanVersionId, DemandOrderCanonicalId, FlowNodeCode)
    INCLUDE (SupplyType, SupplyBusinessKey, AllocatedQty, AllocationMode, FinalTaskId)
    ON PS_PlanVersion(PlanVersionId);
CREATE INDEX IX_PAL_Supply
    ON PeggingAllocationLedger(PlanVersionId, SupplyType, SupplyBusinessKey)
    INCLUDE (DemandOrderCanonicalId, AllocatedQty, AllocationMode, AvailableTime)
    ON PS_PlanVersion(PlanVersionId);
CREATE INDEX IX_PAL_FinalTask
    ON PeggingAllocationLedger(PlanVersionId, FinalTaskId)
    INCLUDE (DemandOrderCanonicalId, RootDemandOrderCanonicalId, TaskComponentQty, AllocationMode, ExecutionLockId)
    WHERE FinalTaskId IS NOT NULL
    ON PS_PlanVersion(PlanVersionId);
CREATE INDEX IX_PAL_ExecutionLock
    ON PeggingAllocationLedger(ExecutionLockId, PlanVersionId)
    WHERE ExecutionLockId IS NOT NULL
    ON PS_PlanVersion(PlanVersionId);
GO

-- -----------------------------------------------
-- 4.7 PeggingSupplyAllocation（非Task现实供给分配结果）
-- -----------------------------------------------
-- 只记录库存/XC/在途/Received等已确认分配；候选但未使用供给不得写入。
-- 物理Pegging表继续只承接Task-to-Task血缘。
CREATE TABLE PeggingSupplyAllocation (
    ...
    BatchNo                         NVARCHAR(50)   NULL,
    RootOrderCanonicalId            BIGINT         NOT NULL,
    DemandOrderCanonicalId          BIGINT         NOT NULL,
    ...
    DemandQty                       DECIMAL(18,4)  NOT NULL,
    AllocatedQty                    DECIMAL(18,4)  NOT NULL,
    AllocationMode                  NVARCHAR(10)   NOT NULL DEFAULT 'SOFT',
    HardLockId                      BIGINT         NULL,
    ExecutionLockId                 BIGINT         NULL,
    ...
    PositionSliceId                 BIGINT         NULL,
    InventoryAvailableSupplyDetailId BIGINT        NULL,
    PipelineSupplyId                BIGINT         NULL,
    ...
    CONSTRAINT FK_PSA_ScheduleRun FOREIGN KEY (ScheduleRunId) REFERENCES ScheduleRun(Id),
    CONSTRAINT FK_PSA_Ledger FOREIGN KEY (LedgerId, PlanVersionId) REFERENCES PeggingAllocationLedger(Id, PlanVersionId),
    CONSTRAINT FK_PSA_RootDemand FOREIGN KEY (RootOrderCanonicalId) REFERENCES Order_Canonical(Id),
    CONSTRAINT FK_PSA_Demand FOREIGN KEY (DemandOrderCanonicalId) REFERENCES Order_Canonical(Id),
    CONSTRAINT FK_PSA_Material FOREIGN KEY (MaterialId) REFERENCES Material(Id),
    CONSTRAINT FK_PSA_HardLock FOREIGN KEY (HardLockId) REFERENCES DemandSupplyHardLock(Id),
    CONSTRAINT FK_PSA_ExecutionLock FOREIGN KEY (ExecutionLockId) REFERENCES ExecutionLock(Id),
    CONSTRAINT FK_PSA_PositionSlice FOREIGN KEY (PositionSliceId) REFERENCES ProductionInstructionPositionSlice(Id),
    CONSTRAINT CK_PSA_Qty CHECK (DemandQty > 0 AND AllocatedQty > 0 AND AllocatedQty <= DemandQty),
    CONSTRAINT CK_PSA_AllocationMode CHECK (AllocationMode IN ('SOFT','HARD')),
    CONSTRAINT CK_PSA_PathJson CHECK (RemainingStagePathJson IS NULL OR ISJSON(RemainingStagePathJson) = 1),
    CONSTRAINT CK_PSA_HardMode CHECK (
        (AllocationMode = 'SOFT' AND HardLockId IS NULL)
        OR (AllocationMode = 'HARD' AND HardLockId IS NOT NULL)
    )
) ON PS_PlanVersion(PlanVersionId);
...
CREATE INDEX IX_PSA_Demand
    ON PeggingSupplyAllocation(PlanVersionId, DemandOrderCanonicalId, MaterialId)
    INCLUDE (AllocatedQty, SupplyType, SupplyBusinessKey, AllocationMode, KnownAvailableTime)
    ON PS_PlanVersion(PlanVersionId);
CREATE INDEX IX_PSA_Supply
    ON PeggingSupplyAllocation(PlanVersionId, SupplyType, SupplyBusinessKey)
    INCLUDE (DemandOrderCanonicalId, AllocatedQty, SupplyDocumentNo, SourceRowKey)
    ON PS_PlanVersion(PlanVersionId);
CREATE UNIQUE INDEX UX_PSA_Ledger
    ON PeggingSupplyAllocation(PlanVersionId, LedgerId)
    ON PS_PlanVersion(PlanVersionId);
GO

-- -----------------------------------------------
-- 4.8 vw_TaskDemandAllocation（Task跨需求数量份额权威查询视图）
-- -----------------------------------------------
-- V1不物化TaskDemandAllocation；性能不足时再评估物化，不能形成第二套数量真相。
CREATE OR ALTER VIEW dbo.vw_TaskDemandAllocation
AS
SELECT
    l.PlanVersionId,
    l.FinalTaskId AS TaskId,
    l.RootDemandOrderCanonicalId,
    l.DemandOrderCanonicalId,
    l.DemandOrderId,
    l.ParentProductionInstructionNo,
    l.AllocationMode,
    l.HardLockId,
    l.ExecutionLockId,
    l.UOM,
    SUM(l.TaskComponentQty) AS TaskComponentQty,
    MIN(l.AvailableTime) AS EarliestAvailableTime,
    MAX(l.AvailableTime) AS LatestAvailableTime
FROM dbo.PeggingAllocationLedger l
WHERE l.FinalTaskId IS NOT NULL
  AND l.TaskComponentQty IS NOT NULL
GROUP BY
    l.PlanVersionId,
    l.FinalTaskId,
    l.RootDemandOrderCanonicalId,
...
-- v5.2.1 设计边界（不建物理对象）
-- =============================================
-- 1) SoftAllocation：不建；由 PeggingAllocationLedger.AllocationMode='SOFT' 表达。
-- 2) DemandPromiseConstraint：不建；复用Order交期/客户等级、Firm/Frozen、规则参数和审批。
-- 3) ExecutionLockTaskLink：不建；不同PlanVersion的Task通过Task.ExecutionLockId关联同一现实执行锁。
-- 4) TaskDemandAllocation：不建物理表；使用vw_TaskDemandAllocation。
-- 5) 动态插件目录/运行中程序集装卸：V1不建；只采用.NET接口+依赖注入的少数策略实现。
-- 6) 完整ExecutionLock事件溯源：V1不建；当前状态在ExecutionLock，关键人工变更进入审计日志。


-- =============================================
-- Batch 5：统一管道供给按来源启用（v5.2.1，覆盖旧“V1固定空跑”口径）
-- =============================================
-- 说明：尚未接入的来源保留同构0行视图；任一来源一旦实现真实SELECT，即自动进入统一主链。
-- sp_SyncPipelineSupply只按本次BatchNo原子替换，不再TRUNCATE全表或固定返回空集合。

USE [MES_Integration];
GO

-- 采购在途 ODS 契约占位（14字段；5号位启用时只替换视图内部实现）
CREATE OR ALTER VIEW dbo.ERP_PurchaseInTransit_View
AS
SELECT
    CAST(NULL AS INT)            AS MasterID,
    CAST(NULL AS NVARCHAR(100))  AS MaterialCode,
...
-- VMI ODS 契约占位
CREATE OR ALTER VIEW dbo.ERP_VMI_View
AS
SELECT
    CAST(NULL AS INT)            AS MasterID,
    CAST(NULL AS NVARCHAR(100))  AS MaterialCode,
...
-- 已到厂未入库 ODS 契约占位
CREATE OR ALTER VIEW dbo.ERP_ArrivedNotReceived_View
AS
SELECT
    CAST(NULL AS INT)            AS MasterID,
    CAST(NULL AS NVARCHAR(100))  AS MaterialCode,
    ...
    CAST('OWNED' AS NVARCHAR(20)) AS OwnershipType,
    CAST('PENDING_INSPECTION' AS NVARCHAR(20)) AS QualityStatus,
    CAST(NULL AS DECIMAL(18,4))  AS Quantity,
    CAST(NULL AS DATETIME2)      AS ETA,
    CAST(NULL AS NVARCHAR(50))   AS StorageCode,
    CAST(NULL AS NVARCHAR(50))   AS SupplierCode,
...
CREATE OR ALTER VIEW dbo.ext_ERP_PurchaseInTransit_View AS
SELECT MasterID, MaterialCode, SourceFactoryCode, FactoryCode,
       SupplyType, OwnershipType, QualityStatus, Quantity, ETA,
       StorageCode, SupplierCode, SourceDocumentNo, SourceDocumentLineNo, SourceUpdatedAt
FROM [MES_Integration].[dbo].[ERP_PurchaseInTransit_View];
GO

CREATE OR ALTER VIEW dbo.ext_ERP_VMI_View AS
SELECT MasterID, MaterialCode, SourceFactoryCode, FactoryCode,
       SupplyType, OwnershipType, QualityStatus, Quantity, ETA,
       StorageCode, SupplierCode, SourceDocumentNo, SourceDocumentLineNo, SourceUpdatedAt
FROM [MES_Integration].[dbo].[ERP_VMI_View];
GO

CREATE OR ALTER VIEW dbo.ext_ERP_ArrivedNotReceived_View AS
SELECT MasterID, MaterialCode, SourceFactoryCode, FactoryCode,
       SupplyType, OwnershipType, QualityStatus, Quantity, ETA,
       StorageCode, SupplierCode, SourceDocumentNo, SourceDocumentLineNo, SourceUpdatedAt
FROM [MES_Integration].[dbo].[ERP_ArrivedNotReceived_View];
GO

-- 统一输入视图：14字段来源契约 + 1列APS派生SourceSystem
CREATE OR ALTER VIEW dbo.ext_PipelineSupply_Source_View
AS
SELECT MasterID, MaterialCode, SourceFactoryCode, FactoryCode,
       SupplyType, OwnershipType, QualityStatus, Quantity, ETA,
       StorageCode, SupplierCode, SourceDocumentNo, SourceDocumentLineNo, SourceUpdatedAt,
       CAST('ERP' AS NVARCHAR(50)) AS SourceSystem
FROM dbo.ext_ERP_InterplantInTransit_View
UNION ALL
SELECT MasterID, MaterialCode, SourceFactoryCode, FactoryCode,
       SupplyType, OwnershipType, QualityStatus, Quantity, ETA,
       StorageCode, SupplierCode, SourceDocumentNo, SourceDocumentLineNo, SourceUpdatedAt,
       CAST('PROCUREMENT' AS NVARCHAR(50)) AS SourceSystem
FROM dbo.ext_ERP_PurchaseInTransit_View
UNION ALL
SELECT MasterID, MaterialCode, SourceFactoryCode, FactoryCode,
       SupplyType, OwnershipType, QualityStatus, Quantity, ETA,
       StorageCode, SupplierCode, SourceDocumentNo, SourceDocumentLineNo, SourceUpdatedAt,
       CAST('VMI' AS NVARCHAR(50)) AS SourceSystem
...
-- 统一管道供给真实装载：按已启用来源读取；单来源0行合法，但技术失败必须抛出。
...
BEGIN
    ...
    IF NULLIF(LTRIM(RTRIM(@BatchNo)), N'') IS NULL
        ;THROW 51001, '@BatchNo must not be NULL or empty', 1;
    IF @DataCutoffTime IS NULL
        ;THROW 51002, '@DataCutoffTime must not be NULL', 1;
    IF OBJECT_ID(N'dbo.ext_PipelineSupply_Source_View', N'V') IS NULL
        ;THROW 51003, 'ext_PipelineSupply_Source_View not found', 1;
    ...
    BEGIN TRY
        ...
        WHERE Quantity > 0
          ...
          AND (SourceUpdatedAt IS NULL OR SourceUpdatedAt <= @DataCutoffTime)
          AND (
                SupplyType <> 'INTERPLANT_IN_TRANSIT'
                OR (SourceDocumentNo IS NOT NULL
                    AND SourceDocumentLineNo IS NOT NULL
                    AND StorageCode IS NOT NULL)
              );

        DECLARE @RawCount INT = @@ROWCOUNT;
        DECLARE @MissingTimeCount INT;
        SELECT @MissingTimeCount = COUNT(*)
        FROM #RawPipelineSupply
        WHERE SourceUpdatedAt IS NULL;

        CREATE TABLE #MappedPipelineSupply (
            ...
            SourceUpdatedAt       DATETIME2      NULL,
            SourceSystem          NVARCHAR(50)   NOT NULL,
            SourceMasterID        INT            NULL
        );

        INSERT INTO #MappedPipelineSupply (
            RawId, MaterialCode, MaterialId, ProductFamilyId,
            FactoryCode, FactoryId, SourceFactoryCode,
            SupplyType, OwnershipType, QualityStatus, Quantity, ETA,
            StorageCode, SupplierCode, SourceDocumentNo, SourceDocumentLineNo,
            SourceUpdatedAt, SourceSystem, SourceMasterID
        )
        SELECT
            r.RawId,
            mat.MaterialCode,
        ...
        OUTER APPLY (
            SELECT COUNT_BIG(*) AS MappingCount,
                   MIN(mm.MaterialCode) AS MappedMaterialCode
            FROM dbo.MaterialMapping mm
            WHERE mm.SourceID = r.MasterID
              AND mm.Source = 'ERP'
              AND mm.IsCurrent = 1
              AND mm.Warehouse_Norm = ISNULL(r.StorageCode, 'N/A')
        ) mapx
        INNER JOIN dbo.Material mat
            ON mapx.MappingCount = 1
           AND mat.MaterialCode = mapx.MappedMaterialCode
           AND mat.IsActive = 1
        INNER JOIN dbo.Factory f
            ON f.Code = r.FactoryCode
           AND f.IsActive = 1;

        DECLARE @MappedCount INT = @@ROWCOUNT;
        DECLARE @SkippedMappingCount INT = @RawCount - @MappedCount;

        BEGIN TRANSACTION;

        DELETE FROM dbo.SupplyFact_Pipeline
        WHERE BatchNo = @BatchNo;
        ...
        ;WITH Evaluated AS (
            SELECT
                ...
                ROW_NUMBER() OVER (
                    PARTITION BY
                        m.SourceSystem, m.SupplyType,
                        ISNULL(m.SourceDocumentNo, N''),
                        ISNULL(m.SourceDocumentLineNo, N''),
                        ISNULL(m.SourceMasterID, -1),
                        ISNULL(m.StorageCode, N''),
                        m.FactoryCode
                    ORDER BY m.RawId
                ) AS DedupRank
            FROM #MappedPipelineSupply m
            OUTER APPLY (
                SELECT TOP (1)
                    r.Id AS RuleId,
                    r.IncludeFlag,
                    r.LeadTimeOffset,
                    r.Priority AS RulePriority
                FROM dbo.SupplyAvailabilityRule r
                WHERE r.IsActive = 1
                  AND (r.EffectiveFrom IS NULL OR r.EffectiveFrom <= @DataCutoffTime)
                  AND (r.EffectiveTo IS NULL OR r.EffectiveTo > @DataCutoffTime)
                  AND (r.ProductFamilyId IS NULL OR r.ProductFamilyId = m.ProductFamilyId)
                  AND (r.FactoryId IS NULL OR r.FactoryId = m.FactoryId)
                  AND (r.SupplyType IS NULL OR r.SupplyType = m.SupplyType)
                  AND (r.OwnershipType IS NULL OR r.OwnershipType = m.OwnershipType)
                  AND (r.QualityStatus IS NULL OR r.QualityStatus = m.QualityStatus)
                ORDER BY
                    r.Priority ASC,
                    (CASE WHEN r.ProductFamilyId IS NOT NULL THEN 1 ELSE 0 END
                    +CASE WHEN r.FactoryId IS NOT NULL THEN 1 ELSE 0 END
        ...
        INSERT INTO dbo.SupplyFact_Pipeline (
            MaterialCode, MaterialId, FactoryCode, FactoryId, ProductFamilyId,
            SupplyType, OwnershipType, QualityStatus, Quantity, ETA, AvailableTime,
            SourceMasterID, SourceFactoryCode, SourceDocumentNo, SourceDocumentLineNo,
            SourceUpdatedAt, StorageCode, SupplierCode, SourceSystem,
            SupplyAvailabilityRuleId, AppliedLeadTimeOffset, RulePriority, RuleEvaluatedAt,
            BatchNo, IsActive, SyncedAt
        )
        SELECT
            e.MaterialCode, e.MaterialId, e.FactoryCode, e.FactoryId, e.ProductFamilyId,
            e.SupplyType, e.OwnershipType, e.QualityStatus, e.Quantity, e.ETA,
            CASE WHEN e.ETA IS NULL THEN NULL
                 ELSE DATEADD(HOUR, ISNULL(e.LeadTimeOffset, 0), e.ETA) END,
            e.SourceMasterID, e.SourceFactoryCode, e.SourceDocumentNo, e.SourceDocumentLineNo,
            e.SourceUpdatedAt, e.StorageCode, e.SupplierCode, e.SourceSystem,
            e.RuleId, ISNULL(e.LeadTimeOffset, 0), e.RulePriority, @DataCutoffTime,
            @BatchNo, 1, GETDATE()
        FROM Evaluated e
        WHERE e.DedupRank = 1
          AND ISNULL(e.IncludeFlag, 1) = 1; -- 无主题规则时采用全局基础口径；明确排除规则优先
        ...
        VALUES (
            ...
            CONCAT('统一管道供给同步完成: raw=', @RawCount,
                   ', mapped=', @MappedCount,
                   ', skippedMapping=', @SkippedMappingCount,
                   ', sourceTimeMissing=', @MissingTimeCount,
                   ', written=', @RowsAffected),
            'SUCCESS', GETDATE()
        );
    ...
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
        SET @ErrorMessage = CONCAT('sp_SyncPipelineSupply failed: ', ERROR_MESSAGE());
        INSERT INTO dbo.APS_ETL_Log (BatchNo, Step, Message, Status, CreatedAt)
        VALUES (@BatchNo, 'sp_SyncPipelineSupply', @ErrorMessage, 'FAILED', GETDATE());
        THROW;
    END CATCH
...
-- =============================================
-- v5.2.2 部署与数据保留运维契约（不在主DDL内自动启用删除作业）
-- =============================================
-- 1) APS_Auth：执行独立脚本《APS_Auth数据库DDL_v1.0.sql》，不属于本APS_Production/MES_Integration主脚本。
-- 2) 建议保留期：ARCHIVED版本明细90天；FAILED/重试半成品14天；Summary/KPI至少2年；终态ExecutionLock/HardLock 1年。
-- 3) 清理作业必须按PlanVersion及依赖顺序执行，并在测试库完成回归后由DBA/Hangfire启用；本DDL不自动删除生产数据。
-- 4) PF_PlanVersion接近100000前必须扩展边界或迁移分区方案，禁止让全部新版本长期落入尾部分区。
GO
