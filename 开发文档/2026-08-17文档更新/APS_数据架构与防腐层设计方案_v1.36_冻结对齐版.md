...
## v1.36 冻结对齐说明（2026-08-12）

> **冻结上位声明**：本文件是第三层权威技术文档，必须服从以下业务冻结层：
> 1. 《APS V1最终全部流程与业务基线_v1.0_20260812》；
> 2. 《APS Pegging供需承接与分层计算业务说明_v1.1_冻结对齐版》；
> 3. 《APS有限产能排产与滚动90天计划业务说明_v1.1_冻结对齐版》。
> 若本文历史章节、旧代码示例或旧版本说明与上述冻结业务冲突，以冻结业务为准；不得因为“更标准、更完整、未来V2需要、旧DDL/旧代码已经如此”而反向修改业务。只有0号位明确重新打开某项冻结决策后，业务口径才可改变。

**v1.36只做冻结符合性技术对齐，不重新设计业务。主要收敛如下：**

- `PriorityScore`退出V1排序权威；需求排序由3号位治理“计算层→有序规则段→第一命中→段内排序”，2号位在内存执行。
- 2号位不再预生成最终Operation Task/ShippingTask；2号位形成逻辑生产需求与完整ScheduleContext，1号位生成FinalTask、Resource、Start/End、最终拆合批与真实Task依赖。
- 新增`NetRequiredQty/NetOutputQty`与`PlannedProcessQty`双数量语义，V1正式纳入计划良率；计划良率不等于BOM材料损耗，不能假设BOM存在ScrapRate。
- 90天采用一套正式有限产能Solver；近期/远期只允许优化深度不同，不再按7天/8-90天切换为“详细排程/LeadTime粗排”两套资源语义。
- 采购、VMI、已到厂未入库、厂间在途正式进入V1供给链；不再把`PipelineSupplies`定义为空集合。无正式承诺时可生成仅内存的`Planning-only Purchase Placeholder`，必须标`ESTIMATED/NOT_COMMITTED`。
- PI总量以ERP `Quantity-ReceivedQty`为边界；MES Stage/XC/PI级在途只用于5号位推导PI Position，不再由1号位直接按StageProgress扣Task。PI Position采用一张最小快照表、多行互斥闭合，支持UNLOCATED。
- Demand Protection、Strict Binding、Execution Constraint三类分离；Protection按数量份额锁定，不锁整个PI/PO。Firm/Frozen是排程不可移动约束，不等于Pegging Lock。
- 两类跨厂重新冻结：STAGE_HANDOFF=`目标M→PI→PI Position→新增生产`；INTER_FACTORY_ORDER=`目标BS/KS→SH→SH内在途→同SH Received→未生产`。V1不建设有限物流Task/ShippingTask体系，运输只形成LeadTime→AvailableTime。
- 跨Domain供给按多段`Quantity + AvailableTime`传递，不持久化`VirtualInventoryBalance`；跨Domain转运LT使用真实工厂/Stage口径，不以统一2天为业务权威。
- 夜间FULL保持Domain独立PlanVersion，但同一物理共享资源必须真实互斥；上游Domain失败时依赖下游本次不得发布新ACTIVE，无关Domain继续发布。失败人工恢复必须创建新ScheduleRun，历史FAILED不可改写。
- 白天Candidate严格单Domain；其它Domain ACTIVE共享设备占用作为不可移动阻挡块。普通未锁Allocation可释放重新竞争；`ScopeJson`只表达发起范围，`MaxImpactedOrders`/PlanHorizon不得截断真实影响；影响过大时同一Solver升级为单Domain完整可移动重排。
- 跨Domain CTP按依赖拓扑串行多个单Domain WHATIF（如C→B→A），逐层传Quantity-Time，前端合并一个CTP+影响答案；不建设MultiDomain Candidate/Group。
- Scenario降为可选业务容器；Candidate正式采用只需最小人工授权确认并记录谁/何时/采用哪个版本，不把完整OA审批作为V1主链前置。
- `FrozenZoneSnapshot`、`VirtualInventoryBalance`、有限物流Task、第二套远期Solver、通用插件/DSL/脚本平台、跨Domain资源配额/借用平台均不得进入V1。
- 保留并保护已稳定对象：ScheduleRun/PlanVersion/ExpectedDomainKeysJson/PARTIAL_SUCCESS、Realtime BOM链、OrderBomRequestLink、Routing三件套、MaterialStageDeptContext、MES五态、规则治理6表和2号位现有Orchestrator/Context外壳。

> ⚠️ **历史说明规则**：v1.35及更早“变更说明”继续保留用于追溯，其中出现的`PipelineSupplies为空`、`Task/ShippingTask实例化`、固定2天跨域LT、重审批、粗排等旧词，只代表当时版本，不再具有当前V1权威性。
...
**上位冻结业务文档（最高业务依据）**：
- **《APS V1最终全部流程与业务基线_v1.0_20260812》**
- **《APS Pegging供需承接与分层计算业务说明_v1.1_冻结对齐版》**
- **《APS有限产能排产与滚动90天计划业务说明_v1.1_冻结对齐版》**

**同层/下游技术文档**：
- **《APS 核心排产全流程走查》V3.17 冻结对齐版**：端到端主流程与0～5号位接力
- **《APS各类基础数据分层承接与演变总表》v3.32 冻结对齐版**：数据演进全景与横向承接
- **《职责分工变更说明_v3.0_Socket-Plug模式》**：历史职责参考，冲突时以当前冻结职责为准
- **《APS_集成接口设计_v1.12》**：下一步按本轮冻结业务继续修订
- **《APS_数据库字段说明文档_v5.0》**：后续按本轮冻结业务继续修订
- **《APS_数据库表结构设计_v5.0.sql》**：后续按本轮冻结业务继续修订
...
6. ✅ **数量真相与时间真相分离**：2号位负责Demand/Supply Allocation数量闭合；1号位负责FinalTask/Resource/Start-End时间与产能真相
7. ✅ **防漂移**：防腐层吸收源系统变化，但不得反向创造新的V1业务规则或平台
...
- **内存**：96GB
- **硬盘**：200GB（操作系统 + 应用程序）
- **用途**：1号位有限产能排程引擎、3号位规则参数治理/运行编排服务、5号位BOM/PI位置/跨厂等复杂事实服务、Web API 及 Hangfire 调度
...
- **夜间批次**（00:00-02:00）：
  ...
  - 00:05 与Canonical装载到Order分区表（补齐MaterialId/ProductFamilyId/FactoryId/DomainKey + 透传TransportMode/CustomerName/CustomerSegment/SalesOrderCategory/DemandMaturityStatus/CustomerTier/DelayStatus/MTS_InstructionNo/IssueDate/OriginalDueDate/ReceivedQty/SourceModel/NonStockShipmentType/OriginalOrderSource）（v5.0.27补后三字段）
  - 02:00 排程引擎消费Order快照
...
#### **2.2.1b 人工发起的白天实时评估与局部重排入口**（v1.36 冻结对齐）

**核心口径**：白天实时评估与局部重排由 PMC/销售/计划员通过4号位发起。每小时订单同步只更新 `Order_Canonical`，不自动创建 Candidate。

**运行边界**：
- 每个白天 Candidate 严格单 Domain；`ExpectedDomainKeysJson` 恰好一个DomainKey。
- `Scenario` **可选**，普通CTP/插单WHATIF不以Scenario为前置依赖。
- `ScopeJson`保留现有11字段契约，但只表达**发起请求与初始业务范围**；它不是物理影响传播的硬边界。
- `MaxImpactedOrders`只能作为告警/人工确认阈值，不能达到阈值就停止传播；`PlanHorizonStart/End`同样不能截断真实工艺、物料、资源影响。

**常用入口**：

| 业务场景 | RunType | Purpose | 结果/采用边界 |
|---|---|---|---|
| CTP/插单WHATIF | `INSERT_ORDER_WHATIF` | 可保留`CTP`或`INSERT_IMPACT_ANALYSIS`入口语义 | 同一次WHATIF Solver应同时产出交期+影响；WHATIF本身永不直接ACTIVE |
| 插单正式采用前重算 | `LOCAL_RESCHEDULE` | `INSERT_RESCHEDULE` | 以当前事实重新计算后，经最小人工授权确认可激活 |
| 人工局部调整 | `LOCAL_RESCHEDULE` | `MANUAL_ADJUSTMENT` | 经最小人工授权确认可激活 |
| 单Domain较大范围重排 | `MANUAL_RESCHEDULE` | `MANUAL_ADJUSTMENT` | 经最小人工授权确认可激活 |

**主链**：
1. 4号位展示新增/变化订单并接收发起请求。
2. 3号位冻结本次规则/参数版本，归一化Scope，创建`ScheduleRun`；Scenario仅在确有业务场景管理需要时创建。
3. 创建单Domain Candidate PlanVersion壳。
4. 2号位准备Candidate订单快照、必要Realtime BOM、当前物理Supply Context、Lock/Firm/Frozen/外域共享资源阻挡等。
5. 2号位重新Pegging：已消耗、Strict Binding、Demand Protection、不可逆执行份额固定；**普通未锁Base Allocation释放重新竞争**。
6. 2号位只形成逻辑生产需求和变化种子，不预生成最终Operation Task。
7. 1号位在Base ACTIVE上做最小扰动局部有限产能重排，影响沿工艺/物料/共享资源时间轴动态传播；传播过大或不稳定时，同一Solver自动升级为当前单Domain完整可移动范围重排。
8. 1号位同时返回FinalTask、交期、影响订单、KPI和Explanation Facts；2号位统一事务持久化。
9. WHATIF只返回评估结果；正式采用时按当前事实重新计算并进行最小人工授权确认，记录操作者、时间、采用的Candidate/运行。

**跨Domain插单/CTP**：如果最终产品Domain A依赖B/C，后台按`Domain_Dependency`拓扑串行多个**单Domain WHATIF**（如C→B→A），每一步只向下游传多段`Quantity + AvailableTime`，前端合并成一个CTP/影响答案。V1不建设MultiDomain Candidate、CandidateGroup或原子多域仿真平台。

**共享资源红线**：白天Candidate如与其它Domain共享设备，必须把其它Domain当前ACTIVE占用作为不可移动阻挡块；不得挤动其它Domain，也不建设跨Domain资源配额、借用或影响传播平台。
...
MES生产库.工艺阶段源数据
  ↓ (5号位/数据Owner负责维护ODS契约视图；3号位只治理排程规则与参数)
...
MES_APS_BOM_Workset (主层：3辅助字段 + ChildRequiredStageCode，不承载根产品自身路径)
  ...
  ↓ 消费职责：**必须按StageScopeType区分查询**；**2号位根据EDGE/ROOT路径形成逻辑生产需求、起始Stage和完整Routing候选上下文，不预生成最终Operation Task**；**1号位根据Routing/Eligibility/资源日历/物料时间约束生成FinalTask并决定最终Resource与Start/End**
```
...
**V1未来供给链说明（v1.36冻结修正）**：
- `InventoryBalance`仍只承接现货库存；采购/VMI/已到厂未入库/厂间在途作为并行未来Supply事实进入V1；
- `sp_SyncPipelineSupply`及现有包装视图骨架优先复用，但不得再固定返回0行；
- 真实未来Supply通过`Quantity + AvailableTime`进入2号位Supply Context，详见后文“2.5 管道/未来供给链”；
- 无正式PO/VMI承诺时仅生成内存`Planning-only Purchase Placeholder`，不污染`InventoryBalance`。

---

### 2.5 环节四：02:00巅峰对决与多源数据缝合

#### **2.5.1 排程发令枪响：先闭合数量，再求真实时间**

**时间**：每天02:00。

**动作与职责**：
1. **2号位**在内存建立DemandBalance/SupplyBalance，执行分层Pegging、Lock、PI选择、采购/VMI/库存/跨厂等Supply选择，并完成双边原子Allocation。
2. 2号位执行计划良率反算，形成`NetRequiredQty/NetOutputQty`与`PlannedProcessQty`，并把所有已选Supply转换为数量—时间约束。
3. 2号位形成**逻辑生产需求（Logical Production Demand）**和完整`ScheduleContext`，不预生成最终Operation Task。
4. **1号位**消费完整Routing、资源资格、日历、物料Quantity-Time、Firm/Frozen/Execution约束和冻结策略，在90天统一有限产能Solver中生成FinalTask、Resource、Start/End、最终拆合批、真实Task依赖和Explanation Facts。
5. **2号位**接收结果，在单Domain事务内一次性持久化Allocation/PSA、FinalTask、AllocationTaskShare、Task依赖、Explanation及摘要。

**硬红线**：
- 2号位不得为1号位提前锁死最终Task结构、最终设备或最终时间槽；
- 1号位不得重新选择2号位已经Pegging确认的业务Supply或改变Allocation数量；
- `InventoryBalance.AllocatedQty`不得作为跨PlanVersion运行期永久扣减真相；运行余额以源Supply事实 + 当前PlanVersion内存Allocation为准；
- 数量闭合、Lock、非法资源、跨版本引用等硬错误必须使该Domain失败；“无法按客户交期完成”是正常业务结果，不是Solver失败。
...
#### **2.5.3 Routing缺失与90天统一有限产能降级规则**

APS V1的90天窗口只有**一套正式有限产能资源语义**。不得再使用“近7天详细排程、8～90天按默认LeadTime粗排”的双轨方案，也不得以DueDate自动把远期需求切成`ROUGH_CUT`或占位工序。

**统一规则**：
- 正常Routing存在：全部进入同一1号位Solver，远期同样真实占用设备未来时间；近期和远期最多只允许搜索深度、候选数量、局部优化轮次不同。
- 业务已明确使用StageLeadTime简化的Stage（例如V1明确简化的表面处理/外协阶段）：按该Stage的确定性LeadTime形成时间约束，但它仍属于同一90天计划，不是第二套远期Solver。
- Routing缺失/资格缺失：按数据质量规则降级或阻断，**不能因为需求在8～90天就自动伪造一条默认Routing**。如果缺失导致无法形成可信物理路径，应登记Issue/Explanation并阻止受影响结果发布；只有业务已经明确允许的StageLeadTime简化才可继续。
- `DefaultLeadTimeDays`可以继续用于采购ETA回退、明确的Stage LeadTime等业务语义，但不得再被滥用为制造工艺路线替代物。

**实现边界**：不建设第二套`LocalSolver/RoughCutSolver`，也不建设可任意编排的Solver Command链。1号位保持“硬约束建模→初排→可行性/延期诊断→有界局部修复→最终压缩与评价”五阶段结构。
...
**核心口径**：每小时增量同步只更新 `Order_Canonical`；白天实时评估与局部重排一律由 PMC/销售/计划员**人工发起**，通过 3号位创建 `Scenario`（适用时）和 `ScheduleRun`。ScopeJson 完整 Schema 引用《集成接口设计 v1.25》唯一权威契约，本节不复制。
...
2. 人工选择业务场景并确认发起
3. **3号位** 归一化 `ScopeJson`（唯一权威见《集成接口 v1.25》），创建 `Scenario`（适用时）
...
#### **2.7.6 Candidate剩余供给、Lock与变化种子**

**动作**：2号位调用`BuildRemainingSupplyContextAsync`时，不再使用旧公式“Base ACTIVE原始供给 - Base ACTIVE全部Allocation”。

Candidate可用供给应从**当前物理Supply事实**出发，并扣除/固定：
- 已真实消耗数量；
- Strict Binding份额；
- Demand Protection份额；
- 已形成不可逆执行归属的份额；
...
Base ACTIVE中的**普通、未锁Allocation应释放回竞争池**，与新订单按本次最新规则重新Pegging。Candidate不修改Base版本，也不把运行时Allocation写回共享`InventoryBalance.AllocatedQty`。

2号位输出给1号位的是：
- Candidate完整逻辑生产需求；
- Allocation份额；
- Pegging前后变化种子；
- 采购/VMI/库存/跨厂/跨Domain的多段`Quantity + AvailableTime`；
- Firm/Frozen/Execution锚点；
- 其它Domain ACTIVE共享设备不可移动占用。

如果采购件在库存、已到厂未入库、PO、VMI均无承诺供给，2号位可以生成**Planning-only Purchase Placeholder**：数量=缺口，AvailableTime按冻结DefaultLT估算，标记`ESTIMATED/NOT_COMMITTED`。该占位只在内存中存在，不生成采购单、不生成Task、不落成已承诺Supply、不下发ERP；依赖它的CTP只能返回估算日期。

#### **2.7.7 Candidate输出、局部传播与采用边界**

1. **1号位**从变化种子开始，在Base ACTIVE上执行最小扰动局部重排。影响沿三类真实关系传播：工艺前后关系、物料供需关系、资源时间轴邻接/Setup关系。
2. `ScopeJson`、`MaxImpactedOrders`和PlanHorizon只能作为发起范围/告警阈值，不能截断已经发生的真实影响。
3. 局部传播过大、修复反复或时间预算逼近上限时，**同一Solver**升级为当前单Domain全部可移动范围重排；已执行、Firm/Frozen、Demand Protection、Strict Binding等仍保持固定。
4. 1号位返回FinalTask、AllocationTaskShare、真实Task依赖、CTP/影响KPI与Explanation Facts；2号位统一事务持久化，Candidate保持`CANDIDATE`。
5. `INSERT_ORDER_WHATIF`本身永不直接激活。正式采用前应以当前事实重新计算必要Domain，并通过**最小人工授权确认**后激活；只需记录谁、何时、采用哪个Candidate/PlanVersion，不把完整OA审批工作流作为V1主链硬依赖。
6. Candidate、无正式PI号的规划占位Task、UNLOCATED保守Task以及依赖Planning-only Purchase Placeholder而仍无真实执行依据的Task，均不得直接进入MES正式下发。

> ⚠️ **旧版 sp_ExpandBOMRealtime(@BOMNO) 已 deprecated**
...
### 2.5 管道/未来供给链：采购、VMI、已到厂未入库与厂间在途（v1.36 V1正式链路）

> **冻结结论**：V1不再允许`PipelineSupplies`固定为空。采购在途、VMI、已到厂未入库、厂间在途均是交期与有限产能计算所需要的真实Supply事实。现有`SupplyFact_Pipeline`/包装视图骨架优先复用，不为“统一漂亮”重建大一统Pipeline平台。
...
`InventoryBalance`继续表示现货库存，不把未来供给强塞进现货余额。未来供给可以复用`SupplyFact_Pipeline`或清晰的独立事实对象，但不得再通过“V1 WHERE 1=0”让正式数据链断开。

#### 2.5.P2 正式V1供给类型

V1至少承接：
- `INTERPLANT_IN_TRANSIT`：厂间在途；
- `PURCHASE_IN_TRANSIT` / Open PO Remaining：采购未结/在途；
- `VMI`：供应商管理库存；
- `ARRIVED_NOT_RECEIVED`：已到厂尚未完成正式入库/可用确认的数量；
- `ERP_Received_ByDocument_View`：按相同出荷指示号承接ZP/BP Received事实。

Warehouse/质量资格仍按已有库存/供给资格规则执行。VMI仓库与普通PO来源按真实业务身份区分，不能对同一物理数量重复建Supply。

#### 2.5.P3 ETA与AvailableTime

采购类AvailableTime优先级：
1. 人工维护ETA；
2. ERP原始ETA；
3. 无ETA时按PO释放/发行时间 + 默认采购LeadTime估算；若估算日期已落在过去，可按冻结参数增加保守延迟余量。

到厂后如还需检验/上架，使用仓库级“到达→可用”偏移。`AvailableTime`应尽量在数据/上下文层稳定形成，1号位只消费结果，不自行推算采购LT。

人工ETA粒度至少能唯一定位到PO号+行+物料+收货仓库；人工ETA覆盖ERP ETA。

#### 2.5.P4 Planning-only Purchase Placeholder

如果某采购件库存、已到厂未入库、PO、VMI均无已承诺供给，2号位可以在内存形成：

```text
SupplyType   = PLANNING_ONLY_PURCHASE_PLACEHOLDER
Quantity     = 缺口
AvailableTime= 当前计划基准时间 + DefaultPurchaseLT（及已冻结保守余量）
...
正式PO/VMI出现后，下一次FULL/Candidate自动用真实Supply重新Pegging，替代占位。

#### 2.5.P5 数量与去重红线

- 采购剩余量按“采购单号+行（如源系统有）+物料+收货工厂/仓库”唯一扣减；
- 厂间在途、同SH号Received、未生产SH份额必须互斥，禁止同一物理数量既算在途又算Received；
- `SupplyFact_Pipeline`是事实/标准化承接对象，不是Allocation余额；Candidate/PlanVersion运行期间不得通过永久UPDATE其“已分配量”表达版本内消耗。

#### 2.5.P6 采购参考需用日期

90天有限产能完成后，采购/供应商参考需用日期应来自**真正消费该采购件的下游FinalTask计划时间**，而不是简单按客户交期减固定LT。V1可以形成`PurchaseSuggestionDraft`供业务参考，但不自动生成正式采购单。

### 2.8 运行编排框架：ScheduleRun / PlanVersion / Scenario / SimulationRun（v1.36冻结对齐）
...
| 插单局部重排 | `LOCAL_RESCHEDULE` | INSERT_RESCHEDULE | 当前事实重算后，经最小人工授权确认可激活 |
| 人工局部重排 | `LOCAL_RESCHEDULE` | MANUAL_ADJUSTMENT | 经最小人工授权确认可激活 |
| 单 Domain 较大范围人工重排 | `MANUAL_RESCHEDULE` | MANUAL_ADJUSTMENT | 经最小人工授权确认可激活 |
...
```text
在进入需要 `ScheduleRunId/DataCutoffTime` 的运行级快照前，由现有编排创建或取得 ScheduleRun：
...
| 步骤 | 负责人 | 动作 |
|------|--------|------|
| ScheduleRun 创建（夜间 FULL_SCHEDULE） | 现有 NightlyBatchOrchestrator / 3号位生命周期服务 | 创建或取得本次 ScheduleRun（RunType=FULL_SCHEDULE，Status=RUNNING），并在运行启动边界冻结 ExpectedDomainKeysJson；**不强制固定到某一分钟点** |
...
| 白天 Candidate 数据构造与结果持久化 | 2号位 | Order快照 / BOM / 当前Supply Context / Pegging / Lock / 逻辑生产需求；接收1号位FinalTask后统一结果落盘，PlanVersion转CANDIDATE |
| PlanVersion 激活（夜间 FULL_SCHEDULE） | 2号位（调度器 Serializable 事务） | 同 DomainKey 旧 ACTIVE→ARCHIVED + 本次 BUILDING→ACTIVE（无需审批） |
| Candidate 人工确认与激活入口 | 3号位/4号位服务边界 | 最小人工授权确认 + 显式调用激活服务（同域替换ACTIVE）；不要求完整OA审批作为V1硬前置 |
| Scenario 创建 | 3号位/业务 | 可选场景容器；普通CTP/插单WHATIF不强制创建 |
...
**时序**（凌晨全量）：
- 在本次运行冻结规则参数前：`StrategyProfileVersion` 已发布完成
- 创建/取得本次 `ScheduleRun` 时同步绑定 `StrategyProfileVersionId`；具体分钟点由2号位现有编排把握，不为文档一致性强制重写时序
...
- 1号位禁止直接读取规则参数维护表
- 5号位不逐Demand/Task执行最终排序、Lock、拆批或调度规则；5号位只提供BOM/PI位置/跨厂/采购等复杂事实。规则由3号位治理，2号位/1号位在内存消费已冻结结果
...
- 📌 **ScopeJson 固定 11 字段（不得新增 ExpectedDomainKeys）**：`ScopeJson` 继续固定其既有 **11 字段**契约（权威定义见《APS_集成接口设计_v1.12》v1.25）；`ExpectedDomainKeys` 不再位于 `ScopeJson` 中，改由 `ScheduleRun.ExpectedDomainKeysJson` 承载。白天 Candidate 场景**不得为保存 DomainKey 而修改 11 字段 ScopeJson**。

#### 2.8.9 白天Candidate严格单Domain、共享资源阻挡与跨Domain WHATIF

- 每个白天Candidate/ScheduleRun仍严格单Domain：`ExpectedDomainKeysJson`唯一元素 = BasePlanVersion.DomainKey = CandidatePlanVersion.DomainKey。
- 多Domain客户CTP不是一个MultiDomain Candidate，而是后台按`Domain_Dependency`拓扑串行多个单Domain WHATIF；例如C→B→A，每个Domain的输出以多段`Quantity + AvailableTime`作为下一个Domain的输入。
- 前端最终汇总成**一次客户CTP + 影响分析答案**；CTP与INSERT_IMPACT_ANALYSIS入口语义可保留，但不得为了两个Purpose重复跑两次独立Solver。
- 白天某Domain与其它Domain共享设备时，加载其它Domain当前ACTIVE占用作为不可移动阻挡块；本Candidate不得挤动其它Domain。
- V1不建设跨Domain资源配额、借用、MultiDomain Candidate、CandidateGroup、原子多域事务平台。
- `ScopeJson`继续保留既有11字段契约，但其业务含义是发起/初始范围；真实影响可以沿资源/物料/工艺关系超出初始范围。

#### 2.8.10 夜间跨Domain失败、发布门禁与人工恢复

夜间FULL按`Domain_Dependency`拓扑运行。Domain仍独立版本、独立落盘/发布，但依赖链必须保持本次运行的一致性：

- 若上游Domain B在本次Run失败，则**直接或间接依赖B本次计划供给的下游Domain**本次不得发布新ACTIVE，继续保留各自上一ACTIVE。
- 与B无依赖的Domain继续计算并可正常发布，因此仍允许`PARTIAL_SUCCESS`，不回退到全域ALL_OR_NOTHING。
- 被阻断下游应能在UI/Explanation中明确显示“未更新/上游B失败”。
- 同一Run内部的技术瞬时失败允许有限自动重试；重试耗尽必须进入FAILED，禁止长期BUILDING。

**人工恢复**：
1. 修复数据/事实后，人工点击“重新计算”；
2. 必须创建**新的ScheduleRun**，旧FAILED/PARTIAL_SUCCESS以及失败PlanVersion保持不可变审计历史；
3. 系统自动计算重跑范围：失败根Domain + 因其失败未发布的下游；若失败根本身仍依赖未恢复上游，则向上扩展到必要上游；
4. 已成功且仍有有效ACTIVE的无关/上游Domain作为边界输入，不无意义重跑；
5. 按拓扑执行并形成新的PlanVersion。

V1不建设Retry Workflow/RetryGroup/审批平台；人工重算复用既有FULL调度语义和最小审计字段。
...
2号位在 00:30 搬运到 `APS_BOM_CROSS_FACTORY_EDGE_RAW`，作为Pegging结构输入。跨厂模式由2号位依据冻结业务规则执行；5号位只提供CrossFactoryEdge、PI Position、ERPProperty等复杂事实，不以运行时插件形式替2号位作最终Allocation裁决。
...
#### 2.10.4 PeggingSupplyAllocation、逻辑Allocation与Task物理血缘

`PeggingSupplyAllocation`保留为**APS内部的非Task供给分配镜像/细账**，不是第二套余额真相源，也不拥有独立生命周期。真正运行数量真相是2号位内存中的DemandBalance/SupplyBalance及其原子Allocation。

持久化边界：
- 库存、采购、VMI、Received、在途等非Task Supply的已确认分配可写入极简PSA；
- FinalTask形成后，2号位根据1号位返回的AllocationTaskShare保存Demand Allocation ↔ FinalTask数量份额；
- Task↔Task物理依赖只表达真实生产工艺/物料血缘；
- PSA、逻辑Allocation、AllocationTaskShare、Task依赖必须在**同一Domain结果事务**中一次性闭合，禁止事务外二次补写。

#### 2.10.5 两类跨厂的冻结顺序

**STAGE_HANDOFF（大工艺接续）**：
`目标工厂M可用库存 → 选择PI → PI内部Position（XC/PI级在途/Stage WIP等）→ 缺口新增生产`。

- 不搜索“上游工厂M库存”作为通用补给；
- 先选择PI号，再在该PI内部消费互斥Position；
- 运输/检验只通过真实LeadTime形成下游AvailableTime；
- 不生成ShippingTask有限物流Task。

**INTER_FACTORY_ORDER（出荷指示）**：
`目标/收货工厂BS/KS可用库存 → 现有SH`；进入某SH后，顺序为`同SH厂间在途 → 同SH号ZP/BP Received → 仍未生产份额`。

- 只有同一SH号的Received才能承接该SH；
- 在途与Received必须互斥去重；
- 未生产份额在下一Pegging层转为来源工厂生产需求；
- 上层Demand只Pegging到SH，不在同一层递归生成来源工厂Task。

#### 2.10.6 号位分工（冻结）

| 工作 | 负责人 |
|---|---|
| `ProcessCodeDict.ERPProperty`真实业务属性同步/透出 | 5号位 |
| `ERP_Received_ByDocument_View`及采购/VMI/在途等ODS事实 | 5号位 |
| CrossFactoryEdge结构事实 | 5号位 |
| APS包装视图/RAW搬运/当前Supply Context | 2号位 |
| PI Position复杂推导 | 5号位 |
| PI选择、跨厂模式、Demand→Supply Allocation、PSA | 2号位 |
| 运输LeadTime转AvailableTime所需事实/参数 | 5号位事实 + 3号位参数 + 2号位组装 |
| FinalTask/Resource/Start-End/真实Task依赖 | 1号位 |

5号位不返回最终HardLock、不替2号位做Allocation，也不生成FinalTask；1号位不直接查询Received/ERPProperty/PSA或原始PI Stage事实。

### 2.9 MES生产进度汇总防腐链路（v1.36 PI Position对齐；保留v1.30快照结构）

#### 2.9.1 定位与设计决策（v1.36 PI Position对齐）

MES生产进度快照继续保留，其职责是提供**生产指示当前事实**，不是让1号位直接按累计Stage数量再次“扣减Task”。

V1冻结链路：

```text
ERP PI总量边界（Quantity - ReceivedQty）
 + MESWorkOrder/Operation/Stage累计事实
 + PI级XC
 + PI级跨厂在途
 + Stage路径/强位置事实
        ↓
5号位 PI Position Calculator
        ↓
一张最小PI位置快照表（同一PI多行互斥份额，总量闭合）
        ↓
2号位选择PI并按Position形成剩余Stage逻辑生产需求
        ↓
1号位只收到已解析起点/剩余Routing/数量/执行锚点
```

**核心红线**：
- PI剩余总量以ERP `max(Quantity-ReceivedQty,0)`为边界；MES完成量、XC、在途只解释“这批量在哪里”，不得重新叠加/缩放PI总量；
- `StageProgressSnapshot`仍有价值，但属于5号位PI Position输入；1号位不得再形成第二套“StageProgress直接扣Task”逻辑；
- 下游Stage累计量超过上游时，V1保守下修下游；中间Stage缺失时采用下游已证明的最小完成量；强位置事实可以保守修正有效计划进度，但不回写源MES；
- 无法可靠定位但PI总量可信时，整个相关份额可降级为`UNLOCATED`，从最早可信Stage开始保守排程并登记Issue；UNLOCATED计划不得下发MES；
- 生产指示不能消费自己：由某PI剩余份额派生出的下阶Demand，禁止再由同一个PI作为同物料Supply承接。

Task/Pegging仍按新PlanVersion重新生成，不跨版本匹配历史TaskId。MES五态保持0～4，不新增PAUSE/RESUME闭环。
...
| 视图 | 字段（完整列表） | 汇总颗粒度 |
|------|----------------|-----------|
| `MES_APS_WorkOrder_View` | ProductionInstructionNo / MESWorkOrderNo / MaterialCode / PlannedQty / WorkOrderStatus / SourceUpdatedAt | 生产指示号 + MES工单号 + 物料编码 |
...
| 快照表 | 颗粒度 | 计算列 | 分区键 |
|--------|--------|--------|--------|
| `MESWorkOrderSnapshot` | 生产指示号 + MES工单号 + 物料编码 | — | `ScheduleRunId` |
...
**执行时机**：三个MES快照同步SP必须在本次 `ScheduleRunId` 与统一 `DataCutoffTime` 已确定之后、ScheduleContext装载之前完成，并共享同一运行切片。**本文不强制00:38/00:40/00:45/00:50等固定分钟点**；具体时序由2号位现有编排把握，只要数据切片一致、可追溯并在正式求解前完成即可。
...
**ScheduleRun / PlanVersion 创建时序（冻结对齐）**：V1只要求本次运行有明确ScheduleRun、每个Domain有独立PlanVersion、规则/数据切片/结果可追溯且终态闭合。**不强制2号位把ScheduleRun或PlanVersion固定创建在某一个分钟点，也不要求为了文档形式改写现有时序。** 若快照SP需要ScheduleRunId，则应在调用这些SP前取得本次RunId与DataCutoffTime。

#### 2.9.4 ScheduleContext装载与PI位置消费

```text
APS.MESWorkOrderSnapshot      → 5号位 PI Position输入
APS.OperationProgressSnapshot → 5号位 PI Position输入（需要工序级精度时）
APS.StageProgressSnapshot     → 5号位 PI Position输入（大工艺累计事实）
PI级XC/跨厂在途/Stage路径     → 5号位 PI Position输入
PI Position快照              → 2号位 Pegging/逻辑生产需求
已解析起点/执行锚点           → 1号位 ScheduleContext
```

**消费规则**：
1. 5号位按`ProductionInstructionNo + MaterialCode`聚合所有相关事实，生成互斥Position份额；
2. 2号位先按PI选择规则选择PI号，再消费该PI内部Position，不允许全局把不同PI的位置切片混排；
3. 被Demand承接的PI份额不再次变成独立Demand；只有未被上层Demand承接的PI剩余量才按层级扫尾形成PI自身需求；
4. 1号位只消费2号位已经解析好的生产起点、Routing剩余段、数量、Material AvailableTime和不可移动执行锚点；
5. 最终Task如需要追溯生产指示号，应透传相应PI业务键，但Task与Demand仍允许多对多，不得再用`Task.OrderId`表达唯一业务归属。


### 2.11 PI Position最小落地模型（v1.36新增）

V1只建设**一张最小PI位置快照表**，同一PI用多行表示互斥位置份额，不建设Header+Slice双生命周期平台。最小业务字段应能表达：
- ScheduleRun/PlanVersion切片锚点；
- ProductionInstructionNo、MaterialId/Code；
- PositionType（STAGE_WIP/XC/INTERPLANT_TRANSIT/WAITING_INBOUND/UNLOCATED等受控枚举）；
- Quantity；
- CurrentStage/NextStage（按位置类型允许为空）；
- AvailableTime；
- SourceType/SourceKey；
- Issue/Confidence必要标记。

同一PI各Position Quantity之和必须与本次可定位PI剩余边界闭合；无法闭合属于P0数量完整性问题，不能仅Warning后发布。

### 2.12 Demand Protection、Firm/Frozen与执行事实

V1明确三类概念：
1. **Strict Binding**：客户专属、质量/环保资格、明确SH/专用Supply等不可替换绑定；
2. **Demand Protection**：短交期/已延期/受保护需求的必要Supply份额，锁的是数量份额，不锁整个PI/PO；
3. **Execution Constraint**：MES已开工/领料/实际执行形成的物理不可逆事实，但不必然等于原Demand所有权永久锁定。

需求—供给锁可以复用一个最小实体，通过`LockType=STRICT_BINDING/DEMAND_PROTECTION`区分；Execution事实保持独立。3号位治理Lock Policy，2号位在Pegging时创建、传播、消费和释放。

Firm/Frozen是**排程不可移动约束**，不等于Pegging Lock。V1不建设`FrozenZoneSnapshot`平台；新Run从上一ACTIVE读取仍有效Firm/Frozen Task作为不可移动锚点，MES执行不可逆事实由Execution约束承接。

### 2.13 计划良率与FinalTask数量模型

- `NetRequiredQty/NetOutputQty`：Demand/Allocation闭合的净合格数量；
- `PlannedProcessQty`：考虑计划良率后用于占用设备能力的计划加工量。

2号位从下游净需求向上反算各Stage计划加工量；1号位按`PlannedProcessQty`占有限产能；AllocationTaskShare按净合格份额闭合。已有ERP PI Supply总量不能因为计划良率被“膨胀”。

`Task.Quantity`在V1统一解释为净合格产出数量；新增`PlannedProcessQty`承载产能加工量。Task不再以单一`OrderId`作为业务真相，应允许一个FinalTask承接多个Allocation，一个Allocation拆到多个FinalTask。

### 2.14 跨Domain Quantity-Time与共享资源

跨Domain上游结果只在内存作为下游Supply Context传递，必须保留数量—时间切片，例如`40@8/15 + 60@8/17`，禁止压成`100@8/17`或持久化成`VirtualInventoryBalance`。

夜间FULL虽然每个Domain独立PlanVersion，但Domain不是资源隔离墙：同一物理设备被多个Domain共享时，当前Run必须保证时间轴真实互斥，不能各域独立算出同一设备同一时段双占。具体实现优先复用真实资源时间轴/拓扑顺序/已计算占用传递，不扩展成跨Domain配额、借用、容量预约平台。

跨Domain转运LeadTime来自真实工厂/Stage/既有运输口径；`Domain_Dependency.DefaultLeadTimeDays`若为兼容字段可保留，但不能以“统一2天”作为业务权威。

### 2.15 V1防过度设计边界

本防腐层的职责是吸收源系统变化并提供稳定事实，不是趁机建设通用平台。V1明确不建设：
- 有限物流Task/ShippingTask Solver；
- 第二套远期Rough-Cut Solver；
- FrozenZoneSnapshot平台；
- VirtualInventoryBalance持久化状态平台；
- MultiDomain Candidate/CandidateGroup/原子多域激活；
- 跨Domain资源配额/借用/Capacity Reservation平台；
- 通用插件中心、DSL、SQL/Python/JS运行时规则脚本；
- 全搜索SolverTrace/因果图平台；
- 为PI Position建设Header+Slice多表状态机。

已有表/代码如仍在且删除风险大，可保留为兼容或历史对象，但不得进入V1正式主链或反向要求业务实现。
...
-- 创建日期：2026-04-01
-- 负责人：5号位/数据Owner（ODS事实契约维护；如现有视图代码已由3号位实现，不要求迁移仓库，但3号位不再作为业务数据Owner）
...
FROM (
    ...
    WHERE r.IsActive = 1
    UNION ALL
    -- 老旧结构工艺（5号位/数据Owner在ODS事实契约层处理MaterialModel→MES_ID；历史实现无需为职责文字迁移）
    SELECT 
        m.MES_ID,
...
-- 契约版本：v1.0
-- 创建日期：2026-04-01
-- 负责人：5号位/数据Owner（ODS事实契约维护；3号位只治理规则/参数）
...
FROM MES.dbo.RoutingDependency  -- ⚠️ 实际物理表名，由5号位/数据Owner在ODS契约层适配（含老结构ETL处理MES_ID）
WHERE IsActive = 1;
...
-- 契约版本：v1.0
-- 创建日期：2026-04-01
-- 负责人：5号位/数据Owner（从MES工序-设备能力关系表输出ODS事实契约；3号位只治理规则/参数）
...
SELECT 
    ...
    IsActive            -- 是否有效（契约字段）
FROM MES.dbo.OperationResourceMapping  -- ⚠️ 实际物理表名，由5号位/数据Owner在ODS契约层适配（含老结构ETL处理MES_ID）
WHERE IsActive = 1;
...
- **5号位/数据Owner负责ODS Routing事实契约**：
  - ~~`MES_APS_Routing_View`~~：⚠️ v5.0废弃，已拆分为以下视图
  - `MES_APS_Routing_Operation_View`：工序节点事实视图（v5.0新增）
  - `MES_APS_Routing_Dependency_View`：工序依赖事实视图（v5.0新增）
  - `APS_OperationResourceEligibility_View`：工序资源资格事实视图（v5.0新增）
  - `MES_APS_Routing_Stage_View`：大工艺阶段事实/字典视图（v1.7新增，v1.8定位调整）
  - **3号位只治理Routing相关可变规则/参数，不维护源事实契约。若历史视图代码已由3号位实现，可原地保留代码，不要求为了职责文字迁移仓库。**
...
- ❌ **1号位不直接读ODS Routing源视图；3号位不作为Routing源事实Owner**。ODS Routing事实契约由5号位/数据Owner维护，2号位同步装载并组装ScheduleContext，1号位只消费2号位给出的合法Routing/Resource候选图；3号位只治理相关规则与参数。
...
**C 类使用边界**：
- ✅ ODS内部：委外/运输事实标准化、运维诊断、BI报表（V1不生成有限物流ShippingTask）
...
> 表结构以 v5.1.1 为准（§5.1.5 / §5.1.6 / §5.1.6b）。
>
...
> **v1.34 修正**：本表结构以 **DDL v5.1.1** 为唯一权威（DDL 中已含辅助字段、回填结果字段、`RequestDetailId` 追溯锚点及索引定义）。本节仅摘录字段约束与索引配合，避免与 DDL 出现二次定义漂移。

**字段摘要**（以 DDL v5.1.1 为准）：
...
**索引摘要**（以 DDL v5.1.1 为准）：
- `IX_Realtime_BOMNO (BOMNO, Level)`：**聚集索引**（因此 `Id` 主键必须为 `NONCLUSTERED`）

> 完整 DDL 请查阅 `APS_数据库表结构设计_v5.0.sql v5.1.1` 中的 `dbo.MES_APS_BOM_Workset_Realtime` 定义。

---

#### **5.1.6b 实时BOM跨厂交接边表**（v1.34 新增，对齐 DDL v5.1.1）

> **本表结构以 DDL v5.1.1 §3.18 和字段说明 v5.1.1 §2.1b 为唯一权威**，本节仅作引用说明。
>
...
CREATE TABLE InventoryBalance (
    ...
    AllocatedQty DECIMAL(18,4) NOT NULL DEFAULT 0,   -- 已分配量
...
##### **6.1.3.1 物理 SP 签名（DDL v5.1.1 权威）**
