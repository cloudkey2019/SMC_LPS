...
**版本**：v5.1.2 冻结对齐版  
**日期**：2026-08-12  
**目的**：供全体开发团队（0-5号位）参考的完整数据库字段与业务语义基线  
**上位业务基线**：《APS V1最终全部流程与业务基线》+《APS Pegging供需承接与分层计算业务说明 v1.1 冻结对齐版》+《APS有限产能排产与滚动90天计划业务说明 v1.1 冻结对齐版》  
**技术对齐基线**：《核心排产全流程走查 V3.17 冻结对齐版》+《各类基础数据分层承接与演变总表 v3.32 冻结对齐版》+《数据架构与防腐层 v1.36 冻结对齐版》+《集成接口 v1.26 冻结对齐版》  
**DDL状态**：当前物理DDL仍为v5.1.1；本文v5.1.2先冻结字段目标语义，下一步由DDL文档按本文实施，不允许DDL反向改业务。  
**适用人员**：0号位（业务负责人）、1-5号位（开发团队）、DBA

> ## 冻结声明
> 本文是**字段层技术对齐文档**，无权重新设计已冻结业务。旧字段、旧表、历史代码或已有审批结构如与上位冻结业务冲突，应由字段/DDL/代码向冻结业务收敛。只有出现真实新业务事实或冻结规则无法实现时，才允许显式“重新打开冻结决策”，并由0号位重新裁决。
> 
> v5.1.1及更早版本说明继续保留用于历史追溯；其中“PriorityScore为主排序权威”“Pipeline V1固定为空”“1号位直接按StageProgress扣Task”“Task只能归属一个Order”“ROUGH_CUT为90天远期主链”“完整OA审批是Candidate激活硬前提”等均不再具有当前V1业务权威性。

**相关文档**：
- **《APS 核心排产全流程走查》V3.17 冻结对齐版**：端到端运行流程与0～5号位接力
- **《APS_各类基础数据分层承接与演变总表》v3.32 冻结对齐版**：数据演进全景图
- **《APS_数据架构与防腐层设计方案》v1.36 冻结对齐版**：物理架构、防腐与数据流
- **《APS_集成接口设计》v1.26 冻结对齐版**：内部服务与数据契约
- **《APS_数据库表结构设计_v5.0.sql》**：当前v5.1.1，下一步按本文v5.1.2对齐
- **《APS_权限与审批系统实施方案_v1.0》**：企业可选审批能力；不等于V1 Candidate激活主链硬依赖
- **《APS_Auth数据库字段说明文档_v1.0》**：权限/审计字段参考

**⚖️ 重要更新（v5.1.2 冻结对齐，2026-08-12）**：
- 🔄 `Order.PriorityScore`降为历史/展示兼容字段，不再作为V1需求排序权威；V1排序由“计算层→有序规则段→第一命中→段内排序”执行。
- 🔄 `PlanVersion.ComputeMode`不再用`ROUGH_CUT/CRITICAL_PATH`选择第二套远期排程引擎；90天正式计划统一进入有限产能Solver。
- 🔄 `Task.OrderId`降为可空兼容主展示字段；真实Demand归属改由`AllocationTaskShare`多对多承接。
- 🆕 `Task.PlannedProcessQty`：与`Task.Quantity`（净合格产出）分离，专供有限产能占用。
- 🆕 `AllocationTaskShare`最小表：保存Allocation↔FinalTask净合格数量份额，不新增独立PeggingAllocationLedger物理表。
- 🆕 `ProductionInstructionPositionSnapshot`：V1只建一张最小PI位置快照，多行互斥闭合，不建Header+Slice平台。
- 🔄 `StageProgressSnapshot/OperationProgressSnapshot`定位改为5号位PI Position输入；1号位不再直接据其形成第二套PI位置解释。
- 🔄 `PeggingSupplyAllocation`补`AllocationSequence`与供给承诺语义；PI内部XC/Stage/在途不再被当成额外Supply。
- 🆕 沿用同一需求—供给锁实体表达Strict Binding与Demand Protection，通过`LockType=STRICT_BINDING/DEMAND_PROTECTION`区分；Execution事实独立。
- 🔄 采购在途、未结PO、VMI、已到厂未入库、厂间在途正式进入V1 Timed Supply；`SupplyFact_Pipeline`不再定义为固定空链路。
- 🔄 `SupplyAvailabilityRule`保留兼容，但不再作为V1必须调用的“唯一管道规则平台”；真实AvailableTime按冻结ETA/DefaultLT口径归一。
- 🔄 `TaskType`新生成任务V1只允许生产Task；TRANSFER/PROCUREMENT仅历史兼容，不建设有限物流Task/采购Task。
- 🔄 `FenceConfig`保留Firm/Frozen参数语义，但不再把Firm调整绑定到完整OA审批；跨版本锚点从上一ACTIVE读取，不建设FrozenZoneSnapshot。
- 🔄 Scenario/ApprovalFlow等既有表保留，但标为可选企业能力，不能反向成为V1主链前置条件。
...
| 消费方 | 全链路 | APS 排程内核 | ODS 内部（委外/跨厂事实诊断、LeadTime派生、运维诊断、BI报表）|
| 是否防腐层 | ✅ 是 | ✅ 是（跨库隔离）| ❌ 非防腐层 |
...
| 归属 | APS自主业务语义字典 | **APS自维护ODS增强字典**；ERPProperty由5号位同步真实ERP属性 |
| 维护方 | APS团队 | APS增强字段由管理员维护；ERPProperty由5号位同步；不恢复整表`sp_SyncMasterData(ProcessCode)` |
...
- 委外/跨厂事实诊断：按`ParentStageFactory ≠ ParentActualFactory`（或子件侧同理）识别委外段；**V1只形成跨厂事实与LeadTime，不生成有限物流Task**
...
**🔒 消费边界**：
- ✅ ODS 内部：5号位`material_stage_chain`推导、2号位`MaterialStageDeptContext`组装、委外/跨厂事实诊断与LeadTime派生、`vw_MES_BOM_Stage_Enriched`、运维诊断、BI报表
...
| 委外/跨厂事实诊断 | 识别`FactoryCode ≠ ActualFactoryCode`段，供跨厂模式/LeadTime事实使用；V1不生成ShippingTask |
...
**用途**：提供工序级生产进度汇总；APS侧同步到`OperationProgressSnapshot`，主要作为5号位PI Position计算和Execution事实解析输入，不再由1号位直接据此建立另一套PI位置/Task扣减真相  
...
**所属库**：`MES_Integration`（ODS库）  
**维护责任**：5号位（收口，各大工艺子视图 UNION ALL）  
**用途**：提供大工艺阶段级累计进度；APS侧同步到`StageProgressSnapshot`，作为5号位PI Position闭合的主要事实输入之一；2号位消费Position结果，1号位只接收已解析的起始Stage/剩余工艺与执行约束  
...
> **一致性说明**：原则上，同一`ProductionInstructionNo + MaterialCode + StageCode`下两类进度应同口径。若不一致，由5号位按冻结PI Position规则结合下游已证明完成量、XC/在途等强事实保守闭合；不得简单让1号位“优先用StageProgress扣Task”。可降级的数据问题登记Issue；若PI总量/位置数量无法闭合则不得发布该Domain。
...
**来源系统**：ERP  
**维护责任人**：5号位  
**业务用途**：ODS层ERP厂间物流运输在途数据契约视图；V1正式Timed Supply来源之一。  
**V1状态**：业务能力正式启用；5号位必须接入真实有效在途事实。某次运行0行仅表示当前无在途或来源暂不可用，**不得再定义成V1固定空链路**。  
**实施边界**：可替换内部SELECT/JOIN/WHERE接真实源表，但不得破坏对外字段契约。  
**字段语义红线**：
...
- V1正式包装链：源视图返回真实厂间在途；某批次0行允许，但不是固定业务规则
...
**定位**：将ODS层各类Timed Supply的APS单来源包装视图通过UNION ALL收敛为统一输入；`sp_SyncPipelineSupply`只消费本视图，避免2号位猜源表。  
**V1状态**：厂间在途、采购在途/未结PO、VMI、已到厂未入库均为正式业务能力。分支1使用既有`ext_ERP_InterplantInTransit_View`；分支2~4由5号位/采购DBA绑定真实契约视图。本文**不虚构具体源表名**，DDL落地时必须以已存在真实契约为准。单一来源某批次0行合法，但不得通过`WHERE 1=0`把整个V1能力固定为空。  
**扩展方式**：新增来源时只增加ODS契约+APS包装+本视图UNION分支，`sp_SyncPipelineSupply`主流程保持稳定。

**分支枚举**：
| 分支 | SourceSystem | SupplyType | V1 来源 |
|------|-------------|------------|---------|
| 1 厂间在途 | ERP | INTERPLANT_IN_TRANSIT | ext_ERP_InterplantInTransit_View |
| 2 采购在途/未结PO | PROCUREMENT | PURCHASE_IN_TRANSIT / OPEN_PO_REMAINING | 5号位/采购DBA正式ODS契约（以现有真实视图为准） |
| 3 VMI | VMI | VMI_ONSITE | 5号位正式VMI契约（以现有真实视图为准） |
| 4 已到厂未入库 | ERP/PROCUREMENT | ARRIVED_NOT_RECEIVED | 5号位正式到厂未入库契约（以现有真实视图为准） |
...
| ComputeMode | 计算模式 | NVARCHAR(50) | **兼容/诊断字段**。V1正式90天计划统一进入同一有限产能Solver，不得用`ROUGH_CUT`/`CRITICAL_PATH`把远期切成第二套排程语义；历史值可保留用于旧记录展示，不作为Solver选择权威 | DOMAIN_SPLIT |
...
> - `(SourceScheduleRunId, DomainKey)` 唯一：对 `SourceSimulationRunId IS NULL` 的正式运行，同一运行 + 同一分域至多一个版本（阶段二 `SimulationRun` 产生的同域多候选版本以 `SourceSimulationRunId` 区分，不受该唯一约束限制）。
...
| Priority | 基础优先级 | INT | 可作为需求排序规则的一个输入信号；不得单独代表最终排序结果 | 80 |
| PriorityScore | 历史综合优先级分数 | DECIMAL(10,2) NULL | **历史/展示兼容字段，V1主链不再使用。**需求排序权威为“计算层→有序规则段→第一命中→段内排序”；不得重新通过全局加权Score决定Pegging先后 | NULL |
...
**所属库**：APS_Production  
**业务用途**：记录1号位有限产能Solver生成的最终生产任务（FinalTask，按PlanVersionId分区）。2号位不得在Solver前提前把逻辑生产需求固化成最终Operation Task。
...
| OrderId | 兼容主展示订单ID | BIGINT NULL | **非真实Demand归属权威。**一个FinalTask允许承接多个Demand；新V1记录可空或仅用于兼容/主展示，真实数量归属必须通过`AllocationTaskShare` | NULL |
...
| Quantity | 净合格产出数量 | DECIMAL(18,4) | FinalTask预计形成的净合格产出（NetOutputQty）；用于与Demand/Allocation数量闭合 | 500 |
| PlannedProcessQty | 计划加工数量 | DECIMAL(18,4) NULL | **v5.1.2新增目标字段**；考虑计划良率后用于占用有限产能的加工量。新V1生产Task必须有值；历史记录可NULL兼容 | 520 |
| UOM | 计量单位 | NVARCHAR(20) | PCS/KG/M等（2026-04-03审计补充） | PCS |
...
| IsLocked | 当前计划内不可移动标记 | BIT | 1=当前计划/求解上下文中不可移动；可用于Firm/Frozen/执行锚点落地展示，但**不能替代DemandSupplyLock或Execution事实** | 1 |
| IsCriticalPath | 是否关键路径 | BIT | 1=关键路径 | 1 |
| TaskType | 任务类型 | NVARCHAR(50) | **V1新生成Task只允许`PRODUCTION`。**`TRANSFER/PROCUREMENT`仅历史兼容；跨厂运输只形成LeadTime→AvailableTime，采购供给不生成Task | PRODUCTION |
...
> |---|---|---|---|
> | 0 | 待开工 | PLANNED 或 RELEASED | 未下发 MES 为 PLANNED；已下发未开工为 RELEASED（状态0不能单靠 MES 区分二者，需结合是否已正式下发 MES） |
...
> **v5.1.2 FinalTask数量与归属红线**：
> - `Task.Quantity` = 净合格产出数量；`Task.PlannedProcessQty` = 占用资源能力的计划加工数量。
> - 一个FinalTask可以承接多个Demand，一个Demand也可以拆到多个FinalTask。
> - `Task.OrderId`不得再被代码当作唯一业务归属真相。
> - 新生成的TRANSFER/PROCUREMENT Task禁止进入V1主链。

---

### 4.4b AllocationTaskShare（Allocation↔FinalTask数量份额表）⭐ **v5.1.2新增目标结构**

**所属库**：APS_Production  
**业务用途**：保存逻辑Demand Allocation与1号位FinalTask之间的净合格数量份额，是Task多Demand归属的权威关系。**不因此新增独立`PeggingAllocationLedger`物理表**；逻辑Ledger仍在2号位内存中原子生成。

| 英文字段名 | 中文含义 | 数据类型 | 业务说明 | 示例值 |
|-----------|---------|---------|---------|--------|
| Id | 记录ID | BIGINT | 主键，自增 | 1 |
| PlanVersionId | 计划版本ID | INT NOT NULL | 与Task/Allocation同版本切片 | 101 |
| AllocationSequence | 分配序号 | BIGINT NOT NULL | Demand/Supply原子扣减成功时生成的稳定序号；禁止到持久化阶段临时编号 | 900001 |
| DemandType | 需求类型 | NVARCHAR(50) NOT NULL | SALES_ORDER / PRODUCTION_INSTRUCTION / SH_REQUIREMENT / LOWER_LEVEL_REQUIREMENT 等受控业务类型 | SALES_ORDER |
| DemandKey | 需求稳定键 | NVARCHAR(200) NOT NULL | 逻辑Demand稳定标识；用于在不新建通用Allocation主表的前提下追溯份额 | SO:20260812-001 |
| RootOrderId | 根订单ID | BIGINT NULL | 可选追溯到顶层Order | 1001 |
| TaskId | FinalTask ID | BIGINT NOT NULL | 逻辑关联Task.Id | 80001 |
| ShareQty | 净合格数量份额 | DECIMAL(18,4) NOT NULL | 本Demand由该FinalTask承接的净合格产出份额；同一Allocation可多Task，同一Task可多Allocation | 120.0000 |
| CreatedAt | 创建时间 | DATETIME2 NOT NULL | 结果持久化时间 | 2026-08-12 02:12:00 |

**闭合红线**：
- 对每个Allocation：`AllocatedQty = Σ AllocationTaskShare.ShareQty`（允许非Task供给部分由PSA承接时按对应逻辑分开校验）；
- 对每个FinalTask：各ShareQty之和不得超过`Task.Quantity`；若Task为合并批，多个Demand份额之和应与其净合格产出闭合；
- 表仅持久化最终结果，不参与1号位求解中间迭代。
...
**所属库**：APS_Production  
**定位**：记录一个业务试算场景的假设、目标和最终选中版本。不是运行表，不是结果版本表。**V1可选业务容器**：业务需要保存场景时可使用，但Candidate/CTP主链不得因为Scenario表存在而被强制先建Scenario；阶段二另含`SimulationRun`与`ScenarioObjectiveScore`骨架。
...
| 3号位 | 规则/参数/策略包治理、发布和运行生命周期相关配置；提供冻结版本快照，不做逐笔Pegging裁决服务器 |
| 4号位 | 规则参数维护界面、策略包选择界面、仿真参数界面、规则命中解释展示 |
| 5号位 | ODS/BOM/Stage/跨厂/采购等复杂事实与PI Position推导；可消费规则参数，但**不做最终Demand↔Supply Allocation、不生成FinalTask、不作为ReasonCode中央插件** |
| 2号位 | APS主流程/Pegging数量真相、数据装载、Demand/Supply原子Allocation、Lock执行、ScheduleContext构造、1号位结果校验与统一持久化、ScheduleRun/PlanVersion状态闭合；保护现有Orchestrator外壳 |
| 1号位 | 有限资源与时间真相所有者；消费完整ScheduleContext，生成FinalTask、资源/时间、拆合批、Task物理依赖、AllocationTaskShare与Explanation；纯内存求解，不写DB |
...
### Pegging补充：ProductionInstructionPositionSnapshot（PI位置最小快照表）⭐ **v5.1.2新增目标结构**

**所属库**：APS_Production  
**业务用途**：保存5号位对同一生产指示（PI）剩余数量所做的**互斥位置闭合结果**。一张PI可多行表示Stage WIP、XC、PI级跨厂在途、等待入库或UNLOCATED；**不建设PI Header + PositionSlice双生命周期平台**。

| 英文字段名 | 中文含义 | 数据类型 | 业务说明 | 示例值 |
|-----------|---------|---------|---------|--------|
| Id | 记录ID | BIGINT | 主键，自增 | 1 |
| ScheduleRunId | 排程运行ID | INT NOT NULL | 绑定本次事实切片 | 101 |
| PlanVersionId | 计划版本ID | INT NOT NULL | 绑定当前Domain版本 | 501 |
| ProductionInstructionNo | 生产指示号 | NVARCHAR(100) NOT NULL | PI稳定业务键 | PI-2026-00123 |
| MaterialId | 物料ID | INT NOT NULL | APS标准物料ID | 200 |
| MaterialCode | 物料编码 | NVARCHAR(100) NOT NULL | 冗余追溯 | MAT-A001 |
| PositionType | 位置类型 | NVARCHAR(50) NOT NULL | `STAGE_WIP / XC / INTERPLANT_TRANSIT / WAITING_INBOUND / UNLOCATED`等受控值；Position不是额外Supply身份 | STAGE_WIP |
| Quantity | 位置数量 | DECIMAL(18,4) NOT NULL | 该PI当前位于此位置的互斥数量份额 | 80.0000 |
| CurrentStageCode | 当前阶段 | NVARCHAR(50) NULL | PositionType允许时填写 | TJ_MACH |
| NextStageCode | 下一需执行阶段 | NVARCHAR(50) NULL | 交给2号位形成逻辑生产需求起点 | TJ_SURF |
| AvailableTime | 可用时间 | DATETIME2 NULL | 在途/等待等事实真正可供下一阶段使用的时间；Stage WIP通常由后续排程决定 | 2026-08-13 08:00 |
| SourceType | 来源类型 | NVARCHAR(50) NULL | MES_STAGE / XC / INTERPLANT_TRANSIT / RECEIVED / DERIVED 等 | MES_STAGE |
| SourceKey | 来源稳定键 | NVARCHAR(200) NULL | 源工单/单据/位置追溯键 | WO:9988 |
| IssueCode | 位置问题码 | NVARCHAR(100) NULL | 无异常时NULL；无法定位可为UNLOCATED类Issue | NULL |
| Confidence | 置信标记 | NVARCHAR(30) NULL | `CONFIRMED / CONSERVATIVE / UNLOCATED`等轻量标记 | CONFIRMED |
| CreatedAt | 快照时间 | DATETIME2 NOT NULL | 本次位置快照生成时间 | 2026-08-12 01:55 |

**数量红线**：`Σ Position.Quantity = 本次PI可定位剩余边界`。PI总剩余边界仍以ERP生产指示`Quantity-ReceivedQty`为准；MES完成量、XC、PI级在途只决定位置，不能重复加成Supply。无法可信定位时以`UNLOCATED`保守承接，不能猜Stage。

---

### Pegging补充：DemandSupplyHardLock（需求—供给份额锁，沿用同一实体）⭐ **v5.1.2冻结语义**

**定位**：不另建DemandProtection第二张表。沿用同一需求—供给锁实体，以`LockType`区分严格绑定与需求保护；Execution事实保持独立。若现有代码实体名为`DemandSupplyHardLock`，优先沿用并补字段/语义，避免无意义重命名。

| 英文字段名 | 中文含义 | 数据类型 | 业务说明 | 示例值 |
|-----------|---------|---------|---------|--------|
| Id | 锁ID | BIGINT | 主键，自增 | 1 |
| LockType | 锁类型 | NVARCHAR(30) NOT NULL | `STRICT_BINDING / DEMAND_PROTECTION` | DEMAND_PROTECTION |
| DemandType | 需求类型 | NVARCHAR(50) NOT NULL | 需求业务类型 | SALES_ORDER |
| DemandKey | 需求稳定键 | NVARCHAR(200) NOT NULL | 跨PlanVersion可重新识别需求，不依赖旧TaskId | SO:20260812-001 |
| SupplyType | 供给类型 | NVARCHAR(50) NOT NULL | INVENTORY / PRODUCTION_INSTRUCTION / SHIPPING_INSTRUCTION / PURCHASE_ORDER / VMI 等 | PRODUCTION_INSTRUCTION |
| SupplyKey | 供给稳定键 | NVARCHAR(200) NOT NULL | PI号、PO行、库存来源键、SH号等 | PI:PI-2026-00123 |
| LockedQty | 锁定数量份额 | DECIMAL(18,4) NOT NULL | **锁份额，不锁整个PI/PO** | 120.0000 |
| SourcePlanVersionId | 来源版本ID | INT NULL | 首次形成/最近确认该锁的版本，用于审计，不作为稳定业务键 | 501 |
| SourceAllocationSequence | 来源分配序号 | BIGINT NULL | 可选追溯到形成锁的Allocation；跨版本重算时可为空/更新 | 900001 |
| Status | 状态 | NVARCHAR(20) NOT NULL | `ACTIVE / RELEASED / BROKEN` | ACTIVE |
| CreatedAt | 创建时间 | DATETIME2 NOT NULL | | 2026-08-12 02:03 |
| CreatedBy | 创建来源 | NVARCHAR(100) NULL | SYSTEM/PMC/规则来源 | SYSTEM |
| ReleasedAt | 释放时间 | DATETIME2 NULL | 完成/取消/人工释放时填写 | NULL |
| ReleasedBy | 释放人/来源 | NVARCHAR(100) NULL | | NULL |
| ReleaseReason | 释放原因 | NVARCHAR(500) NULL | 人工释放必须可追溯 | NULL |

**边界**：
- Strict Binding：客户专属、质量/环保资格、明确业务单据绑定等不可替换关系；
- Demand Protection：短交期/已延期等受保护需求的必要Supply份额；
- Execution Constraint：MES已开工/领料等现实不可逆事实，**不写入本表替代所有权锁**，由ExecutionLock/执行约束对象独立承接；
- Firm/Frozen属于排程不可移动约束，也不等同于本表。

---

### Pegging 补充：PeggingSupplyAllocation（v5.0.46 新增；v5.1.2冻结对齐）
...
|------|------|------|------|
| Id | 主键 | BIGINT | 自增 |
| PlanVersionId | 计划版本ID | INT NOT NULL | 逻辑关联 PlanVersion.Id；数据库未建立物理外键 |
| ScheduleRunId | 排程运行ID | INT NOT NULL | 逻辑关联 ScheduleRun.Id；数据库未建立物理外键 |
| AllocationSequence | 分配序号 | BIGINT NOT NULL | Demand/Supply原子扣减成功时生成；用于关联TaskShare/解释，不得在持久化阶段临时seq++ |
...
| SupplyType | 供给类型 | NVARCHAR(50) | `INVENTORY / PRODUCTION_INSTRUCTION / SHIPPING_INSTRUCTION / INTERPLANT_IN_TRANSIT / ARRIVED_NOT_RECEIVED / PURCHASE_IN_TRANSIT / OPEN_PO_REMAINING / VMI_ONSITE`等。**PI内部Stage WIP/XC/PI级在途属于PI Position，不得作为额外Supply重复写入；NEW_REQUIREMENT不是Supply身份** |
...
| CrossFactoryEdgeId | 跨厂边ID | BIGINT NULL | 逻辑关联 CrossFactoryEdge.Id；数据库未建立物理外键 |
| TransportLeadTimeHours | 运输提前期 | INT NULL | 仅记录真实业务转运LT；禁止用统一2天默认值作为跨Domain/跨厂权威 |
| ETA | 预计到达时间 | DATETIME2 NULL | |
| KnownAvailableTime | 本次分配可用时间 | DATETIME2 NULL | 对应Supply Context中的`AvailableTime`：库存=当前可用；在途/采购/VMI按冻结ETA/DefaultLT规则归一；1号位不得自行重算 |
| CommitmentStatus | 供给承诺状态 | NVARCHAR(30) NULL | `COMMITTED / CONFIRMED / ESTIMATED / NOT_COMMITTED`等；正式物理供给按来源事实填写。Planning-only Purchase Placeholder仅在内存，不落本表 |
...
物理 Pegging 表（§5.1）是**1号位产出的FinalTask-to-FinalTask物理血缘**，记录 `UpstreamTaskId/DownstreamTaskId`。库存、Received、正式采购/VMI、厂间在途等非Task供给分配写入`PeggingSupplyAllocation`。Demand↔FinalTask真实数量归属写入`AllocationTaskShare`。V1需要逻辑Ledger能力，但**不强制新增独立PeggingAllocationLedger物理表**。
...
| UOM | 计量单位 | NVARCHAR(20) | PCS/KG/M等 | PCS |
| PeggingType | Pegging类型 | NVARCHAR(50) | BOM/ROUTING/MATERIAL_DEPENDENCY等物理依赖；`TRANSFER`仅历史兼容，不代表V1存在有限物流Task | BOM |
| LeadTimeDays | 历史提前期天数 | INT NULL | 兼容字段；不得再用固定2天作为跨Domain/跨厂权威。真实可用时间由Supply Context/Task时间和真实转运LT决定 | NULL |
| IsCrossDomain | 是否跨域 | BIT | 1=跨域，0=单域 | 0 |
| AllocatedQuantity | 分配数量 | DECIMAL(18,4) | 该Pegging分配的具体数量 | 500 |
| InheritedPriority | 历史继承优先级 | INT NULL | 历史兼容/解释字段；不得作为V1全局PriorityScore或排序权威 | NULL |
| AllocationReason | 分配理由摘要 | NVARCHAR(200) | 仅用于展示/追溯；正式排序、Lock、Explanation分别以对应规则/事实为准 | 需求规则段命中 |
| CreatedAt | 创建时间 | DATETIME2 | 记录创建时间 | 2026-03-05 02:05:00 |
...
| AllocatedQty | 已分配量（兼容快照字段） | DECIMAL(18,4) | **V1运行期不得由排程引擎永久UPDATE。** `sp_SyncInventorySnapshot`可初始化/重建为快照值（通常0）；本次ScheduleRun的真实分配与剩余量由2号位在内存`SupplyBalance`中维护 | 0 |
| AvailableQty | 快照可用量 | DECIMAL(18,4) | 计算列：`OnHandQty-AllocatedQty`；仅表达库存快照层可用量，不代表本次排程运行期扣减后的实时剩余量 | 1500 |
...
**V1正式来源**：ERP厂间在途、采购在途/未结PO、VMI、已到厂未入库等Timed Supply；由5号位/采购DBA通过正式ODS契约提供，2号位统一装载。  
**定位说明**：`ETA`保留来源预计时间；`AvailableTime`是APS统一可用时间。采购类优先采用人工ETA→ERP ETA→正式发行时间+DefaultPurchaseLT，并可叠加到厂→可用偏移；具体fallback由冻结规则/参数版本与5号位事实层提供，1号位不得自行重算。`SupplyAvailabilityRule`可作为兼容规则来源之一，但不是唯一权威。
...
| **AvailableTime** | **排程可用时间** | **DATETIME2** | **V1权威Timed Supply时间**；由来源ETA/人工ETA/DefaultLT/到厂可用偏移按冻结规则归一后落库，供2号位和1号位直接消费 | 2026-05-15 16:00:00 |
| CommitmentStatus | 供给承诺状态 | NVARCHAR(30) NULL | COMMITTED / CONFIRMED / ESTIMATED / NOT_COMMITTED等。真实PO/VMI/到厂事实按来源状态填写；**Planning-only Purchase Placeholder不落本表** | COMMITTED |
...
**消费口径（V1正式）**：
- 夜间FULL：按本次ScheduleRun/DataCutoff/Batch切片读取`IsActive=1`的真实Timed Supply；确定性承诺要求`AvailableTime`可用且CommitmentStatus满足业务资格。
- 白天Candidate：基于Base ACTIVE对应的有效物理Supply事实与当前真实消耗/Lock/执行事实构造竞争池，禁止全局粗暴读取所有历史`IsActive=1`记录。
- ETA缺失的正式PO可按冻结DefaultLT规则形成估算AvailableTime；若仍属非确定承诺，CTP必须明确标识。
- Planning-only Purchase Placeholder仅在本次运行内存生成，不写`SupplyFact_Pipeline`。

---

### 6.6a Planning-only Purchase Placeholder（规划性采购占位）🚫 **仅内存，不建表**

当库存、已到厂未入库、正式PO/采购在途、VMI均无可用供给时，2号位可为90天计划在当前运行内存生成估算供给。该对象**禁止落为已承诺Supply事实、禁止生成采购单、禁止生成Task、禁止下发ERP**。

最小内存字段语义：

| 字段 | 说明 |
|---|---|
| SupplyType | 固定`PLANNING_ONLY_PURCHASE_PLACEHOLDER` |
| MaterialId/MaterialCode | 缺口物料 |
| Quantity | 当前缺口 |
| AvailableTime | 计划基准时间 + DefaultPurchaseLT（及已冻结保守余量） |
| CommitmentStatus | `ESTIMATED / NOT_COMMITTED` |
| SourceSystem | `INTERNAL_ESTIMATE` |

**红线**：CTP若依赖该对象，只能返回“估算、非确定性承诺”；正式PO/VMI出现后，下一次排程自动使用真实Supply替换。**本文明确不新增`PlanningPurchasePlaceholder`物理表。**
...
**规则引擎说明**：本表不是“统一万能规则引擎总表”；V1规则/参数治理归3号位，5号位提供复杂事实，2号位执行最终供给资格与Allocation。本表仅是已有供给主题配置来源之一  
...
**v5.1.2 V1定位：兼容的供给资格/偏移规则来源，不是唯一管道规则平台。**
- V1正式读取真实厂间在途、采购/VMI/到厂未入库等Timed Supply。
- 若现有实现使用本表做资格/LeadTimeOffset，可继续复用；但人工ETA→ERP ETA→DefaultPurchaseLT等采购时间规则由3号位冻结参数与5号位事实层共同提供，不能被本表单一`ETA+Offset`公式垄断。
- 无匹配规则不得自动猜可用性；按冻结默认/保守规则处理并登记Issue。
- 映射失败（MaterialId/FactoryId无法解析）的行直接跳过，不进入排程。
- 不为了“规则统一”把本表扩展成DSL/脚本/万能Scope平台。
...
| FrozenDays | Frozen天数 | INT | 形成排程不可移动边界；新Run从上一ACTIVE读取仍有效锚点，不建设FrozenZoneSnapshot | 3 |
| FirmDays | Firm天数 | INT | 近端Firm计划锚点；调整须满足授权/业务规则，但**不强制依赖完整OA审批平台** | 2 |
...
**所属库**：APS_Production  
**业务用途**：作为1号位最终拆批/合批候选的参数来源之一（MOQ、EOQ、瓶颈倾向）。**不代表5号位提前生成TaskSplitVoucher，也不允许2号位在Solver前先拆最终Task。**
...
| 11 | MES_APS_Routing_Operation_View | 视图契约 | 工序节点 | 3号位/数据Owner |
| 12 | MES_APS_Routing_Dependency_View | 视图契约 | 工序依赖 | 3号位/数据Owner |
| 13 | APS_OperationResourceEligibility_View | 视图契约 | 工序资源资格 | 3号位/数据Owner |
| 14 | ERP_InterplantInTransit_View | 视图契约 | V1正式厂间在途Timed Supply | 5号位/ERP DBA |
| 15 | ERP_Received_ByDocument_View | 视图契约 | Received按单据汇总 | 5号位 |
...
| 3 | ext_MES_APS_Routing_Operation_View | 跨库包装视图 | 工序节点包装 | 2号位 |
| 4 | ext_MES_APS_Routing_Dependency_View | 跨库包装视图 | 工序依赖包装 | 2号位 |
| 5 | ext_APS_OperationResourceEligibility_View | 跨库包装视图 | 资源资格包装 | 2号位 |
| 6 | ext_PipelineSupply_Source_View | 统一输入视图 | V1正式Timed Supply统一入口 | 2号位 |
| 7 | APS_BOM_RAW | 表 | BOM原始数据缓存 | 2号位 |
| 8 | APS_ETL_Log | 表 | ETL日志 | 2号位 |
| 9 | ERP_Order_Staging | 表 | ERP订单同步临时表 | 2号位 |
| 10 | MaterialMapping | 表 | 物料映射表（SCD Type 2） | 2号位 |
| 11 | ProductionInstructionPositionSnapshot | 表 | PI互斥位置快照 | 5号位计算 / 2号位持久化 |
| 12 | DemandSupplyHardLock | 表 | Strict Binding / Demand Protection份额锁 | 2号位执行 / 3号位策略 |
| 13 | AllocationTaskShare | 表 | Demand Allocation↔FinalTask多对多份额 | 1号位输出 / 2号位持久化 |
| 14 | Task | 表 | FinalTask；Quantity/PlannedProcessQty双数量 | 1号位输出 / 2号位持久化 |
| 15 | PeggingSupplyAllocation | 表 | 非Task供给Allocation极简内部镜像 | 2号位 |
| 16 | SupplyFact_Pipeline | 表 | V1正式Timed Supply事实 | 2号位装载 |
| 17+ | ... | 表 | 其它主数据、运行、读模型等见正文 | 各号位 |
...
**PI PositionType**：`STAGE_WIP / XC / INTERPLANT_TRANSIT / WAITING_INBOUND / UNLOCATED`（可按真实业务追加受控值，但不得把Position变成独立Supply）。

**DemandSupplyHardLock.LockType**：`STRICT_BINDING / DEMAND_PROTECTION`。

**Timed Supply CommitmentStatus**：`COMMITTED / CONFIRMED / ESTIMATED / NOT_COMMITTED`（实际来源可做受控映射；CTP必须区分确定与非确定承诺）。

**TaskType（V1新记录）**：仅`PRODUCTION`；`TRANSFER/PROCUREMENT`历史兼容。
...
### 1号位（有限产能Solver/时间与资源真相）
- 重点关注：Task表、Pegging表、ExplainTrace表
- 技术要点：分区表、索引优化、内存管理

### 2号位（APS主流程/Pegging数量真相）
- 重点关注：所有表、视图契约、跨库访问
...
| OrderId | 订单ID | BIGINT NOT NULL | 本摘要按单一Order生成，逻辑关联Order.Id；与Task多Demand归属不是同一语义 | 100 |
...
**定位**：APS本地保存ODS MES工单/工序/大工艺累计进度事实。当前V1主要由5号位用于PI Position闭合、由2号位形成剩余生产需求与Execution约束；1号位消费的是已解析起点/剩余工艺/执行约束，**不再直接用原始StageProgress建立第二套PI位置真相**。该链独立于现货库存。

**V1 设计决策（不可随意修改）**：
- 不接 MES 每条报工明细，只接 ODS 汇总后的工序级/大工艺级进度
- 工序识别主字段 = `OperationName`（MES工序名称），不以 MES 工序编码为主
- Task/Pegging随新PlanVersion全量重算；MES进度不匹配历史TaskId，必须先按PI号/工序/Stage等稳定业务键解释成PI Position/Execution事实
- 生产进度快照绝对不写入 `InventoryBalance`

**负责人**：ODS 统一视图由5号位收口；APS本地快照同步（sp_Sync*）由2号位实现

**StageScopeType / RepWorksetId 与 MES 报工链路的设计边界**：
- `StageScopeType`（EDGE/ROOT）属于BOM侧`APS_BOM_STAGE_PATH_RAW`，不进入MES进度快照三表；5号位结合Stage路径与MES进度形成PI Position，2号位再将解析后的起点/剩余路径交给1号位；
...
- **V1不建**`ERP_APS_OrderStatus_View`：订单完工/取消仍由`Order_Canonical.Status`控制；MES快照只提供执行/位置事实，不替代订单状态管理，也不重新计算ERP PI总剩余边界。
...
**定位**：记录工序级生产进度事实。主要供5号位PI Position计算和Execution事实解析；2号位消费解析结果构造剩余生产需求，1号位不直接据此建立第二套PI位置真相。**汇总颗粒度**：生产指示号 + MES工单号 + 物料编码 + 工序名称 + 大工艺阶段码。
...
| RemainingQty | 剩余数量 | DECIMAL(18,4) | 计算列（PERSISTED）；作为进度事实辅助值，由5号位结合PI总量/XC/在途等做位置闭合；不得让1号位直接据此重算PI总剩余 | 40.0000 |
| LastReportTime | 最后报工时间 | DATETIME2 NULL | 该工序最后一次报工时间 | 2026-06-12 01:15:00 |
| SourceUpdatedAt | 源更新时间 | DATETIME2 NULL | MES 汇总数据最后更新时间 | 2026-06-12 01:20:00 |
...
**技术要点**：
- `RemainingQty`为持久化计算列，不需SP显式计算；**当前V1由5号位/2号位解释并形成PI Position/Execution约束，不再作为1号位直接扣Task权威**
- `ScrapQty` / `ReworkQty` 可选字段：ODS 来源能提供则填写，不能提供则保留 NULL，**不阻断快照同步**
- `OperationName`仍是MES工序识别主字段；但V1先由5号位/2号位把进度解释成PI Position与执行约束，再交1号位排未来剩余工序，避免直接用历史TaskId或原始报工建立第二套真相；
...
**定位**：记录大工艺级累计进度（工序进度上层汇总），是5号位PI Position闭合的重要输入。**不再定义“1号位优先消费StageProgress扣Task”**。汇总颗粒度：生产指示号 + 物料编码 + 大工艺阶段码。
...
| ScrapQty | 报废数量 | DECIMAL(18,4) NULL | 阶段累计报废数量（可选字段） | 2.0000 |
| ReworkQty | 返工数量 | DECIMAL(18,4) NULL | 阶段累计返工数量（可选字段） | NULL |
| RemainingQty | 剩余数量 | DECIMAL(18,4) | 计算列（PERSISTED）；仅是Stage累计事实的辅助剩余，不替代ERP PI总量边界；交5号位PI Position闭合 | 40.0000 |
| LastReportTime | 最后报工时间 | DATETIME2 NULL | 该阶段最后一次报工时间 | 2026-06-12 01:15:00 |
| SourceUpdatedAt | 源更新时间 | DATETIME2 NULL | MES 汇总数据最后更新时间 | 2026-06-12 01:20:00 |
...
**技术要点**：
- `RemainingQty`为持久化计算列，但不再建立`StageProgress > OperationProgress`给1号位直接扣Task的消费优先级。
- 同一PI的Stage/Operation进度若不一致，由5号位按下游已证明完成量、强位置事实等冻结规则保守闭合；中间Stage缺失/下游大于上游等异常由PI Position算法处理。
- 位置无法精确确定但总量可信时可形成UNLOCATED；若总量/位置份额无法闭合，则属于数量完整性错误，不得发布Domain。
...
> **v5.1.2 V1边界**：以下APS_Auth审批表可保留作为企业级可选流程/历史能力，但**Candidate正式采用不以完整多级OA审批为硬前提**。若2号位现有代码无完整审批主链，V1只要求最小人工授权确认并记录“谁/何时/采用哪个Candidate”。不得因为这些表已存在而反向把审批平台变成V1必做能力。
