...
**基于：DDL v5.2.5；相关文档：演变总表 v3.38 / 防腐层 v1.42 / 集成接口 v1.32 / 核心走查 V3.23；对齐：APS Pegging代码调整与接口冻结最终口径（2026-07-31）**  
**适用人员**：0号位（业务负责人）、1-5号位（开发团队）、DBA

**相关文档**：
- **《APS_各类基础数据分层承接与演变总表_v5.0》**（当前 v3.38）：数据演进全景图（架构总纲），建议先读此文档理解数据流向
- **《APS_数据架构与防腐层设计方案_v5.0》**（当前 v1.42）：防腐层设计、存储过程、数据管道详细说明
- **《APS_权限与审批系统实施方案_v1.0》**：权限与审批系统设计方案
- **《APS_Auth数据库字段说明文档_v1.0》**：APS_Auth数据库详细字段说明
- **《APS_Auth数据库DDL_v1.0.sql》**：APS_Auth独立物理部署脚本；不并入APS_Production主DDL

## v5.2.5 更新说明（2026-07-31 1号位纯内存接口与数据库隔离澄清）

- 🔒 `TaskDraft / FinalTaskDraft / ScheduledTaskDraft / AllocationShare`均为应用层内存对象，不是本数据库中的表或持久化实体。
- 🔒 Stage路径、Routing、进度快照、资源日历和参数表均由2号位查询并装入`ScheduleContext/DomainSolveRequest`；1号位不得直接读取任何数据库表。
- 🔒 1号位返回内存`FinalTaskDraft`及数量份额；正式`[Task]`、Ledger、PSA和物理Pegging只由2号位在统一Domain事务中写入。
- 📝 本版仅修改职责和字段消费说明；不新增、删除或调整任何表、字段、索引、约束及计算列。

## v5.2.4 更新说明（2026-07-31 库存净量、SupplyBusinessKey与统一持久化语义）

- 🔧 `ERP_Inventory_View.Quantity`定义为ODS已扣除`WasterQty`后的APS净可用量，视图字段集合不变；`InventoryFact_ERP`及后续库存层直接继承，不得再次扣减。
- 🔧 无法按客户区分数量的专属仓库由`InventoryAvailabilityRule`整体排除；不新增客户专属、质量等级或批次字段。
- 🔧 `SupplyBusinessKey`六类V1格式冻结，并明确DOC→普通INV转换边界。
- 🔧 `AllocationSequence`由每PlanVersion的Pegging局部计数器在成功双边扣减后生成。
- 🔧 `FinalTaskId/TaskComponentQty`在同一Domain事务中通过FinalTaskDraftKey映射后写入；不先插入半成品Ledger再逐行UPDATE。
- 🔧 ScheduleRunId、DomainKey和CanonicalId来源写死；缺失不得以0或最新记录兜底。
- 📌 本版没有新增表、列、索引或约束；DDL只更新库存包装视图显式列和契约注释。

## v5.2.3 更新说明（2026-07-30 定点补丁：MES快照服务边界与ExecutionLock数量修正）

- 🔧 三类MES运行快照由三个独立定时Job按同一`ScheduleRunId + DataCutoffTime`生成；`INightlyBatchOrchestrator`只创建运行记录，`ISchedulingOrchestrator`只校验和消费快照。
- 🔧 `ExecutionLock`删除人工短量差额字段；`RemainingExecutionQty`改为2号位维护的现实MES工单未来Stage产出承诺上限。
- 🔧 MES工序状态4只表示小工序执行记录人工完结；小工序剩余加工量以`OperationProgressSnapshot.RemainingQty`为权威，PI总剩余仍以`Order.Quantity - Order.ReceivedQty`为边界。
- 🔧 整张MES工单终结后，未完成且未正式取消的差额不留在ExecutionLock中，返回PI未承诺剩余池重新生成Task。


## v5.2.2 更新说明（2026-07-29 独立审计裁决与MES双向视图集成定点修正）

- 🆕 新增 `MESPlanRelease` 与 `APS_MES_PlanRelease_View`：APS按“同一生产指示 + 同一Stage + 同一执行批次”固化一张未来MES工单发布单元；MES拉取视图建单，`ReleaseItemKey`为跨系统唯一幂等键，`TaskNo`仅作展示和诊断；发布数量取Stage级执行批次单一流转量，禁止累加串行小工序Task数量。
- 🔄 `Task`新增 `MESPlanReleaseId`：发布后、MES尚未建单时Task保持`PLANNED`；MES实时工单视图回传`ReleaseItemKey + MESWorkOrderNo`后，2号位创建`ExecutionLock`并将相关Task转为`RELEASED`。
- ⚠️ `ExecutionLock`人工差额数量已由v5.2.3撤销；`MESPlanReleaseId`继续保留。MES工序状态4不得改变PI总剩余边界。
- 🔄 `DemandSupplyHardLock`允许部分解除：`ACTIVE`状态可同时存在部分`FulfilledQty`和部分`ReleasedQty`，只要`RemainingLockedQty > 0`，剩余数量继续保持硬归属。
- 🔒 MES→APS采用实时累计视图 + `ScheduleRun.DataCutoffTime`运行快照，不建设`MES_Actual_Staging`事件累计链；时间范围只用于缩小工作集，不用于由APS累计事件增量。
- 🔒 PI数量闭合改为“物理身份”和“需求归属”两个正交维度：ExecutionLock、PUBLISHED发布承诺、普通PI位置切片互斥表达物理数量；HardLock仅表达归属，不额外增加物理数量。
- 🧾 补齐审计发现的字段说明缺口：`Domain_Dependency`、急单三张桥接表、StageDetail归档变体、PlanVersion创建人字段及`Order_Canonical.MaterialCode`物理FK说明。
- 🗄️ 分区函数调整为每100个PlanVersion一个分区并预建至100000；补充ARCHIVED/FAILED/汇总/终态锁的建议保留周期。
- 🔐 明确APS_Auth由独立脚本`APS_Auth数据库DDL_v1.0.sql`部署，主DDL不重复创建权限库对象。

## v5.2.1 更新说明（2026-07-29 六份文档全局机械审计与场景反向审计修正）

- `Task`新增`ProductionInstructionNo`、`StageCode`；MES下发和ExecutionLock创建直接读取这两个字段。`MTS_InstructionNo`仅历史兼容。
- `PeggingAllocationLedger`新增`AllocationSequence`、`TaskComponentQty`；前者PlanVersion内唯一，后者是Task—需求组成份额，`vw_TaskDemandAllocation`只汇总该字段。
- `ExecutionLock`新增严格生命周期约束；`DemandSupplyHardLock`补RowVersion并收紧ACTIVE状态；`Task.IsLocked`明确只代表计划冻结。
- `PeggingSupplyAllocation`增加`UNIQUE(PlanVersionId, LedgerId)`；Ledger FinalTask建立复合FK到当前PlanVersion Task。
- ⚠️ 历史接口推送口径已由v5.2.2覆盖：APS不主动调用MES建单接口，改由MES读取发布视图；不存在HTTP部分接受语义。

**⚖️ 重要更新（v5.2.0）**（2026-07-29 Pegging软硬锁、PI位置快照与跨版本执行闭环）：
- 🆕 新增 `ProductionInstructionSupplySnapshot` / `ProductionInstructionPositionSlice`：按 `ScheduleRunId + DomainKey + ProductionInstructionNo` 保存PI总量边界与互斥Stage/XC/PIPELINE/UNLOCATED位置；`AvailableProductionQty=max(0,InstructionQty-ReceivedQty)`。
- 🆕 新增 `ExecutionLock`：以 `SourceSystem + MESWorkOrderNo` 为现实执行稳定锚点；`RELEASED`固定执行过程，但普通通用产出次日仍可重新Pegging。
- 🆕 新增 `DemandSupplyHardLock`：承载特殊出荷指示类型、客户专属、质量/环保资格、不可逆出货准备及人工审批等硬归属；与ExecutionLock严格分离。
- 🆕 新增 `PeggingAllocationLedger`：每个PlanVersion保存需求—供给数量血缘、SOFT/HARD、位置切片、逻辑块与最终Task组成份额。
- 🔧 补建 `PeggingSupplyAllocation` 正式DDL：只保存库存/在途/Received等非Task现实供给分配；物理`Pegging`继续仅Task-to-Task。
- 🔄 `Task`新增 `ExecutionLockId` / `IsVirtual`；`Task.OrderId`仅为代表订单，跨需求数量归属以Ledger和`vw_TaskDemandAllocation`为权威；虚拟Task不可下发MES。
- 🆕 新增 `vw_TaskDemandAllocation`；V1不建独立`SoftAllocation`、`DemandPromiseConstraint`、`ExecutionLockTaskLink`、`TaskDemandAllocation`物理表。
- 🔒 插件/Voucher瘦身：普通领域计算返回Result，供需建议返回Decision；仅人工审批或正式状态变化使用Voucher；V1不建设动态插件平台或完整事件溯源。
- 🔄 管道供给覆盖旧固定空跑口径：`ext_PipelineSupply_Source_View`统一厂间在途、采购在途、VMI、已到厂未入库；未接入来源允许0行，已启用来源由`sp_SyncPipelineSupply`按本次BatchNo真实装载；无主题规则时采用全局基础口径，明确排除规则优先。

**⚖️ 重要更新（v5.1.0）**（2026-07-13 白天实时评估与局部重排链路，对齐 DDL v5.1.0 / 防腐层 v1.34 / 演变总表 v3.30 / 集成接口 v1.24 / 核心走查 V3.15）：
- 🆕 **新增 MES_APS_BOM_Workset_CrossFactoryEdge_Realtime**：实时跨厂交接边表，业务语义与批量表 `MES_APS_BOM_Workset_CrossFactoryEdge` 对齐；使用 `RequestDetailId` 替代 `BatchNo` 作为隔离键；三个索引；0 行合法；只表达结构事实
- 🆕 **新增 sp_GenerateBOMCrossFactoryEdgeRealtime**：由 `sp_EnrichBOMWorksetRealtime` 在 Step 5 日志前调用；按 RequestDetailId 严格隔离；StageDict.FactoryCode 权威取值
- 🔄 **MES_API_BOM_Request_Realtime**：字段表按 DDL v5.1.0 完整落实：`Id / BOMNO / RequestDetailId(NULL) / OrderCanonicalId(NULL) / ResolvedBOMNO / RequestTime / Status / CompletedTime / ExpandedRowCount / ErrorMessage / RetryCount / Priority`；无物理 FK；READY 以 `Status='READY'` 为唯一权威；`ExpandedRowCount` 仅诊断；同一 RequestDetailId 存在多请求时按 `Id DESC` 取最新
- 🔄 **MES_APS_BOM_Workset_Issues.BatchNo 口径**：批量链路使用正式 BatchNo；实时链路正式路径 `RT:RD:{RequestDetailId}`；`RT:{ResolvedBOMNO}` 仅 deprecated 兼容
- 🔄 **CompletedTime vs CompletedAt**：仅 `MES_API_BOM_Request_Realtime.CompletedTime`；`MES_API_BOM_Request.CompletedAt`、`PlanVersion.CompletedAt`、`ScheduleRun.CompletedAt`、`SimulationRun.CompletedAt` 保持不变
...
- 【设计决策】2号位装载Stage路径时必须显式按`StageScopeType`区分（不得混查EDGE+ROOT），再将内存TaskDraft传给1号位
...
- PlanVersion新增CreatedByUserId + CreatedByUserName；`CreatedBy`继续作为NOT NULL创建来源/操作者字段保留
...
| RequestDetailId | 来源追溯锚点 | BIGINT | v5.0.21 逻辑关联 MES_API_BOM_Request_Detail.Id；数据库未建立物理外键；nullable；非业务键；1号位主链不消费；2/5号位追溯、回写、运营闭环用 | NULL |
...
- **2号位装载查询约定**：`WHERE ChildMaterialCode=@Mat AND StageScopeType='ROOT'`（根产品路径）；`WHERE ParentMaterialCode=@Parent AND ChildMaterialCode=@Child AND StageScopeType='EDGE'`（边级路径）。查询结果进入ScheduleContext/TaskDraft后再通过内存接口传给1号位。
...
|-----------|---------|---------|---------|--------|
| Id | 记录ID | BIGINT | 主键，自增 | 1 |
| BatchNo | 批次号 | NVARCHAR(50) | 批量链路=正式 BatchNo；实时链路正式路径 `RT:RD:{RequestDetailId}`；`RT:{ResolvedBOMNO}` 仅 deprecated 兼容（用 LEFT 截到 50 字符）| 20260305_000000 或 RT:RD:102385 |
...
**所属库**：MES_Integration  
**业务用途**：RequestDetail 驱动的订单级实时 BOM 展开请求；人工发起 CTP/插单影响分析/插单局部重排/人工局部重排后，2号位写入 RequestDetail，触发单请求级实时展开。

| 英文字段名 | 中文含义 | 数据类型 | 业务说明 | 示例值 |
|-----------|---------|---------|---------|--------|
| Id | 请求ID | BIGINT IDENTITY PK NOT NULL | 主键，自增 | 1 |
| BOMNO | BOM编号 | NVARCHAR(50) NOT NULL | 展开目标 BOMNO；无 RequestedBOMNO 场景由 `sp_ExpandBOMRealtime_vNext` 解析（如 `MAT:{MaterialCode}`） | BOM-10001 |
| RequestDetailId | 请求详情ID | BIGINT NULL | 逻辑关联 `MES_API_BOM_Request_Detail.Id`；订单级追溯锚点；DDL 无物理 FK；v5.1.0 正式路径必填，deprecated 兼容路径可空 | 102385 |
| OrderCanonicalId | 订单Canonical ID | BIGINT NULL | 逻辑关联 `Order_Canonical.Id`；DDL 无物理 FK | 200011 |
| ResolvedBOMNO | 解析后BOMNO | NVARCHAR(50) NULL | `sp_ExpandBOMRealtime_vNext` 解析结果，与 `Workset_Realtime.BOMNO` 用同一键过滤 | BOM-10001 |
| RequestTime | 请求时间 | DATETIME2 NOT NULL DEFAULT GETDATE() | 插入 PROCESSING 时的时间 | 2026-03-05 10:30:00 |
| Status | 状态 | NVARCHAR(20) NOT NULL DEFAULT 'PENDING' | PENDING / PROCESSING / READY / FAILED；**READY 为唯一权威完成信号（v5.1.0）** | READY |
| CompletedTime | 完成时间 | DATETIME2 NULL | Status 变为 READY 或 FAILED 时的时间；本表专有字段名，其它 Request 表使用 CompletedAt | 2026-03-05 10:30:05 |
| ExpandedRowCount | 展开后行数 | INT NULL | 当前 RequestDetailId 对应 `Workset_Realtime` 实际行数；**仅用于诊断，不参与 READY 判断** | 50 |
| ErrorMessage | 错误信息 | NVARCHAR(MAX) NULL | 失败时的错误描述 | NULL |
| RetryCount | 重试次数 | INT NOT NULL DEFAULT 0 | 失败后的重试次数 | 0 |
| Priority | 优先级 | INT NOT NULL DEFAULT 0 | 0=普通，1=紧急 | 1 |

**索引**：`IX_Realtime_Status (Status, Priority DESC, RequestTime)`

**技术要点**：
- **READY 权威口径**：`Status='READY'` 是完成的唯一权威。三张实时结果表（Workset_Realtime / StageDetail_Realtime / CrossFactoryEdge_Realtime）行数仅用于诊断，行数 = 0 可以是合法结果。
- **多请求最新记录**：同一 `RequestDetailId` 若失败重试产生多条记录，读取状态时按 `ORDER BY Id DESC` 取 TOP 1。
- **DDL 无物理 FK**：`RequestDetailId` 和 `OrderCanonicalId` 均为逻辑关联，允许 NULL；不新增物理外键。
- **性能目标**：单个 RequestDetail 级实时展开控制在 5 秒内。
- **优先级**：紧急插单优先处理。

---

### 2.1b MES_APS_BOM_Workset_CrossFactoryEdge_Realtime（实时跨厂交接边表）⭐ **v5.1.0 新增**

**所属库**：MES_Integration  
**业务用途**：实时 BOM 展开链路的跨厂交接边；由 `sp_GenerateBOMCrossFactoryEdgeRealtime` 在 `sp_EnrichBOMWorksetRealtime` Step 5 日志之前生成；**业务语义与批量表 `MES_APS_BOM_Workset_CrossFactoryEdge` 对齐；实时表使用 `RequestDetailId` 替代 `BatchNo` 作为隔离键**。

| 英文字段名 | 中文含义 | 数据类型 | 业务说明 |
|-----------|---------|---------|---------|
| Id | 主键 | BIGINT IDENTITY PK NOT NULL | 自增 |
| RequestDetailId | 请求详情ID | BIGINT NOT NULL | 实时请求隔离键；SP 按此字段幂等清理与写入 |
| WorksetId | Workset ID | BIGINT NOT NULL | 逻辑关联 `MES_APS_BOM_Workset_Realtime.Id`；DDL 无物理 FK |
| BOMNO | BOM编号 | NVARCHAR(50) NULL | 追溯 |
| ParentMaterialCode | 父件物料 | NVARCHAR(100) NOT NULL | 消耗方 |
| ChildMaterialCode | 子件物料 | NVARCHAR(100) NOT NULL | 被供给物料 |
| FromStageCode | 发出大工艺 | NVARCHAR(50) NOT NULL | 来自 `MES_APS_BOM_Workset_StageDetail_Realtime.StageCode`；`StageDict` 负责校验并提供对应 `FactoryCode`；禁止截取 StageCode 前缀推导工厂 |
| FromFactoryCode | 发出工厂 | NVARCHAR(50) NOT NULL | 来自 `StageDict.FactoryCode` |
| ToStageCode | 接收大工艺 | NVARCHAR(50) NOT NULL | 通过 `LEAD` 窗口函数取相邻阶段 |
| ToProcessCode | 接收工序/仓库码 | NVARCHAR(50) NULL | **V1 当前固定 NULL**；预留字段供 V2 扩展 |
| ToFactoryCode | 接收工厂 | NVARCHAR(50) NOT NULL | 来自 `StageDict.FactoryCode` |
| CreatedAt | 创建时间 | DATETIME2 NOT NULL DEFAULT GETDATE() | 系统时间 |

**索引**：
- `IX_CrossFactoryEdge_RT_RequestDetail (RequestDetailId)`
- `IX_CrossFactoryEdge_RT_Workset (WorksetId)`
- `IX_CrossFactoryEdge_RT_BOMNO (BOMNO)`

**职责与红线**：
- **5号位** 通过 `sp_GenerateBOMCrossFactoryEdgeRealtime` 生成结构事实；
- **2号位** 搬运至 APS 层作为 Pegging 输入；
- 只表达跨厂结构事实（`FromFactoryCode <> ToFactoryCode`），**不判断 STAGE_HANDOFF / INTER_FACTORY_ORDER**；
- **0 行合法**，`StageDict.StageCode` 未命中登记 `STAGE_DICT_NOT_FOUND WARN` 但不阻断；
- 不生成 Task、不写 Pegging、不修改 Order。
...
| ChildRequiredStageCode | 子件供给所需阶段码 | NVARCHAR(50) | v5.0.7 同Workset | TJ_OUTS |
| **ChildRequiredFactory** | **子件应归属账面工厂** | **NVARCHAR(20)** | **v5.0.10 同Workset** | **SH** |
| **RequestDetailId** | **来源追溯锚点** | **BIGINT NULL** | **v5.0.26 / v5.1.0**：实时插单来源追溯；逻辑关联 MES_API_BOM_Request_Detail.Id；DDL 无物理 FK | **102** |
...
SELECT 
    ...
    END AS Quantity,       -- APS净可用量；已经扣除WasterQty
    FactoryCode,
    SnapshotTime,
    IsActive
...
| Quantity | APS净可用库存数量 | DECIMAL(18,4) | `max(0, ERP总量-WasterQty)`；不等于ERP账面总量，WasterQty不向APS透传 | 1500 |
...
- `Quantity`从ODS开始即为净可用量；APS下游不得再次扣减WasterQty
- ERP不能按客户提供专属数量时，相关专属仓库由`InventoryAvailabilityRule`整体排除
...
**维护责任**：5号位（收口）  
**用途**：提供MES当前工单累计事实并回传APS发布幂等键；APS侧在`ScheduleRun.DataCutoffTime`一次性读取并同步到`MESWorkOrderSnapshot`，核心循环不得反复读取实时视图

| 字段名 | 中文名 | 数据类型 | 说明 | 示例 |
|--------|--------|----------|------|------|
| ReleaseItemKey | APS发布幂等键 | NVARCHAR(50) NULL | MES由APS发布视图建单时必须原样保存并回传；历史/外部工单可为空 | RLS:6F3A... |
...
> **契约红线**：视图返回每张工单的当前累计状态，不返回需要APS逐条累加的事件流。时间范围只用于筛选活跃PI、未终结工单或最近变化工作集；最终返回值仍须是当前累计事实。
...
**定位**：将各管道供给来源的 APS 单来源包装视图通过 `UNION ALL` 收敛为统一输入；`sp_SyncPipelineSupply` 只读取本视图，不直接读取各来源底表。  
**V1当前口径（v5.2.0覆盖旧空跑口径）**：来源按实际接入进度逐项启用；尚未接入的契约视图允许返回0行，但整个管道链不得再被固定为空。同步过程按本次 `BatchNo` 和 `DataCutoffTime` 装载，执行物料/工厂映射、稳定来源业务键去重及规则裁决，只替换本次批次切片，禁止 `TRUNCATE` 全表。  
**扩展方式**：新增来源时，在 ODS 层增加同构契约视图、在 APS 层增加单来源包装视图，再在本视图追加同构分支；Pegging主流程与同步过程主体不分叉。

**分支枚举**：
| 分支 | SourceSystem | SupplyType | APS包装视图 |
|------|-------------|------------|-------------|
| 1 厂间在途 | ERP | INTERPLANT_IN_TRANSIT | `ext_ERP_InterplantInTransit_View` |
| 2 采购在途 | PROCUREMENT | PURCHASE_IN_TRANSIT | `ext_ERP_PurchaseInTransit_View` |
| 3 VMI | VMI | VMI_ONSITE | `ext_ERP_VMI_View` |
| 4 已到厂未入库 | ERP | ARRIVED_NOT_RECEIVED | `ext_ERP_ArrivedNotReceived_View` |
...
**所属库**：APS_Production  
**业务用途**：从ODS库 MES_APS_BOM_Workset_StageDetail 拉取的本地缓存，由2号位查询并组装为内存Stage路径/TaskDraft后传给1号位排程；1号位不直接查询本表。含EDGE（子件供给路径）和ROOT（根产品完工路径）两类记录。
...
|-----------|---------|---------|---------|--------|
| Id | 参数ID | INT | 主键，自增 | 1 |
| ResourceId | 资源ID | INT | 逻辑关联 Resource.Id；数据库未建立物理外键 | 5 |
...
**所属库**：APS_Production  
**业务用途**：为无小工序的外协阶段、以及Routing数据不完整的阶段提供参数化提前期。2号位读取StageDetail阶段顺序并查询本表，形成内存标准TaskDraft后传给1号位；1号位不直接查询本表。
...
| MaterialCode | 物料编码 | NVARCHAR(100) | 物理FK→`Material.MaterialCode`。无法解析或尚未存在于Material的订单停留在`ERP_Order_Staging`并标记失败，不提升到Canonical | MAT-10001 |
...
| DomainKey | 分域标识 | NVARCHAR(100) | 产品族_工厂；对正式版本（DAILY_BASELINE / RESCHEDULE_CANDIDATE / LOCAL_RESCHEDULE_CANDIDATE）必填，必须有值 | X1_F001 |
...
| CreatedByUserId | 创建用户ID | INT | 发起排程的用户ID（逻辑关联APS_Auth.User表） | 5 |
| CreatedBy | 创建来源/操作者 | NVARCHAR(100) NOT NULL | DDL必填；可写SYSTEM、用户名或服务账号，是版本创建审计字段 | SYSTEM |
| CreatedByUserName | 创建用户名 | NVARCHAR(100) NULL | 用户显示名冗余；系统任务可为空 | 张三 |
...
> **DomainKey 业务必填与唯一性约束（v5.0.46，V1最终决策）**：
> - 对正式版本 `DAILY_BASELINE` / `RESCHEDULE_CANDIDATE` / `LOCAL_RESCHEDULE_CANDIDATE`，`DomainKey` 必须有值（不允许 NULL）。
> - 一个 `ScheduleRun` 可经 `PlanVersion.SourceScheduleRunId` 反查得到**多个** `PlanVersion`（按分域拆分，多域多版本）。
> - `(SourceScheduleRunId, DomainKey)` 唯一索引过滤条件为 `DomainKey IS NOT NULL AND SourceSimulationRunId IS NULL`：同一正式运行 + 同一非空分域至多一个版本；历史兼容的DomainKey=NULL行有意排除在索引之外，但创建/激活服务仍须拒绝新正式版本写NULL。
> - 同一 `Domain` 在生命周期中至多一个 `Status = ACTIVE` 的 `PlanVersion`（正式采用版本唯一）。
> - **V1 不强制改库 `NOT NULL`**，但创建 / 激活服务须校验 `DomainKey` 非空（正式版本缺 DomainKey 视为非法，拒绝创建 / 激活）。
...
> **生产指示数量边界（v5.2.0）**：当 `OrderType='PRODUCTION_INSTRUCTION'` 时，APS本次可承接总量统一为 `max(0, Quantity - ISNULL(ReceivedQty,0))`。`Quantity`是生产指示总量，`ReceivedQty`是最终目标M库累计入库量；MES在制、Stage等待、XC和在途只负责定位该总量，不得再次从总量中扣减。该计算值不在Order表重复落列，由 `ProductionInstructionSupplySnapshot.AvailableProductionQty` 固化每次运行结果。
...
| PlanVersionId | 计划版本ID | INT | 逻辑关联 PlanVersion.Id；数据库未建立物理外键 | 1 |
| TaskNo | 任务编号 | NVARCHAR(50) | 唯一标识 | TASK-20260305-000001 |
| OrderId | 订单ID | BIGINT | 逻辑关联 Order.Id；数据库未建立物理外键 | 100 |
| MaterialId | 物料ID | INT | 逻辑关联 Material.Id；数据库未建立物理外键 | 200 |
...
| IsLocked | 是否在计划冻结区 | BIT | 1=当前PlanVersion内排程冻结/人工锁定；不等于ExecutionLock，不自动形成HardLock | 1 |
| IsCriticalPath | 是否关键路径 | BIT | 1=关键路径 | 1 |
| TaskType | 任务类型 | NVARCHAR(50) | PRODUCTION/TRANSFER/PROCUREMENT | PRODUCTION |
| MTS_InstructionNo | 历史兼容生产指示字段 | NVARCHAR(50) | 从代表Order冗余；跨需求合并后不得作为执行PI权威 | PI-20260305-001 |
| **ProductionInstructionNo** | **正式执行生产指示号** | **NVARCHAR(100) NULL** | **Task实际由哪张PI承接。非虚拟生产Task进入MES发布单元前必须有值** | PI-1001 |
| **StageCode** | **Task所属大工艺阶段** | **NVARCHAR(50) NULL** | **MES发布单元分组、ExecutionLock创建和跨版本恢复直接使用** | CN_SURF |
| **MESPlanReleaseId** | **MES发布单元ID** | **BIGINT NULL** | **v5.2.2新增；FK→MESPlanRelease.Id。PUBLISHED但尚未被MES建单时跨版本保护该发布承诺；同一PI、同一Stage、同一执行批次的多个小工序Task可关联同一发布单元** | 501 |
| **ExecutionLockId** | **现实执行锁ID** | **BIGINT NULL** | **FK→ExecutionLock.Id。MES实时工单视图确认`ReleaseItemKey`后由2号位写入；不同PlanVersion中的新Task可通过同一ExecutionLockId连接同一现实MES工单** | 10001 |
| **IsVirtual** | **是否虚拟占位Task** | **BIT** | **1=没有正式生产指示的排程占位，只能PLANNED/CANCELLED，且`MESPlanReleaseId`与`ExecutionLockId`均必须为NULL，绝对禁止进入MES发布视图** | 0 |
| CreatedAt | 创建时间 | DATETIME2 | 记录创建时间 | 2026-03-05 02:05:00 |
| UpdatedAt | 更新时间 | DATETIME2 | 最后更新时间 | 2026-03-06 10:40:00 |

> **Task.Status 正式值域（v5.0.46，V1最终决策）**：`PLANNED` / `RELEASED` / `IN_PROGRESS` / `COMPLETED` / `CANCELLED`。
> - V1 **不支持** `PAUSED` / `SUSPENDED` / `WAITING` / `PENDING` / `RUNNING` 作为 Task 状态。
> - `RUNNING` 仅用于运行类对象（`ScheduleRun` / `SimulationRun` / `Scenario` / Hangfire 任务等），**不得**作为 `Task.Status`。
> - 若任何字段说明曾将 `WAITING` / `RUNNING` 等列为 Task 状态，一律以上述正式值域为准（本表已对齐）。

> **MES 工序报工五态（0-4）↔ APS Task.Status 映射（v5.0.46，V1最终决策）**：APS 不复制 MES 全部现场执行状态，仅映射为"生命周期 + 剩余量事实"。
>
> | MES状态 | 含义 | APS Task.Status | 处理 |
> |---|---|---|---|
> | 0 | 待开工 | PLANNED 或 RELEASED | 仅写入`MESPlanRelease(PUBLISHED)`但MES尚未建单时仍为PLANNED；MES工单视图已确认现实工单后为RELEASED |
> | 1 | 开工中 | IN_PROGRESS | 记录实际开工及进度 |
> | 2 | 完工报工 | COMPLETED | 记录完成数量和时间 |
> | 3 | 未完工报工 | IN_PROGRESS | 保留累计报工、计算 RemainingQty，工序未结案（状态3非完成） |
> | 4 | 未完工报工已完结 / 手动完工 | COMPLETED | 视为 COMPLETED，但必须保留"手动完工、数量不足"来源事实，不得伪装足量完工 |


> **Task归属与生成时机（v5.2.0）**：
> - `Task.OrderId`保持NOT NULL，但只作为代表订单/兼容查询入口，不再是跨需求合并后的唯一归属权威；
> - 真正的需求数量份额由 `PeggingAllocationLedger` 保存，并通过 `vw_TaskDemandAllocation`查询；
> - 正式Task在1号位完成有限产能合并/拆分与时间排定后，由2号位批量持久化；Pegging阶段只形成内存`LogicalBlock/TaskDraft`；正式生产Task必须写入`ProductionInstructionNo + StageCode`；
> - 已存在不同`ExecutionLockId`的现实MES工单，不得在新PlanVersion中合并成一张Task；一条`MESPlanRelease`可关联同一PI、同一Stage、同一执行批次下的多个小工序Task。
> - Task写入发布单元时保持`PLANNED`；MES实时工单视图回传`ReleaseItemKey + MESWorkOrderNo`后，由2号位原子创建ExecutionLock、将发布单元置`CONSUMED`并把相关Task转为`RELEASED`。

---

### 4.4a MESPlanRelease（MES拉取模式稳定发布快照）（v5.2.2新增）

**所属库**：APS_Production  
**定位**：APS与MES之间的稳定发布边界。一条记录对应一张未来MES现实工单，可关联同一生产指示、同一Stage、同一执行批次下的多个APS小工序Task。APS不主动调用MES建单接口，MES读取`APS_MES_PlanRelease_View`创建工单。

| 字段名 | 中文含义 | 数据类型 | 业务说明 |
|---|---|---|---|
| Id | 发布记录ID | BIGINT | 主键，自增 |
| ReleaseItemKey | 发布幂等键 | NVARCHAR(50) | 唯一；APS生成`RLS:{GUID}`并永久不变；MES必须原样保存并在工单视图回传 |
| SourcePlanVersionId | 来源计划版本ID | INT | FK→PlanVersion.Id；只记录首次形成发布单元的版本，不用于跨版本身份判断 |
| ProductionInstructionNo | 生产指示号 | NVARCHAR(100) | 一条发布单元只允许一个PI |
| MaterialId | 物料ID | INT | FK→Material.Id |
| FactoryId | 工厂ID | INT | FK→Factory.Id |
| StageCode | 大工艺阶段码 | NVARCHAR(50) | 一条发布单元只允许一个Stage |
| Quantity | 发布数量 | DECIMAL(18,4) | 大于0；取该Stage级执行批次的单一流转数量，MES按整条发布单元建单；禁止把关联NC/MC/精修等串行小工序Task.Quantity相加 |
| PlannedStartTime / PlannedEndTime | 计划起止时间 | DATETIME2 NULL | 结束时间不得早于开始时间 |
| PublishStatus | 发布状态 | NVARCHAR(20) | `PUBLISHED / CONSUMED / CANCELLED` |
| PublishedAt | 首次发布时间 | DATETIME2 | 对MES可见的时间 |
| ConsumedAt | MES承接时间 | DATETIME2 NULL | 工单视图确认`ReleaseItemKey`后写入 |
| MESWorkOrderNo | MES工单号 | NVARCHAR(100) NULL | `CONSUMED`时必填；随后由ExecutionLock承接现实执行 |
| CancelledAt | 发布取消时间 | DATETIME2 NULL | 仅MES尚未建单时允许取消 |
| CreatedBy | 创建人/来源 | NVARCHAR(100) NULL | SYSTEM或人工来源 |
| CreatedAt / UpdatedAt | 创建/更新时间 | DATETIME2 | |
| RowVersion | 行版本 | ROWVERSION | 并发校验 |

**数量组装规则**：同一发布单元关联的多个Task只是同一批流转数量在不同小工序上的排程表示。正常情况下它们应表达同一批次数量；存在工艺损耗或单位换算时，必须由配置规则显式换算到发布口径并通过闭合校验。不得使用`SUM(Task.Quantity)`生成发布数量。

**生命周期**：
- `PUBLISHED`：已向MES视图发布，尚未确认建单；`ConsumedAt/MESWorkOrderNo/CancelledAt`均为空。
- `CONSUMED`：MES已创建现实工单，`ConsumedAt/MESWorkOrderNo`必填；后续由ExecutionLock延续。
- `CANCELLED`：MES建单前取消，`CancelledAt`必填；不得再被MES创建工单。

**发布视图**：`APS_MES_PlanRelease_View`对MES公开`ReleaseItemKey、ProductionInstructionNo、StageCode、MaterialCode、FactoryCode、Quantity、计划时间、PublishStatus、PublishedAt、UpdatedAt`；`MaterialId/FactoryId`仅保留在APS内部表中，不作为跨系统契约字段。MES重复轮询不得重复建单，并须在真正落工单前再次确认该键仍为`PUBLISHED`。

---

### 4.4b ExecutionLock（跨版本现实执行锁）（v5.2.0新增，v5.2.2升级）

**所属库**：APS_Production  
**定位**：MES实时工单视图确认`ReleaseItemKey + MESWorkOrderNo`后形成的现实执行事实，以`SourceSystem + MESWorkOrderNo`为跨PlanVersion稳定锚点。它固定生产指示、Stage、MES工单及剩余执行量，但普通通用产出归属默认可在次日重新Pegging。

| 字段名 | 中文含义 | 数据类型 | 业务说明 |
|---|---|---|---|
| Id | 执行锁ID | BIGINT | 主键，自增 |
| ExecutionLockNo | 执行锁编号 | NVARCHAR(50) | APS稳定编号，唯一 |
| SourceSystem | 来源系统 | NVARCHAR(50) | 默认MES；与MESWorkOrderNo组成唯一键 |
| MESWorkOrderNo | MES工单号 | NVARCHAR(100) | 现实执行稳定锚点，跨PlanVersion不变 |
| MESPlanReleaseId | 来源MES发布单元ID | BIGINT NULL | FK→MESPlanRelease.Id；APS发布形成的工单必须关联，历史/外部MES工单可为空；一条发布单元最多形成一个ExecutionLock |
| ProductionInstructionNo | 生产指示号 | NVARCHAR(100) | 被MES现实工单执行的正式生产指示 |
| MaterialId | 物料ID | INT | FK→Material.Id |
| FactoryId | 工厂ID | INT | FK→Factory.Id |
| StageCode | 大工艺阶段码 | NVARCHAR(50) | 当前现实执行Stage；同一工单不得跨Stage漂移 |
| OriginalExecutionQty | 原始执行量 | DECIMAL(18,4) | MES现实工单最初承诺的Stage级执行数量 |
| CompletedQty | 累计良品完成量 | DECIMAL(18,4) | 以MES实时累计事实覆盖式更新，不按事件增量叠加 |
| CancelledQty | MES确认取消量 | DECIMAL(18,4) | 仅MES明确取消事实写入 |
| RemainingExecutionQty | 剩余执行量 | DECIMAL(18,4) | 2号位依据MES现实工单状态维护的未来Stage产出承诺上限；不是PI总剩余量，也不用于生成小工序Task |
| AvailableTime | 预计产出可用时间 | DATETIME2 NULL | 次日作为ExecutionLockedOutput供给的时间 |
| Status | 当前状态 | NVARCHAR(20) | ACTIVE/COMPLETED/CANCELLED |
| OriginalTriggerDemandOrderCanonicalId | 最初触发需求ID | BIGINT NULL | FK→Order_Canonical.Id；只追溯，不表示永久归属 |
| LastMESUpdatedAt | MES最后更新时间 | DATETIME2 NULL | Candidate现实校验及快照新鲜度依据 |
| CreatedAt / UpdatedAt | 创建/更新时间 | DATETIME2 | 当前权威状态时间 |
| RowVersion | 行版本 | ROWVERSION | 并发更新控制 |

**生命周期约束**：
- `ACTIVE`：`RemainingExecutionQty > 0`。
- `COMPLETED`：MES工单视图明确整张现实工单已经终结，`RemainingExecutionQty = 0`；允许`CompletedQty < OriginalExecutionQty`，未完成且未正式取消差额返回PI未承诺剩余池。
- `CANCELLED`：MES明确取消整张现实工单，`RemainingExecutionQty = 0`且`CancelledQty > 0`。

**数量红线**：`CompletedQty + CancelledQty + RemainingExecutionQty <= OriginalExecutionQty`。MES工序状态4只用于小工序进度解释，不直接关闭ExecutionLock；MES视图返回当前累计事实，APS按`ScheduleRun.DataCutoffTime`形成快照并覆盖更新，不建设事件累计链。

---

### 4.4c DemandSupplyHardLock（需求—供给硬归属锁）（v5.2.0新增，v5.2.2升级）

**所属库**：APS_Production  
**定位**：跨PlanVersion保存“某项供给的指定数量必须服务某个需求”。普通RELEASED任务不自动产生HardLock。HardLock表达需求归属，不是额外物理供给数量。

| 字段名 | 中文含义 | 数据类型 | 业务说明 |
|---|---|---|---|
| Id | 硬锁ID | BIGINT | 主键，自增 |
| HardLockNo | 硬锁编号 | NVARCHAR(50) | 唯一业务编号 |
| DemandOrderCanonicalId | 指定需求ID | BIGINT | FK→Order_Canonical.Id |
| SupplyType | 供给类型 | NVARCHAR(50) | EXECUTION_LOCKED_OUTPUT/PIPELINE/INVENTORY/SHIPPING_INSTRUCTION等 |
| SupplyBusinessKey | 供给稳定业务键 | NVARCHAR(300) | 不依赖TaskId/PlanVersion。Pipeline与Received保留单据身份时使用同一DOC键；进入普通池化库存并失去单据身份后使用INV键；严格绑定数量不得提前丢失DOC身份。PI位置与ExecutionLockedOutput沿用PI键 |
| ExecutionLockId | 执行锁ID | BIGINT NULL | 供给来自现实MES工单时FK→ExecutionLock.Id |
| LockedQty | 原始硬归属数量 | DECIMAL(18,4) | 必须大于0，建立后不因部分履约而改写 |
| FulfilledQty | 已履约数量 | DECIMAL(18,4) | 已被原需求不可逆消耗/履约的累计数量，幂等更新 |
| ReleasedQty | 已正式释放数量 | DECIMAL(18,4) | 经规则或审批解除、可重新竞争的累计未消费数量；允许部分释放 |
| RemainingLockedQty | 当前剩余硬锁量 | DECIMAL(18,4) PERSISTED | `max(0,LockedQty-FulfilledQty-ReleasedQty)`；夜间和Candidate只恢复此数量 |
| LockReason | 硬锁原因 | NVARCHAR(100) | SPECIAL_SHIPMENT_INSTRUCTION_TYPE/CUSTOMER_EXCLUSIVE/QUALITY_QUALIFICATION/MANUAL_APPROVAL等 |
| RuleSetVersionId | 规则集版本ID | BIGINT NULL | FK→RuleSetVersion.Id；人工锁可为空 |
| EffectiveAt | 生效时间 | DATETIME2 | 默认当前时间 |
| ReleasedAt | 首次/最近释放时间 | DATETIME2 NULL | `ReleasedQty>0`时必须有值 |
| CompletedAt | 履约完成时间 | DATETIME2 NULL | COMPLETED时写入 |
| Status | 状态 | NVARCHAR(20) | ACTIVE/COMPLETED/RELEASED/CANCELLED |
| CreatedBy / ReleasedBy | 创建/解除人 | NVARCHAR(100) NULL | 审计追溯 |
| CreatedAt / UpdatedAt | 创建/更新时间 | DATETIME2 | |
| RowVersion | 行版本 | ROWVERSION | Candidate激活前并发校验 |

**数量生命周期**：`FulfilledQty + ReleasedQty <= LockedQty`。`FulfilledQty`由2号位依据截至运行截止时间的累计履约/不可逆消耗事实覆盖式、单调更新，同一事实重读不得重复累计；`ReleasedQty`只来自规则或审批确认的未履约释放量。`ACTIVE`允许部分履约与部分解除，只要`RemainingLockedQty>0`，剩余数量继续硬锁；全部履约进入`COMPLETED`；剩余量全部释放后进入`RELEASED`或`CANCELLED`。已履约数量不得再次释放。

**供给总量红线**：同一物理供给的所有HardLock剩余量与SOFT分配合计不得超过真实物理数量。ExecutionLock、PUBLISHED发布承诺、普通PI位置切片属于互斥物理身份；HardLock只是这些物理供给内部的归属属性，不得再次相加形成新供给。
...
| Status | 运行状态 | NVARCHAR(20) | `RUNNING` / `COMPLETED` / `PARTIAL_SUCCESS` / `FAILED`（运行状态，非版本状态） | COMPLETED |
| ScenarioId | 关联场景ID | INT NULL | 逻辑关联 Scenario.Id；数据库未建立物理外键，由应用服务校验引用有效性；仅 SIMULATION/INSERT_ORDER_WHATIF 类型填充 | NULL |
| BasePlanVersionId | 基准版本ID | INT NULL | 逻辑关联 PlanVersion.Id，表示白天 Candidate 所基于的当前 ACTIVE 版本；数据库未建立物理外键，由创建服务校验版本存在、状态和 Domain 一致性；凌晨全量为 NULL | NULL |
...
| ScopeJson | 重排范围JSON | NVARCHAR(MAX) NULL | **固定 11 字段**：`Purpose` / `OrderCanonicalIds` / `FactoryIds` / `ProductFamilyIds` / `ResourceGroupIds` / `PlanHorizonStart` / `PlanHorizonEnd` / `LockedTaskIds` / `AllowTouchFrozenZone` / `AllowDelaySalesOrder` / `MaxImpactedOrders`；局部重排时记录范围；**⚠️ 不含 ExpectedDomainKeys（预期域集合归独立字段 ExpectedDomainKeysJson，见下）** | NULL |
| ExpectedDomainKeysJson | 预期域集合JSON | NVARCHAR(MAX) NULL | 运行启动时冻结的预期 DomainKey 集合（JSON 数组）；是 ScheduleRun 终态判定的**唯一权威来源**；创建后只读、不可修改；**不得**根据已创建的 PlanVersion 反推；**独立字段，不属于 ScopeJson**。各 RunType 数组规则：FULL_SCHEDULE 必须非空 JSON 数组（≥1 个不重复 DomainKey，此时 ScopeJson 可为 NULL）；白天 Candidate（LOCAL_RESCHEDULE / MANUAL_RESCHEDULE / INSERT_ORDER_WHATIF）**恰好 1 个元素 = BasePlanVersion.DomainKey**；SIMULATION 阶段二骨架可用独立 ExpectedDomainKeysJson（1 或多个） | ["DOMAIN_A"] |
...
> **ScheduleRun.Status 枚举定义（v5.0.46，V1最终决策）**："本次运行计划覆盖的分域清单"= `ExpectedDomainKeysJson`（运行启动时冻结、创建后只读的独立字段，唯一权威来源，不得反推）。
> - `RUNNING`：仍有预期 Domain 未进入终态（计算 / 落盘 / 对应 PlanVersion 进入运行成功终态——夜间 `FULL_SCHEDULE` = `ACTIVE`，白天 Candidate = `CANDIDATE`——尚未全部完成）。
> - `COMPLETED`：所有 ExpectedDomainKeys（本次运行计划覆盖的分域清单）对应 Domain 均"计算成功 + 落盘成功 + 对应 PlanVersion 进入运行成功终态"（夜间 `FULL_SCHEDULE` = `ACTIVE`；白天 Candidate = `CANDIDATE`；完整 RunType 状态矩阵见防腐层 §2.8.7）。
> - `PARTIAL_SUCCESS`：至少一域成功发布（PlanVersion `ACTIVE` 且落盘成功），且至少一域失败 / 未启动 / 未完成。**⚠️ 本状态主要用于多预期 Domain 的 `FULL_SCHEDULE` 运行；白天单 Domain Candidate 运行（`ExpectedDomainKeysJson` 恰好 1 项）正常只有 `COMPLETED` / `FAILED`，不产生 `PARTIAL_SUCCESS`**。
> - `FAILED`：运行级致命错误（进程崩溃、数据切片不可用等），或零成功（没有任何 Domain 成功发布）。
> - `CompletedAt` 在所有终态（含 `PARTIAL_SUCCESS`）均写入。
...
**所属库**：APS_Production  
**定位**：记录一个业务试算场景的假设、目标和最终选中版本。不是运行表，不是结果版本表。**V1 白天实时评估中的正式业务容器**，业务适用时创建（`INSERT_ORDER_WHATIF` 可使用 Scenario 保存假设和目标）；阶段二另含 `SimulationRun`（仿真算法执行骨架）与 `ScenarioObjectiveScore`（多方案评分骨架）。
...
| SelectedPlanVersionId | 选中版本ID | INT NULL | 逻辑关联 PlanVersion.Id；数据库未建立物理外键，用于记录人工选中的候选版本；候选版本生成后回填；可空 | NULL |
...
| ScenarioId | 关联场景ID | INT | 逻辑关联 Scenario.Id；阶段二骨架字段，当前未建立物理外键 | 1 |
| ScheduleRunId | 关联排程运行ID | INT NULL | 逻辑关联 ScheduleRun.Id；阶段二骨架字段，当前未建立物理外键 | 5 |
...
|-----------|---------|---------|---------|--------|
| Id | 评分ID | INT | 主键，自增 | 1 |
| ScenarioId | 关联场景ID | INT | 逻辑关联 Scenario.Id；当前未建立物理外键 | 1 |
| PlanVersionId | 关联版本ID | INT | 逻辑关联 PlanVersion.Id；当前未建立物理外键 | 2 |
...
| 5号位 | 少数策略接口与普通领域计算模块；只消费已发布规则/参数，返回Result或PeggingAllocationDecision，不直接扣余额、不写Ledger/Task/Pegging；只有人工审批或正式状态变化才生成Voucher；V1不建设动态插件平台 |
| 2号位 | 稳定运行内核与唯一余额修改者；数据装载、PI快照、供需余额、Ledger原子执行、TaskDraft构造、正式Task/Pegging批量持久化及运行/版本状态回填（白天Candidate版本壳仍由3号位创建） |
| 1号位 | 纯内存排程算法消费方，只通过2号位方法参数接收已装载的TaskDraft、规则结果、参数、资源和约束；禁止读取或写入任何数据库 |
...
| WorksetId | Workset行ID | BIGINT | 逻辑关联 MES_APS_BOM_Workset.Id；数据库未建立物理外键 |
...
### 5.0-0 SupplyBusinessKey统一格式（v5.2.4）
...
字符串Trim、英文代码大写、`|`分隔；禁止包含数量、时间、TaskId、PlanVersionId或数据库自增Id。由来源Loader生成，Ledger/PSA/HardLock只保存和复制最终键。

### 5.0 PeggingAllocationLedger（需求—供给数量血缘权威账本）（v5.2.0新增）

**所属库**：APS_Production  
**定位**：每一笔成功的需求—供给分配均按PlanVersion持久化；它是数量闭合、软硬归属、Task组成份额、Candidate释放及解释回放的唯一权威账本。分配时必须原子完成“扣需求余额、扣供给余额、追加Ledger”。

| 字段名 | 中文含义 | 数据类型 | 业务说明 |
|---|---|---|---|
| Id | Ledger行ID | BIGINT | 与PlanVersionId组成分区主键 |
| PlanVersionId | 计划版本ID | INT | 分区键 |
| ScheduleRunId | 排程运行ID | INT | FK→ScheduleRun.Id |
| DomainKey | 分域键 | NVARCHAR(100) | 当前单域计算上下文 |
| AllocationSequence | 分配序号 | BIGINT | 每PlanVersion的一次Pegging调用局部递增；只有需求和供给余额均扣减成功后从1生成；不使用数据库Sequence，不在INSERT阶段才生成 |
| FlowNodeCode | 流程节点码 | NVARCHAR(100) NULL | DFS/跨厂/孤儿扫尾等节点追溯 |
| RootDemandOrderCanonicalId | 根需求ID | BIGINT | FK→Order_Canonical.Id |
| DemandOrderCanonicalId | 当前需求ID | BIGINT | FK→Order_Canonical.Id |
| DemandOrderId | 当前版本订单ID | BIGINT NULL | 逻辑关联Order.Id |
| DemandMaterialId | 需求物料ID | INT | FK→Material.Id |
| RequiredStageCode | 需求阶段 | NVARCHAR(50) NULL | 当前阶段净需求 |
| SupplyType | 供给类型 | NVARCHAR(50) | INVENTORY/PIPELINE/PRODUCTION_INSTRUCTION/EXECUTION_LOCKED_OUTPUT等 |
| SupplyBusinessKey | 供给稳定业务键 | NVARCHAR(300) | 按§5.0-0格式由Loader生成；不使用TaskId/PlanVersionId代替 |
| ParentProductionInstructionNo | 承接生产指示号 | NVARCHAR(100) NULL | PI供给时填写 |
| PositionSliceId | PI位置切片ID | BIGINT NULL | FK→ProductionInstructionPositionSlice.Id |
| PositionSliceKey | 位置切片键 | NVARCHAR(100) NULL | 便捷追溯 |
| AllocatedQty | 分配数量 | DECIMAL(18,4) | 必须大于0 |
| UOM | 计量单位 | NVARCHAR(20) | |
| AvailableTime | 供给可用时间 | DATETIME2 NULL | Pegging不反算目标时间 |
| PriorityRank / PriorityScore | 优先级序位/分数 | INT / DECIMAL(18,6) NULL | 当前规则版本结果 |
| PriorityReason | 优先级原因 | NVARCHAR(500) NULL | |
| RuleSetVersionId | 规则集版本ID | BIGINT NULL | FK→RuleSetVersion.Id |
| AllocationMode | 归属模式 | NVARCHAR(10) | SOFT/HARD |
| HardLockId | 硬锁ID | BIGINT NULL | HARD时必须有值；FK→DemandSupplyHardLock.Id |
| ExecutionLockId | 执行锁ID | BIGINT NULL | 供给/需求来自现实执行时FK→ExecutionLock.Id |
| LogicalBlockKey | 逻辑块键 | NVARCHAR(100) NULL | TaskDraft形成前的聚合锚点 |
| FinalTaskId | 最终Task ID | BIGINT NULL | 1号位排程后由2号位回填；与PlanVersionId复合FK→Task |
| TaskComponentQty | Task需求组成份额 | DECIMAL(18,4) NULL | ComponentShares回填；只用于Task—需求数量视图，不等于AllocatedQty |
| SourceRowKey / SourceDocumentNo | 来源行键/单据号 | NVARCHAR(500)/NVARCHAR(100) NULL | 非Task现实供给追溯 |
| CreatedAt | 创建时间 | DATETIME2 | |

**V1瘦身**：不另建`SoftAllocation`表；SOFT/HARD均由本表`AllocationMode`表达。不另建`TaskDemandAllocation`物理表。`FinalTaskId`与`TaskComponentQty`必须同时为空或同时有值。

---

**Ledger写入语义（v5.2.4）**：
- Pegging阶段只形成内存`PeggingLedgerEntry`；
- 1号位返回`AllocationSequence→FinalTaskDraftKey→ComponentQty`；
- 同一Domain事务先写Task并建立DraftKey→TaskId映射，再写带完整`FinalTaskId/TaskComponentQty`的Ledger；
- 不先插入`FinalTaskId=NULL`的半成品Ledger再逐行UPDATE；
- `ScheduleRunId=PlanVersion.SourceScheduleRunId`，`DomainKey=PlanVersion.DomainKey`，CanonicalId来自`Order.OrderCanonicalId`，缺失即失败。

### 5.0a PeggingSupplyAllocation（非Task现实供给分配结果）（v5.2.0补建正式DDL）

**定位**：只记录库存、XC、厂间/采购在途、VMI、已到厂未入库、Received等已经确认用于需求的非Task供给；候选但未使用供给不得写入。每行必须关联一条`PeggingAllocationLedger`，且`PlanVersionId + LedgerId`唯一，一条Ledger不得重复物化。

| 字段名 | 中文含义 | 数据类型 | 业务说明 |
|---|---|---|---|
| Id | 主键 | BIGINT | 与PlanVersionId组成分区主键 |
| PlanVersionId | 计划版本ID | INT | 分区键 |
| ScheduleRunId | 排程运行ID | INT | FK→ScheduleRun.Id |
| LedgerId | Ledger行ID | BIGINT | 与PlanVersionId复合FK→PeggingAllocationLedger |
| BatchNo | 数据批次号 | NVARCHAR(50) NULL | |
| RootOrderCanonicalId | 根需求ID | BIGINT | FK→Order_Canonical.Id |
| DemandOrderCanonicalId | 当前需求ID | BIGINT | FK→Order_Canonical.Id |
| RootOrderId / CurrentOrderId | 当前版本订单ID | BIGINT NULL | 逻辑关联Order.Id |
| RootOrderNo / CurrentOrderNo | 单据号 | NVARCHAR(100) NULL | |
| OrderType | 订单类型 | NVARCHAR(50) NULL | |
| WorksetId | BOM工作集行ID | BIGINT NULL | 逻辑关联 |
| MaterialId / MaterialCode | 物料ID/编码 | INT / NVARCHAR(100) | |
| DemandFactoryCode / DemandStageCode | 需求工厂/阶段 | NVARCHAR(50) NULL | |
| DemandQty / AllocatedQty | 需求量/本条分配量 | DECIMAL(18,4) | `0 < AllocatedQty <= DemandQty` |
| AllocationMode | 归属模式 | NVARCHAR(10) | SOFT/HARD |
| HardLockId / ExecutionLockId | 硬锁/执行锁ID | BIGINT NULL | 适用时关联 |
| SupplyType | 供给类型 | NVARCHAR(50) | INVENTORY/XC/PIPELINE/RECEIVED等 |
| SupplyBusinessKey | 供给稳定业务键 | NVARCHAR(300) | 必填；防重复使用的现实键 |
| SupplyFactoryCode / SupplyWarehouseCode | 供给工厂/仓库 | NVARCHAR(50) NULL | |
| ERPProperty | ERP位置属性 | NVARCHAR(20) NULL | M/XC/ZP/BP |
| AttachStageCode / CompletedStageCode / NextRequiredStageCode | 阶段定位 | NVARCHAR(50) NULL | |
| RemainingStagePathJson | 剩余Stage路径 | NVARCHAR(MAX) NULL | JSON |
| SupplyMode | 跨厂/供给模式 | NVARCHAR(50) NULL | STAGE_HANDOFF/INTER_FACTORY_ORDER等 |
| CrossFactoryEdgeId | 跨厂边ID | BIGINT NULL | 逻辑关联 |
| PositionSliceId | PI位置切片ID | BIGINT NULL | FK→ProductionInstructionPositionSlice.Id |
| InventoryAvailableSupplyDetailId | 库存明细ID | BIGINT NULL | 逻辑关联；来源表会TRUNCATE，不建FK |
| PipelineSupplyId | 管道供给ID | BIGINT NULL | 逻辑关联SupplyFact_Pipeline.Id |
| TransportLeadTimeHours | 运输提前期 | INT NULL | |
| ETA / KnownAvailableTime | 预计到达/已知可用时间 | DATETIME2 NULL | |
| SupplyDocumentType / SupplyDocumentNo | 供给单据类型/号 | NVARCHAR(50)/NVARCHAR(100) NULL | |
| SourceSystem / SourceRowKey | 来源系统/稳定行键 | NVARCHAR(50)/NVARCHAR(500) NULL | |
| CreatedAt | 创建时间 | DATETIME2 | |

---

### 5.0b vw_TaskDemandAllocation（Task跨需求数量份额视图）（v5.2.0新增）

基于`PeggingAllocationLedger.FinalTaskId`聚合`TaskComponentQty`，输出`PlanVersionId + TaskId + RootDemandOrderCanonicalId + DemandOrderCanonicalId + AllocationMode + HardLockId + ExecutionLockId + TaskComponentQty`。它是V1 Task多需求查询入口，不形成第二套数量真相；性能不足时才能评估物化。

---

### Pegging 表定位澄清

物理 Pegging 表（§5.1）是 Task-to-Task 供需血缘表，记录 `UpstreamTaskId/DownstreamTaskId`。库存、ZP/BP 出口库、Received 汇总、管道在途等非Task供给先写`PeggingAllocationLedger`，再生成`PeggingSupplyAllocation`；物理Pegging表不承接这些供给。
...
| PlanVersionId | 计划版本ID | INT | 逻辑关联 PlanVersion.Id；数据库未建立物理外键 | 1 |
| TaskId | 任务ID | BIGINT | 逻辑关联 Task.Id；数据库未建立物理外键 | 100 |
...
**所属库**：APS_Production  
**业务用途**：记录从APS专用ODS契约视图同步的ERP净可用库存事实。ODS已完成`总量-WasterQty`净额化，APS表本身不再二次处理  
...
| Quantity | ERP净可用库存数量 | DECIMAL(18,4) | 直接继承`ext_ERP_Inventory_View.Quantity`；已扣除WasterQty，不等于ERP账面总量 | 1000 |
...
| Quantity | 候选库存数量 | DECIMAL(18,4) | ERP来源已是ODS净可用量；不得再次扣减WasterQty | 1500 |
...
| `Quantity` | 当前明细可用数量 | DECIMAL(18,4) | 规则命中后的净可用数量；ERP来源已在ODS扣除WasterQty |
...
- ERP来源`OnHandQty`已经是ODS净可用量，后续不得再次扣减WasterQty
- 无法按客户区分数量的专属仓库必须配置为不可用规则并整体排除
...
| AllocatedQty | 已分配量（兼容列） | DECIMAL(18,4) | V1快照同步固定写0，排程引擎不回写本表；实际分配只进入内存余额、PeggingAllocationLedger和PeggingSupplyAllocation | 0 |
| AvailableQty | 可用量 | DECIMAL(18,4) | 计算列：OnHandQty-AllocatedQty；V1因AllocatedQty固定0，等于本批次可用库存快照总量 | 1500 |
...
> **V1余额边界**：`InventoryBalance`是运行前可用库存汇总快照，不是跨版本占用账。`AllocatedQty`在V1固定为0，禁止由排程循环更新；分配与占用以当前Domain内存余额、Ledger及非Task分配细账为权威。
...
### 6.5a 管道供给统一输入契约（v5.2.0当前权威）

| 来源 | ODS契约视图 | APS包装视图 | 当前接入方式 |
|---|---|---|---|
| 厂间在途 | ERP_InterplantInTransit_View | ext_ERP_InterplantInTransit_View | 已建契约；源端可按实际数据启用 |
| 采购在途 | ERP_PurchaseInTransit_View | ext_ERP_PurchaseInTransit_View | 同构0行占位，启用时只替换ODS内部实现 |
| VMI | ERP_VMI_View | ext_ERP_VMI_View | 同构0行占位 |
| 已到厂未入库 | ERP_ArrivedNotReceived_View | ext_ERP_ArrivedNotReceived_View | 同构0行占位 |

四类包装视图由`ext_PipelineSupply_Source_View`以`UNION ALL`收敛为15列统一输入（14业务字段+APS派生`SourceSystem`）。未来新增来源只增加同构分支，不修改Pegging主流程。
...
**当前统一来源**：厂间在途、采购在途、VMI、已到厂未入库四类同构契约；尚未接入的单一来源视图允许返回0行，但整个管道主链不得固定空跑  
...
**消费口径**（v5.2.0当前V1）：
- 夜间快照：`WHERE BatchNo = @CurrentBatchNo AND IsActive = 1`；`AvailableTime IS NULL`记录可用于数量存在性和异常解释，但不得作为有确定时间的供给解除纳期风险
- 白天Candidate：读取`BasePlanVersion.SourceScheduleRunId`对应的准确BatchNo切片，不全局读取最新IsActive记录
...
**规则引擎说明**：本表不是统一万能规则引擎；由少数策略接口消费。来源自身硬约束先校验，本表只做可用性例外覆盖和LeadTimeOffset  
...
**v5.2.0当前状态：统一主链按来源启用**：
- `sp_SyncPipelineSupply`读取`ext_PipelineSupply_Source_View`，按`DataCutoffTime`截断、物料/工厂映射、来源业务键去重，并仅替换本次`BatchNo`切片；禁止TRUNCATE全表。
- 厂间在途、采购在途、VMI、已到厂未入库分别保留同构ODS契约；单一未接入来源返回0行不阻断其他来源。
- 来源硬约束、MaterialId/FactoryId映射失败行先排除；规则匹配只处理例外纳入/排除及LeadTimeOffset。
- 无命中规则时采用全局基础口径：默认纳入、LeadTimeOffset=0；命中`IncludeFlag=0`的明确排除规则时不写入。
- 同一五维组合同时只允许一条`IsActive=1`规则；新规则发布须先停用旧规则，规则有效期按`DataCutoffTime`判断。
...
| Id | 规则ID | INT | 主键，自增 | 1 |
| MaterialId | 物料ID | INT | 外键关联Material表 | 100 |
| ResourceGroupId | 资源组ID | INT | 逻辑关联 ResourceGroup.Id；数据库未建立物理外键（v5.0 已废弃 ResourceGroup，仅兼容保留） | 10 |
...
| 6b | MES_APS_BOM_Workset_CrossFactoryEdge_Realtime | 表 | 实时BOM跨厂结构边（v5.1.0 新增） | 5号位生成 / 2号位搬运 |
...
**文档版本历史**：
- v5.2.0（2026-07-29）：Pegging软硬锁、PI位置快照、Ledger、ExecutionLock/HardLock、Task跨需求份额与非Task供给分配闭环
...
| PlanVersionId | 计划版本ID | INT NOT NULL | 逻辑关联 PlanVersion 表（不建物理FK）；同时作为结果版本隔离分区键（保持 NOT NULL） | 1 |
| ScheduleRunId | 排程运行ID | INT NULL | 外键关联 ScheduleRun 表（FK→ScheduleRun.Id）；本次原因事实归属的运行；索引 IX_SEF_ScheduleRun(ScheduleRunId, ReasonCode) | 1 |
| ObjectType | 对象类型 | NVARCHAR(50) | `ORDER`（订单）/ `TASK`（任务）/ `RESOURCE`（资源）/ `STAGE`（阶段）/ `DOMAIN`（分域，用于跨域版本不一致风险） | ORDER |
| OrderId | 订单ID | INT | 逻辑关联 Order 表（不建物理FK）；ObjectType=ORDER/TASK 时填充 | 100 |
| TaskId | 任务ID | INT | 逻辑关联 Task 表（不建物理FK）；ObjectType=TASK 时填充 | 200 |
| ResourceId | 资源ID | INT | 逻辑关联 Resource 表（不建物理FK）；ObjectType=RESOURCE/TASK 时填充 | 5 |
| StageCode | 阶段代码 | NVARCHAR(50) | 相关阶段代码；ObjectType=STAGE 时填充 | ASSEMBLE |
| ReasonCode | 原因代码 | NVARCHAR(100) | **权威枚举（全文 ReasonCode 必须属于此列表，0号位审批冻结）**：`RESOURCE_CAPACITY_WAIT` / `MATERIAL_SHORTAGE` / `PRECEDENCE_WAIT` / `FROZEN_ZONE_LOCK` / `ROUTING_FALLBACK` / `STAGE_LEADTIME_FALLBACK` / `BOM_DEGRADE` / `CROSS_ORG_HANDOFF` / `PRIORITY_LOWER_THAN_OTHERS` / `DUE_DATE_RISK` / `LOGISTICS_DELAY` / `PRIORITY_INHERITANCE` / `CROSS_DOMAIN_VERSION_MISMATCH_RISK` / `MANUAL_COMPLETED_SHORT` / `EQUIPMENT_BREAKDOWN_RISK`；不得同义重复创建多个原因码；ReasonCode 只写入 ScheduleExplanationFact 链路，不写入 Task 状态字段 | RESOURCE_CAPACITY_WAIT |
| Severity | 严重等级 | NVARCHAR(20) | `INFO` / `WARN` / `ERROR`（以 DDL 为准；不含 CRITICAL，CRITICAL 仅用于 BOM Issues 表，不用于本表） | WARN |
| ImpactHours | 影响小时数 | DECIMAL(10,2) | 该原因导致的延误或影响小时数；可为 0 | 4.5 |
...
- **跨域版本不一致风险写法（ObjectType=DOMAIN）**：每受影响域写一条 `ScheduleRunId=本次RunId`、`PlanVersionId=该域本次PlanVersion.Id`、`ObjectType=DOMAIN`、`ReasonCode=CROSS_DOMAIN_VERSION_MISMATCH_RISK`；`EvidenceJson` 含 `FailedDomainKeys` / `AffectedDomainKeys` / `SuccessfulDomainKeys` / `CurrentActivePlanVersions` / `NewPlanVersions` / `RecommendedRecalculateDomainKeys`（上游延期等其他跨域不一致原因统一并入此 `ReasonCode`，不再另设 `UPSTREAM_DELAY` 等独立码）。
...
| PlanVersionId | 计划版本ID | INT | 逻辑关联 PlanVersion.Id；数据库未建立物理外键；分区键 | 1 |
| OrderId | 订单ID | BIGINT | 逻辑关联 Order.Id；数据库未建立物理外键 | 100 |
...
| PlanVersionId | 计划版本ID | INT | 逻辑关联 PlanVersion.Id；数据库未建立物理外键；唯一索引 | 1 |
| ScheduleRunId | 排程运行ID | INT | 逻辑关联 ScheduleRun.Id；数据库未建立物理外键 | 1 |
...
**定位**：APS本地存储从ODS MES进度汇总视图同步的工单/工序/大工艺事实。`StageProgressSnapshot`是PI大工艺位置计算权威输入，`OperationProgressSnapshot`只做当前Stage内部小工序裁剪和诊断，`MESWorkOrderSnapshot`用于识别现实工单并恢复ExecutionLock；三者均独立于现货库存链路。

**V1 设计决策（不可随意修改）**：
- 不接 MES 每条报工明细，只接 ODS 汇总后的工序级/大工艺级进度
- 工序识别主字段 = `OperationName`（MES工序名称），不以 MES 工序编码为主
- Task/Pegging随新PlanVersion重生成；跨版本现实执行通过`MESWorkOrderNo → ExecutionLock → Task.ExecutionLockId`恢复，不匹配旧TaskId
...
**来源**：独立夜间定时Job调用`sp_SyncMESWorkOrderSnapshot(@ScheduleRunId, @DataCutoffTime)`从`ODS.MES_APS_WorkOrder_View`同步；不由两个Orchestrator直接调用。

| 字段名 | 中文名 | 数据类型 | 说明 | 示例 |
|--------|--------|----------|------|------|
| Id | 主键 | BIGINT | 自增主键 | 1 |
| ScheduleRunId | 排程运行ID | INT NOT NULL | 关联 ScheduleRun.Id；MES进度快照必须绑定具体排程运行；快照分区键，每次排程全量替换 | 101 |
| ReleaseItemKey | APS发布幂等键 | NVARCHAR(50) NULL | 从MES实时工单视图回传；用于`MESPlanRelease PUBLISHED→CONSUMED`及创建ExecutionLock；历史/外部工单可为空 | RLS:6F3A... |
...
**唯一约束**：`UQ_MESWorkOrderSnapshot_Key (ScheduleRunId, ProductionInstructionNo, MESWorkOrderNo)`。  
**索引**：`IX_MESWorkOrderSnapshot_Run_InstructionNo (ScheduleRunId, ProductionInstructionNo)`；`IX_MESWorkOrderSnapshot_ReleaseItemKey (ScheduleRunId, ReleaseItemKey) WHERE ReleaseItemKey IS NOT NULL`。

**读取口径**：夜间全量与白天Candidate均先确定各自`DataCutoffTime`，一次性从MES实时权威视图形成运行快照；时间窗口只缩小工作集，快照保存的是当前累计事实。Candidate可重读最新MES现实执行事实，但不得借此重新开放当前全部库存/管道/Received供给。
...
**来源**：独立夜间定时Job调用`sp_SyncOperationProgressSnapshot(@ScheduleRunId, @DataCutoffTime)`从`ODS.MES_APS_OperationProgress_View`同步；不由两个Orchestrator直接调用。

**任务数量定位**：本表是各小工序剩余加工量与Task裁剪的权威进度输入；`RemainingQty=max(0,PlannedQty-GoodQty)`仅表达该小工序剩余加工量，不等于PI总剩余量或Stage未来产出量。
...
| RemainingQty | 剩余数量 | DECIMAL(18,4) | **计算列（PERSISTED）**：`CASE WHEN PlannedQty - ISNULL(GoodQty,0) < 0 THEN 0 ELSE PlannedQty - ISNULL(GoodQty,0) END`；2号位消费此字段裁剪内存剩余TaskDraft后传给1号位 | 40.0000 |
| LastReportTime | 最后报工时间 | DATETIME2 NULL | 该工序最后一次报工时间 | 2026-06-12 01:15:00 |
| SourceUpdatedAt | 源更新时间 | DATETIME2 NULL | MES 汇总数据最后更新时间 | 2026-06-12 01:20:00 |
...
**技术要点**：
- `RemainingQty`为持久化计算列，不需SP显式计算；由2号位查询并装入内存TaskDraft/DomainSolveRequest，1号位不直接读取本表
- `ScrapQty` / `ReworkQty` 可选字段：ODS 来源能提供则填写，不能提供则保留 NULL，**不阻断快照同步**
- `OperationName` 是 APS 与 MES 工序对应的唯一主匹配字段（不以 MES 工序编码为准，编码不跨大工艺稳定）；2号位按`OperationName + StageCode`联合识别工序并裁剪TaskDraft，将结果作为内存请求传给1号位；
...
**定位**：记录大工艺级累计进度，是生产指示当前位置计算的权威输入；先由2号位/5号位位置模块进行跨Stage单调修复和互斥化，再由2号位将已确定起始Stage写入内存TaskDraft并传给1号位。**汇总颗粒度**：生产指示号 + 物料编码 + 大工艺阶段码。

**来源**：独立夜间定时Job调用`sp_SyncStageProgressSnapshot(@ScheduleRunId, @DataCutoffTime)`从`ODS.MES_APS_StageProgress_View`同步；不由两个Orchestrator直接调用。

**定位边界**：本表用于大工艺累计进度、PI位置定位与Pegging供给身份判断；不得替代`OperationProgressSnapshot`直接生成各小工序Task数量。

> **同步失败边界**：MESWorkOrder/OperationProgress/StageProgress三类运行快照任一技术失败必须记录FAILED并向上抛出，禁止将连接失败、契约字段错误或事务异常当作合法0行；只有源视图对本次有效工作集真实返回0行才允许空快照。
...
| ScrapQty | 报废数量 | DECIMAL(18,4) NULL | 阶段累计报废数量（可选字段） | 2.0000 |
| ReworkQty | 返工数量 | DECIMAL(18,4) NULL | 阶段累计返工数量（可选字段） | NULL |
| RemainingQty | 剩余数量 | DECIMAL(18,4) | **计算列（PERSISTED）**：`CASE WHEN PlannedQty - ISNULL(GoodCompletedQty,0) < 0 THEN 0 ELSE PlannedQty - ISNULL(GoodCompletedQty,0) END`；2号位优先消费此字段完成位置/TaskDraft计算后传给1号位（粒度粗、性能好） | 40.0000 |
| LastReportTime | 最后报工时间 | DATETIME2 NULL | 该阶段最后一次报工时间 | 2026-06-12 01:15:00 |
| SourceUpdatedAt | 源更新时间 | DATETIME2 NULL | MES 汇总数据最后更新时间 | 2026-06-12 01:20:00 |
...
**技术要点**：
- `RemainingQty`为来源诊断字段；PI总量仍以`Order.Quantity-ReceivedQty`为唯一边界，StageProgress不二次扣减总量
...
### §9.4 ProductionInstructionSupplySnapshot（生产指示总量闭合快照）（v5.2.0新增）

**所属库**：APS_Production  
**定位**：按`ScheduleRunId + DomainKey + ProductionInstructionNo`固化每张PI在本次运行中的总量边界和位置闭合结果。Header不可用MES/XC/在途再次扣减。

| 字段名 | 中文含义 | 数据类型 | 业务说明 |
|---|---|---|---|
| Id | 快照ID | BIGINT | 主键，自增 |
| ScheduleRunId | 排程运行ID | INT | FK→ScheduleRun.Id |
| DomainKey | 分域键 | NVARCHAR(100) | 与ScheduleRun共同隔离 |
| PlanVersionId | 对应版本ID | INT NULL | FK→PlanVersion.Id；便捷关联，不作为主隔离键 |
| DataCutoffTime | 数据截止时间 | DATETIME2 | 与ScheduleRun一致 |
| OrderId | 当前版本订单ID | BIGINT NULL | 逻辑关联Order.Id |
| OrderCanonicalId | 稳定订单ID | BIGINT | FK→Order_Canonical.Id |
| ProductionInstructionNo | 生产指示号 | NVARCHAR(100) | 运行内唯一 |
| MaterialId / FactoryId | 物料/工厂ID | INT | FK→Material/Factory |
| InstructionQty | PI总量 | DECIMAL(18,4) | Order.Quantity |
| ReceivedQty | 最终M库累计入库量 | DECIMAL(18,4) | Order.ReceivedQty，NULL按0处理后写入 |
| AvailableProductionQty | 本次可承接总量 | DECIMAL(18,4) PERSISTED | `max(0,InstructionQty-ReceivedQty)` |
| LocatedQty | 已定位数量 | DECIMAL(18,4) | 所有非UNLOCATED切片合计 |
| UnlocatedQty | 未定位数量 | DECIMAL(18,4) | 不得丢弃，保守回最早Stage |
| SnapshotStatus | 快照状态 | NVARCHAR(20) | READY/WITH_ISSUE |
| IssueCode | 主问题码 | NVARCHAR(100) NULL | 如PI_POSITION_UNLOCATED |
| SourceUpdatedAt | 来源更新时间 | DATETIME2 NULL | |
| CreatedAt | 创建时间 | DATETIME2 | |

**闭合约束**：`LocatedQty + UnlocatedQty = AvailableProductionQty`。

---

### §9.5 ProductionInstructionPositionSlice（生产指示互斥位置切片）（v5.2.0新增）

**所属库**：APS_Production  
**定位**：把Header总量拆成互斥`STAGE / XC / PIPELINE / UNLOCATED`位置，决定每一数量份额的当前Stage、下一Stage、剩余路径和AvailableTime。

| 字段名 | 中文含义 | 数据类型 | 业务说明 |
|---|---|---|---|
| Id | 切片ID | BIGINT | 主键，自增 |
| SupplySnapshotId | PI快照ID | BIGINT | FK→ProductionInstructionSupplySnapshot.Id |
| PositionSliceKey | 切片稳定键 | NVARCHAR(100) | 同一Snapshot内唯一 |
| PositionType | 位置类型 | NVARCHAR(30) | STAGE/XC/PIPELINE/UNLOCATED |
| Quantity | 切片数量 | DECIMAL(18,4) | 大于0；全部切片合计等于Header可承接总量 |
| CompletedStageCode | 已完成Stage | NVARCHAR(50) NULL | |
| CurrentStageCode | 当前所在/待执行Stage | NVARCHAR(50) NULL | XC命中某Stage时，该Stage仍未完成 |
| NextRequiredStageCode | 下一必需Stage | NVARCHAR(50) NULL | TaskDraft起点 |
| RemainingStagePathJson | 剩余路径 | NVARCHAR(MAX) NULL | JSON |
| AvailableTime | 可用时间 | DATETIME2 NULL | |
| SupplyRepresentationType | 供给表示类型 | NVARCHAR(50) | PRODUCTION_INSTRUCTION/INVENTORY/PIPELINE/UNLOCATED等 |
| SourceSystem / SourceRowKey / SourceDocumentNo | 来源追溯 | NVARCHAR | 稳定来源键 |
| InventoryAvailableSupplyDetailId | 库存供给明细ID | BIGINT NULL | 逻辑关联，不建FK |
| PipelineSupplyId | 管道供给ID | BIGINT NULL | 逻辑关联SupplyFact_Pipeline.Id |
| IsEstimated | 是否估算 | BIT | UNLOCATED/默认LT等保守降级 |
| IssueCode | 问题码 | NVARCHAR(100) NULL | |
| CreatedAt | 创建时间 | DATETIME2 | |

**位置规则**：下游Stage累计完成量超过上游时下修下游；中间Stage缺失时采用下游已证明的最小完成量。进入PI位置快照的XC必须能够识别`ProductionInstructionNo`，缺少PI号的XC不得按物料相同强行归入该PI；UNLOCATED从PI最早承载Stage保守生成任务并登记Issue。


---

## 第十节：v5.2.2独立审计补充对象与运维口径

### §10.1 Domain_Dependency（跨产品族域依赖表）

**所属库**：APS_Production  
**定位**：每日01:50由2号位基于BOM静态扫描生成，3号位据此构建Domain拓扑顺序；不在排程核心循环中动态发现跨域依赖。

| 字段名 | 数据类型 | 说明 |
|---|---|---|
| UpstreamDomainCode | NVARCHAR(50) | 上游供应域编码，复合主键之一 |
| DownstreamDomainCode | NVARCHAR(50) | 下游消耗域编码，复合主键之一 |
| ChildMaterialCode | NVARCHAR(50) | 形成跨域依赖的半成品物料，复合主键之一 |
| DefaultLeadTimeDays | INT | V1默认跨域物流提前期，默认2天 |
| ScannedAt | DATETIME2 | 本次静态扫描时间 |

**主键**：`(UpstreamDomainCode, DownstreamDomainCode, ChildMaterialCode)`。  
**索引**：按DDL建立上游域和下游域查询索引，供拓扑排序与受影响域查询。

### §10.2 OrderEmergencyMaterialOverride（急单临时物料补齐桥接表）

| 字段名 | 数据类型 | 说明 |
|---|---|---|
| Id | BIGINT | 主键，自增 |
| OrderStagingId | BIGINT | 逻辑追溯ERP_Order_Staging.Id，不建FK |
| OrderNo | NVARCHAR(50) | 可读订单号 |
| Model | NVARCHAR(100) | 人工录入型号 |
| MaterialCode | NVARCHAR(100) | 临时物料编码 |
| FactoryCode | NVARCHAR(50) NULL | 工厂编码 |
| MaterialName | NVARCHAR(200) NULL | 物料名称辅助信息 |
| UOM | NVARCHAR(20) | 默认PCS |
| Reason | NVARCHAR(500) | 补齐原因，必填 |
| CreatedBy / CreatedAt | NVARCHAR(100) / DATETIME2 | 操作审计 |
| ExpireAt | DATETIME2 NULL | 失效时间 |
| IsActive | BIT | 当前是否启用 |
| PromotedByOfficialSync | BIT | 夜间正式主数据已覆盖标记 |

**索引**：`(OrderStagingId, IsActive)`、`(Model, IsActive)`。

### §10.3 OrderEmergencyBomWorkset（急单临时BOM入口桥接表）

| 字段名 | 数据类型 | 说明 |
|---|---|---|
| Id | BIGINT | 主键，自增 |
| OrderStagingId | BIGINT | 逻辑追溯Staging，不建FK |
| OrderNo | NVARCHAR(50) | 可读订单号 |
| MaterialCode | NVARCHAR(100) | 根物料编码 |
| BOMNO | NVARCHAR(50) NULL | 临时指向BOM；NULL表示只提供阶段标记 |
| BomEntryResult | NVARCHAR(50) | EXPLICIT_BOMNO_USED / DEFAULT_BOMNO_RESOLVED / SPECIAL_INTERNAL_PART / SPECIAL_PURCHASED_PART / BOM_ENTRY_UNRESOLVED |
| Reason | NVARCHAR(500) | 人工补齐原因 |
| CreatedBy / CreatedAt | NVARCHAR(100) / DATETIME2 | 审计字段 |
| ExpireAt | DATETIME2 NULL | 失效时间 |
| IsActive | BIT | 当前是否启用 |
| PromotedByOfficialSync | BIT | 正式BOM已覆盖标记 |

**索引**：`(OrderStagingId, IsActive)`。

### §10.4 OrderEmergencyBomStageDetail（急单临时Stage路径表）

| 字段名 | 数据类型 | 说明 |
|---|---|---|
| Id | BIGINT | 主键，自增 |
| EmergencyWorksetId | BIGINT | 物理FK→OrderEmergencyBomWorkset.Id |
| MaterialCode | NVARCHAR(100) | 当前路径物料 |
| StageSeq | INT | 阶段顺序号，建议10/20/30间隔 |
| StageCode | NVARCHAR(50) | 必须来自StageDict |
| IsSupplyThreshold | BIT | 是否供给阈值点 |
| CreatedAt | DATETIME2 | 创建时间 |

**索引**：`EmergencyWorksetId`；`(MaterialCode, StageSeq)`。

### §10.5 MES_APS_BOM_Workset_StageDetail_Archive（阶段路径归档变体）

字段结构与`MES_APS_BOM_Workset_StageDetail`一致，并增加`ArchivedAt DATETIME2`。归档时必须保留`StageScopeType、ParentMaterialCode(NULL允许)、WorksetId`及完整Stage路径，以支持历史BOM与排程解释追溯。

### §10.6 PlanVersion分区与数据保留策略

- `PF_PlanVersion`：每100个PlanVersion共用一个分区，DDL预建至`PlanVersionId=100000`；接近边界前由DBA评估扩展，禁止长期落入尾部分区。
- ARCHIVED版本的Task/Pegging/Ledger/运行快照明细：建议保留90天。
- FAILED与重试产生的半成品数据：建议保留14天，清理前保留运行错误摘要。
- OrderScheduleSummary/ResourceLoadSummary/PlanKpiSummary：至少保留2年。
- ExecutionLock/DemandSupplyHardLock终态：建议保留1年；未终态和仍被引用记录不得清理。
- V1 DDL只声明策略，不自动创建启用清理Job；DBA须在测试库验证后独立部署清理作业。

---

**END OF APS_Production FIELD DEFINITIONS**
...
**库名**：`APS_Auth`  
**物理部署脚本**：`APS_Auth数据库DDL_v1.0.sql`。该脚本独立创建权限库及13张核心表，不属于`APS_数据库表结构设计_v5.0_v5.2.2`主DDL；测试库部署包必须同时执行两份脚本。  
**用途**：独立的权限与审批数据库，负责：
1. 用户认证与授权
2. 角色与权限管理
3. 数据范围控制（工厂/产品族/资源组）
...
| UserId | 用户ID | INT | 逻辑关联 User.Id；数据库未建立物理外键（APS_Auth 库） | 5 |
| RoleId | 角色ID | INT | 逻辑关联 Role.Id；数据库未建立物理外键（APS_Auth 库） | 3 |
...
| Id | 关联ID | BIGINT | 主键，自增 | 1 |
| RoleId | 角色ID | INT | 逻辑关联 Role.Id；数据库未建立物理外键（APS_Auth 库） | 3 |
| PermissionId | 权限ID | INT | 逻辑关联 Permission.Id；数据库未建立物理外键（APS_Auth 库） | 1 |
...
| Id | 关联ID | BIGINT | 主键，自增 | 1 |
| UserId | 用户ID | INT | 逻辑关联 User.Id；数据库未建立物理外键（APS_Auth 库） | 5 |
| PolicyId | 策略ID | INT | 逻辑关联 DataScopePolicy.Id；数据库未建立物理外键（APS_Auth 库） | 5 |
...
| ApplicantUserId | 申请人ID | INT | 逻辑关联 User.Id；数据库未建立物理外键（APS_Auth 库） | 5 |
...
| ApprovalFlowId | 审批流ID | BIGINT | 逻辑关联 ApprovalFlow.Id；数据库未建立物理外键（APS_Auth 库） | 5 |
| NodeSeq | 节点序号 | INT | 审批顺序（1, 2, 3...） | 1 |
| ApproverRoleId | 审批角色ID | INT | 逻辑关联 Role.Id；数据库未建立物理外键（APS_Auth 库） | 4 |
...
| NodeId | 节点ID | BIGINT | 逻辑关联 ApprovalNode.Id；数据库未建立物理外键（APS_Auth 库） | 1 |
| ApproverUserId | 审批人ID | INT | 逻辑关联 User.Id；数据库未建立物理外键（APS_Auth 库） | 10 |
