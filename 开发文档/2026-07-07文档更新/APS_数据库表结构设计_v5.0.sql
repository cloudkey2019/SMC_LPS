-- =============================================
-- APS 数据库表结构设计 DDL v5.0 (金牌 DDL 模板)
-- 版本: v5.0.46
...
--   ▸ 新增 sp_EnrichBOMWorksetRealtime（§3.5）：实时链路回填，操作 _Realtime 表，Issues.BatchNo='RT:'+@BOMNO
...
CREATE TABLE MES_APS_BOM_Workset (
    ...
    RequestDetailId BIGINT NULL,                        -- v5.0.21 来源追溯锚点（FK→MES_API_BOM_Request_Detail.Id）；nullable；非业务键；1号位主链不消费
    CreatedAt DATETIME2 NOT NULL DEFAULT GETDATE()
);
GO
...
CREATE TABLE MES_APS_BOM_Workset_Realtime (
    ...
    ChildRequiredStageCode NVARCHAR(50) NULL,           -- v5.0.7 同Workset
    ChildRequiredFactory NVARCHAR(20) NULL,             -- v5.0.10 同Workset
    RequestDetailId BIGINT NULL,                            -- v5.0.26 实时插单来源追溯锚点（FKMES_API_BOM_Request_Detail.Id，nullable）
    CreatedAt DATETIME2 NOT NULL DEFAULT GETDATE()
);
GO
...
-- =============================================
-- 9a. 多来源管道供给统一输入视图（v5.0.42 新增 2026-06-15）
-- =============================================
-- 所属层：APS 层
-- 所属库：APS_Production
-- 维护责任人：2号位
...
-- ⚠️ V1 状态：仅包含厂间在途一个来源（返回0行），其余分支全为 placeholder（1=0）骨架；
--    V1.1/V2 逐步将 placeholder 替换为真实 ODS 视图引用。
...
CREATE TABLE InventoryBalance (
    ...
    AllocatedQty DECIMAL(18,4) NOT NULL DEFAULT 0,   -- 已分配量（订单占用）
...
-- ⚠️ V1 状态：本表已建立，V1 sp_SyncPipelineSupply 不调用本表；规则配置为 V1.1/V2 激活预留（待 ERP 管道供给来源字段确认后再启用）
...
-- =============================================
-- 2.8.3 管道供给同步存储过程（V1 空跑骨架；v5.0.42 升级为完整Step 0～Step 5 完整骨架）
-- =============================================
-- V1 设计口径：
--   sp_SyncPipelineSupply 不读取任何管道供给来源视图；
--   仅 TRUNCATE SupplyFact_Pipeline 并写 SUCCESS 日志；
--   空契约视图和统一输入视图仅承担字段契约骨架职责；
--   SupplyFact_Pipeline 结果为空，不影响现有排程
-- V1.1/V2 真实同步 TODO（设计存档 — V1 完全不执行以下步骤；仅 TRUNCATE+SUCCESS）
-- V1.1/V2：@DataCutoffTime 用于源数据截断与规则有效期判定（与 ScheduleRun.DataCutoffTime 同值）
-- V1 调用方仍必须传入 @DataCutoffTime，但 V1 空跑逻辑不使用该值
CREATE OR ALTER PROCEDURE sp_SyncPipelineSupply
    @BatchNo          NVARCHAR(50),
    @DataCutoffTime   DATETIME2,          -- 统一切片边界；V1 空跑阶段不使用
    @RowsAffected     INT OUTPUT,
    @ErrorMessage     NVARCHAR(MAX) OUTPUT
AS
BEGIN
    ...
    BEGIN TRY
        ...
        -- V1：SP 不读取任何管道供给来源视图，只清空空表并写成功日志
        TRUNCATE TABLE SupplyFact_Pipeline;
...
CREATE TABLE MESWorkOrderSnapshot (
    Id                       BIGINT        PRIMARY KEY IDENTITY(1,1),
    ScheduleRunId            INT           NOT NULL,                    -- 对应本次 APS 排程运行（FK→ScheduleRun.Id；MES进度快照必须绑定具体排程运行）
...
-- 1号位消费：按 OperationName + StageCode 联合识别工序，将 GoodQty 对应到排程生成的 Task 上扣减剩余量
-- 注意：StageScopeType（EDGE / ROOT）是 BOM 侧 APS_BOM_STAGE_PATH_RAW 的字段，不属于本表；1号位消费快照时不依赖 StageScopeType
...
BEGIN
    ...
    BEGIN TRY
        ...
        --    d) 降级容错（v5.0.11）：写入 MES_APS_BOM_Workset_Issues（BatchNo=NULL，按 BOMNO 登记，含 DegradeAction 标签）
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUES
                (N'RT:RD:' + CAST(@RequestDetailId AS NVARCHAR(20)),
    ...
    BEGIN TRY
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    IF EXISTS (SELECT 1 FROM #RT_EntryCandidates)
                        INSERT INTO MES_APS_BOM_Workset_Issues
                            (BatchNo, BOMNO, IssueType, Severity, Detail, RequestDetailId, CreatedAt)
                        VALUES (@BatchNo, @ResolvedBOMNO, 'FACTORY_MISMATCH_FALLBACK', 'WARN',
        ...
        BEGIN
            INSERT INTO MES_APS_BOM_Workset_Issues
                (BatchNo, BOMNO, IssueType, Severity, Detail, DegradeAction, RequestDetailId, CreatedAt)
            VALUES
                (N'RT:' + @ResolvedBOMNO, @ResolvedBOMNO,
        ...
        BEGIN
            ...
            INSERT INTO MES_APS_BOM_Workset_Issues
                (BatchNo, BOMNO, ChildMaterialCode,
                 IssueType, Severity, Detail, DegradeAction, CreatedAt)
            SELECT DISTINCT N'RT:' + @ResolvedBOMNO, @ResolvedBOMNO, ls.ChildMaterialCode,
                   N'MULTI_BOMNO_UNRESOLVED', N'WARN',
                   N'物料' + ls.ChildMaterialCode + N' 存在' + CAST(ls.ERPNNBOMCnt AS NVARCHAR(10))
                       + N' 个ERP BOMNO均为IsDefaultVersion=1，已取MAX BOMNO='
                       + ISNULL(ls.MaxERPBOMNO, N'?') + N'，建议ODS版本裁决',
        ...
        -- 8. 更新 READY
        UPDATE MES_API_BOM_Request_Realtime
        SET Status = 'READY', CompletedTime = GETDATE()
        WHERE Id = @RequestId;
...
-- ⚠️ 实时链路版（sp_EnrichBOMWorksetRealtime）逻辑相同，操作 _Realtime 表，BatchNo=NULL
-- =============================================
...
--   4) Issues 表：MES_APS_BOM_Workset_Issues（BatchNo = 'RT:' + @BOMNO 合成键，NOT NULL 约束兼容）
...
BEGIN
    ...
    DECLARE @Now DATETIME2 = GETDATE();
    DECLARE @SyntheticBatch NVARCHAR(50) = N'RT:' + @BOMNO;  -- Issues 表 BatchNo NOT NULL 兼容（@BOMNO 始终非空）
    DECLARE @EnrichedCount INT = 0;
    DECLARE @StageDetailCount INT = 0;
    DECLARE @IssueCount INT = 0;
...
-- 3号位/NightlyBatchOrchestrator 最晚00:38前创建记录（Status=RUNNING）并确定DataCutoffTime；
-- 00:40/00:45/00:50三个MES快照SP必须使用同一ScheduleRunId+DataCutoffTime；
...
-- Status 值域：RUNNING / COMPLETED / FAILED
CREATE TABLE ScheduleRun (
    Id                  INT PRIMARY KEY IDENTITY(1,1),
    RunType             NVARCHAR(50)  NOT NULL,          -- 运行类型
    Status              NVARCHAR(20)  NOT NULL,          -- RUNNING / COMPLETED / FAILED（运行状态）
    ScenarioId          INT           NULL,              -- FK → Scenario.Id（仿真/插单分析关联；凌晨全量为空）
    BasePlanVersionId   INT           NULL,              -- FK → PlanVersion.Id（重排/仿真基于哪个已存在版本；凌晨全量可为空）
    TriggeredBy         NVARCHAR(100) NOT NULL,          -- 'Hangfire' / UserId / 'API' / 'Agent'
    DataCutoffTime      DATETIME2     NOT NULL,          -- 本次运行统一数据切片边界
    ScopeJson           NVARCHAR(MAX) NULL,              -- 局部重排范围JSON
    StartedAt           DATETIME2     NOT NULL DEFAULT GETDATE(),
    CompletedAt         DATETIME2     NULL,              -- 2号位回填
    ErrorMessage        NVARCHAR(MAX) NULL,
    CreatedAt           DATETIME2     NOT NULL DEFAULT GETDATE()
);
GO
...
CREATE TABLE ScheduleExplanationFact (
    ...
    PlanVersionId   INT           NOT NULL,              -- 分区键，FK → PlanVersion.Id
    ObjectType      NVARCHAR(50)  NOT NULL,              -- ORDER / TASK / RESOURCE
    OrderId         INT           NULL,
    TaskId          INT           NULL,
    ResourceId      INT           NULL,
...
CREATE TABLE ResourceLoadSummary (
    Id              INT           PRIMARY KEY IDENTITY(1,1),
    PlanVersionId   INT           NOT NULL,              -- FK → PlanVersion.Id
    ResourceId      INT           NOT NULL,              -- FK → Resource.Id
...
CREATE TABLE PlanKpiSummary (
    Id                  INT           IDENTITY(1,1) NOT NULL,
    PlanVersionId       INT           NOT NULL,          -- 分区键，FK → PlanVersion.Id（1:1）
...
-- -----------------------------------------------
-- 3.7 Scenario（what-if 场景表，阶段二骨架，阶段一不实装）
-- -----------------------------------------------
-- 定位：记录一个业务试算场景的假设、目标和最终选中版本。不是运行表，不是结果版本表。
-- ⚠️ 阶段一：建表骨架，不写入任何数据。阶段二实装。
CREATE TABLE Scenario (
    ...
    SelectedPlanVersionId INT           NULL,                  -- FK→PlanVersion.Id；候选版本生成后回填；可空
...
CREATE TABLE SimulationRun (
    Id                  INT           PRIMARY KEY IDENTITY(1,1),
    ScenarioId          INT           NOT NULL,             -- FK → Scenario.Id
    ScheduleRunId       INT           NULL,                 -- FK → ScheduleRun.Id
...
ON APS_BOM_CROSS_FACTORY_EDGE_RAW(BatchNo, ChildMaterialCode, ToFactoryCode);
GO

-- =============================================
-- 3.16 PeggingSupplyAllocation（Pegging 供给分配细账表 v5.0.46 新增）
-- =============================================
-- 负责：2号位。定位：不是候选供给表，只记录确认可用于当前需求的供给分配结果。
-- 不可用库存/未匹配出荷指示号的 ZP/BP 库存不进入本表。
-- 与物理 Pegging 表（Task-to-Task）职责分离：本表记录库存/在途/Received等非Task供给；
-- 物理 Pegging 表记录 Task-to-Task 供需血缘。
CREATE TABLE dbo.PeggingSupplyAllocation (
    Id                      BIGINT IDENTITY(1,1) PRIMARY KEY,
    PlanVersionId           INT            NOT NULL,        -- FK→PlanVersion.Id
    ScheduleRunId           INT            NOT NULL,        -- FK→ScheduleRun.Id
    BatchNo                 NVARCHAR(50)   NOT NULL,
    RootOrderId             BIGINT         NULL,
    RootOrderNo             NVARCHAR(100)  NULL,
    CurrentOrderId          BIGINT         NULL,            -- 当前生产指示/厂间出荷指示，可空
    CurrentOrderNo          NVARCHAR(100)  NULL,
    OrderType               NVARCHAR(50)   NULL,            -- SALES_ORDER / PRODUCTION_INSTRUCTION
    WorksetId               BIGINT         NULL,            -- BOM Workset 行
    MaterialId              INT            NOT NULL,
    ...
    SupplyType              NVARCHAR(50)   NOT NULL,        -- INVENTORY / WIP / PIPELINE / INTER_FACTORY_ORDER / PRODUCTION_INSTRUCTION / PURCHASE_ORDER / NEW_REQUIREMENT
    SupplyFactoryCode       NVARCHAR(50)   NULL,
    SupplyWarehouseCode     NVARCHAR(50)   NULL,
    ERPProperty             NVARCHAR(20)   NULL,            -- M / XC / ZP / BP（来自 ProcessCodeDict）
    AttachStageCode         NVARCHAR(50)   NULL,            -- 供给挂接到哪道大工艺
    CompletedStageCode      NVARCHAR(50)   NULL,            -- 物料已完成到哪步
    NextRequiredStageCode   NVARCHAR(50)   NULL,            -- 后续第一道要排的大工艺
    RemainingStagePathJson  NVARCHAR(MAX)  NULL,            -- 后续剩余大工艺路径
    SupplyMode              NVARCHAR(50)   NULL,            -- STAGE_HANDOFF / INTER_FACTORY_ORDER / PURCHASE_IN_TRANSIT / NULL
    CrossFactoryEdgeId      BIGINT         NULL,            -- FK→CrossFactoryEdge；涉及跨厂时填
    TransportLeadTimeHours  INT            NULL,
    ETA                     DATETIME2      NULL,
    KnownAvailableTime      DATETIME2      NULL,            -- 库存=当前，在途=ETA
    SupplyDocumentType      NVARCHAR(50)   NULL,            -- STOCK / SHIPPING_INSTRUCTION / PURCHASE_ORDER / PRODUCTION_INSTRUCTION / PIPELINE
    SupplyDocumentNo        NVARCHAR(100)  NULL,            -- 真正匹配的业务单据号
    CreatedAt               DATETIME2      NOT NULL DEFAULT GETDATE()
);
GO

CREATE INDEX IX_PeggingAlloc_PlanVersion
ON PeggingSupplyAllocation(PlanVersionId);
CREATE INDEX IX_PeggingAlloc_ScheduleRun
ON PeggingSupplyAllocation(ScheduleRunId);
CREATE INDEX IX_PeggingAlloc_Order
ON PeggingSupplyAllocation(RootOrderId)
WHERE RootOrderId IS NOT NULL;
CREATE INDEX IX_PeggingAlloc_SupplyDoc
ON PeggingSupplyAllocation(SupplyType, SupplyDocumentNo)
WHERE SupplyDocumentNo IS NOT NULL;
GO

-- =============================================
-- SupplyFact_Pipeline MOCK数据机制（v5.0.46 注释补充）
-- =============================================
-- V1 开发/测试联调期间可使用 MOCK 数据写入 SupplyFact_Pipeline：
--   SourceSystem = 'MOCK'
--   BatchNo = 'MOCK_YYYYMMDD'
--   写入 SupplyFact_Pipeline 正式字段（MaterialCode/FactoryCode/SupplyType/Quantity/ETA 等）
--   SourceDocumentNo 使用模拟出荷/运输单号（如 MOCK-SHIP-TJ-CN-001）
-- 后续真实数据接入时，不改 Pegging 主流程。
