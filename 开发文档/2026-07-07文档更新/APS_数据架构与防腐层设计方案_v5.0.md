...
**相关文档**：
- **《APS_各类基础数据分层承接与演变总表_v5.0》**（当前 v3.29）：数据演进全景图（架构总纲），建议先读此文档理解全局
- **《职责分工变更说明_v3.0_Socket-Plug模式》**：Socket-Plug职责分工详细说明
- **《APS_数据库字段说明文档_v5.0》**（当前 v5.0.46）：数据库表结构与字段定义
- **《APS_数据库表结构设计_v5.0.sql》**（当前 v5.0.46）：DDL脚本，数据库表结构
...
- **内存**：96GB
- **硬盘**：200GB（操作系统 + 应用程序）
- **用途**：1号位、3号位、5号位的排程引擎、Web API、Hangfire调度
...
#### **2.2.1 白天增量同步（每小时执行）**（2026-04-03修正）
...
**同步路径**：ERP → `ERP_Order_Staging` → `Order_Canonical`（**必须经过Staging验证**）

**同步逻辑**：
```csharp
// APS守护进程（Hangfire后台任务）
// ⚠️ 2026-04-03修正：统一走 Staging → Canonical 路径，不再直接写入 Order_Canonical
public async Task SyncOrdersFromERP()
{
    // 1. 从ERP契约视图拉取最近1小时的增量订单（含SO/MTO/MTS/SS）
    ...
    //    校验通过 → Upsert到Order_Canonical，Staging状态改为PROCESSED
    //    校验失败 → Staging状态改为FAILED，ErrorMessage留痕
    await orderPromotionService.ValidateAndPromoteAsync();
    
    // 4. 检测需要实时BOM展开的新订单（v5.0.31：缓存判断改为 OrderCanonicalId）
    //    RequestedBOMNO可空：有 RequestedBOMNO 直接展开；无 RequestedBOMNO 时，5号位按 OrderType + MaterialCode + FactoryCode + BOM边/ProcessCode 规则解析入口
    var newOrdersNeedingBOM = orders
        .Where(o => !worksetCache.Contains(o.OrderCanonicalId))  // v5.0.31 改用 OrderCanonicalId
        .ToList();
    
    if (newOrdersNeedingBOM.Any())
    {
        // 5. 写入 MES_API_BOM_Request_Detail（v5.0.31新结构：OrderCanonicalId/RequestedBOMNO可空）
        await RequestRealtimeBOMExpansion(newOrdersNeedingBOM);
    }
}
```
...
#### **2.2.1b 实时插单流程（紧急订单快速通道）**（2026-04-03补充）

**触发条件**（v5.0.31 更新）：白天增量同步发现无BOM Workset缓存的新订单（`OrderCanonicalId` 不在 Workset 缓存中）；不再依赖"新BOMNO"检测——BOMNO可空，含无BOMNO订单

**流程**（2026-04-15 v1.8更新，v1.17补BOMNO可空）：
1. 新订单正常走 Staging → Canonical 路径
2. 同时写入 `MES_API_BOM_Request_Detail`（v5.0.32最终结构：OrderCanonicalId/OrderNo/SourceSystem/SourceOrderId/MaterialCode/FactoryCode/OrderType；RequestedBOMNO可空；**Model/OrderStagingId/ResolvedBOMNO已删除**）并以 `RequestDetailId` 触发 `sp_ExpandBOMRealtime_vNext(@RequestDetailId)`
3. BOM展开完成后，结果写入 `MES_APS_BOM_Workset_Realtime`（透传3辅助字段 + `RequestDetailId`）；`sp_ExpandBOMRealtime_vNext` 内部解析 `@ResolvedBOMNO`，再调用 `sp_EnrichBOMWorksetRealtime(@ResolvedBOMNO, @RequestDetailId)` 执行回填 `ChildRequiredStageCode` / `ChildRequiredFactory` + 写入 `MES_APS_BOM_Workset_StageDetail_Realtime`（双层结果 EDGE+ROOT）+ Issues 降级登记（`BatchNo='RT:'+@BOMNO`）
4. 若需要立即排程（非等到夜间），由5号位手动触发"增量排程"
5. 增量排程仅处理新插入的订单，不重排已冻结区域

**⚠️ 限制**：实时插单不触发Order分区表装载（仍等夜间00:05批次），除非5号位手动触发
...
**物理真相**：
- APS的5号位（大脑）在02:00进行Pegging（血缘连线）时，是**一层一层往下扣减和展开**的
...
MES_APS_BOM_Workset (主层：3辅助字段 + ChildRequiredStageCode，不承载根产品自身路径)
  ...
  ↓ 1号位消费：**必须按StageScopeType区分查询**；EDGE路径串接RoutingOperation排Task，ROOT路径排根产品自身Task，无小工序的外协阶段查StageLeadTimeParam
```
...
**V1 管道供给链空运行说明**：
- ODS 层 `ERP_InterplantInTransit_View`（所属库：MES_Integration；来源系统：ERP）返回 0 行——ODS 空契约骨架
- APS 层 `ext_ERP_InterplantInTransit_View`（所属库：APS_Production）同步返回 0 行（APS 跨库包装视图，显式列字段）
- `sp_SyncPipelineSupply` 正常运行但 `SupplyFact_Pipeline` 结果为空，不影响 `InventoryBalance`
- `ScheduleContext.PipelineSupplies` 为空集合，空集合是正常结果不是 ETL 失败
...
- 从ERP拉取最近1小时的订单
- Upsert到APS本地库
- 检测新增订单或 OrderCanonicalId 无 Workset 缓存的订单，写入 MES_API_BOM_Request_Detail，以 RequestDetailId 触发 sp_ExpandBOMRealtime_vNext
...
**触发条件**（v5.0.31 更新）：
- 白天增量同步发现新增订单，或 OrderCanonicalId 无 Workset 缓存
- RequestedBOMNO 为空时，5号位按 OrderType + MaterialCode + FactoryCode + BOM边/ProcessCode 规则解析入口

**解决方案（v5.0.26 vNext 主路径）**：
- 在1小时/次的订单增量同步程序中检测无 Workset 缓存的新订单（OrderCanonicalId）
- 写入 MES_API_BOM_Request_Detail（v5.0.32 最终结构），以 RequestDetailId 触发 sp_ExpandBOMRealtime_vNext
- 实时展开结果写入 MES_APS_BOM_Workset_Realtime

**【实时展开流程（v5.0.26 vNext 主路径）】**：
...
> ⚠️ **以下为旧版 sp_ExpandBOMRealtime(@BOMNO) 文档（v5.0.26 起已由 sp_ExpandBOMRealtime_vNext 替代），仅供历史参考**
...
### 2.5 管道供给链同步：sp_SyncPipelineSupply（v5.0.42 升级为完整骨架）
...
ERP 采购在途 / VMI / 已到厂未入库（V1.1/V2 预留）
  ...
  → sp_SyncPipelineSupply（V1 仅 TRUNCATE+SUCCESS，不读取任何来源视图；
      V1.1/V2 启用后才读取 ext_PipelineSupply_Source_View，不直接读单来源视图）
  → SupplyFact_Pipeline
  → ScheduleContext.PipelineSupplies（空集合）
```

#### **V1 口径：空跑骨架（当前实装）**

> ⚠️ **V1 设计决策写死**：V1 不读取正式 ERP 在途数据，不执行字段映射，不应用 `SupplyAvailabilityRule`，不写入业务数据。`SupplyFact_Pipeline` 结果为空，不影响现有排程。`ScheduleContext.PipelineSupplies` 为空集合，空集合是正常结果不是 ETL 失败。

**V1 执行流程**：
```
sp_SyncPipelineSupply
    @BatchNo          = @BatchNo,
    @DataCutoffTime   = @DataCutoffTime,    -- V1 传值但不使用
    @RowsAffected     = @RowsAffected OUTPUT,
    @ErrorMessage     = @ErrorMessage OUTPUT

Step 1  TRUNCATE TABLE SupplyFact_Pipeline
        -- 清空管道供给表，确保 V1 链路结果为空

Step 2  写入 APS_ETL_Log SUCCESS
        -- BatchNo=@BatchNo, Message='V1 空跑完成：SupplyFact_Pipeline 已 TRUNCATE'
```

**❗ 设计决策写死（V1）**：
- `InventoryBalance` 定义不变；现货库存主链 `sp_SyncInventorySnapshot` 六步 ETL 完整执行，管道链路不干扰
- `SupplyFact_Pipeline` 结果为空时不影响现有排程
- `ERP_InterplantInTransit_View` 在 ODS 侧建空契约视图（WHERE 1=0 返回 0 行）；V1 不读取真实 ERP 数据
- `ext_ERP_InterplantInTransit_View`（APS 跨库包装视图）同步返回 0 行；显式列字段，禁止 SELECT *
- `ext_PipelineSupply_Source_View`（多来源统一输入视图）V1 已建立，当前仅厂间在途一个分支返回0行，其余 placeholder WHERE 1=0；未来新增来源只需追加 UNION ALL，不改变 SP 主流程
- `SupplyAvailabilityRule` 表已建立，V1 不调用；规则配置为 V1.1/V2 激活预留
- 现货链统一使用 `InventoryAvailabilityRule`（旧 `ProductFamilyInventoryScope` + `InventorySourceRule` 已于 v5.0.39 删除）

#### **V1.1 / V2 真实同步 TODO（设计存档 — 当前 V1 完全不执行）**

> ⚠️ 以下全部为 V1.1/V2 设计存档。当前 V1 的 SP 不查询任何视图、不执行任何映射、不调用任何规则、不写入任何业务数据。
> 具体来源字段、异常码、ETA 处理策略须在真实数据源确认后才能完成最终实施设计。
> V1 上线验收不检查以下内容。

**⚠️ 契约锁定规则**：
ODS 层 `ERP_InterplantInTransit_View` 字段结构为强契约。V1.1/V2 启用真实数据时，
允许调整视图内部的 SELECT 表达式、FROM、JOIN 及 WHERE 实现逻辑；
...
```
[V1.1/V2 TODO] sp_SyncPipelineSupply 真实装载流程：

**契约锁定规则**：
ODS 层视图字段结构为强契约。V1.1/V2 启用真实数据时，
允许调整视图内部的 SELECT 表达式、FROM、JOIN、WHERE 及必要转换逻辑；
...
Step 5  写入 SupplyFact_Pipeline（事务原子批次重建）：
        ...
        - V1 空集合 = 正常；V1.1/V2 SP 崩溃后的空集合 <> 正常
```
...
**⚠️ 批次生命周期说明**：
> V1 当前继续执行 TRUNCATE TABLE SupplyFact_Pipeline（结果始终为空，无副作用）。
> V1.1/V2 必须使用事务内的 DELETE + INSERT 按 BatchNo 逐批次操作。

**⚠️ 消费口径**：
> - 夜间快照：WHERE BatchNo = @CurrentBatchNo AND IsActive = 1
> - 白天实时：WHERE BatchNo IS NULL AND IsActive = 1
> - 禁止只按 IsActive = 1 查询（会同时读到多个历史批次）
> - 白天实时链路当前仅为字段预留，V1.1 暂不启用；正式消费只读取夜间批次
---
...
| RunType | 触发方 | 触发时机 | PlanVersion.VersionCategory | 产出版本默认 Status | 能否自动激活 |
|---------|--------|----------|---------------------------|--------------------|-------------|
| `FULL_SCHEDULE` | Hangfire 定时器 | 凌晨全量 | DAILY_BASELINE | BUILDING→ACTIVE | 是（调度器自动）|
| `MANUAL_RESCHEDULE` | 用户/API | 人工主动触发 | RESCHEDULE_CANDIDATE | CANDIDATE | 否（须显式激活）|
| `LOCAL_RESCHEDULE` | 用户/API | 局部重排 | LOCAL_RESCHEDULE_CANDIDATE | CANDIDATE | 否（须显式激活）|
| `SIMULATION` | API（阶段二） | 仿真计算 | SIMULATION_CANDIDATE | CANDIDATE | 否（禁止自动激活）|
| `INSERT_ORDER_WHATIF` | API | 插单影响分析 | WHATIF_CANDIDATE | CANDIDATE | 否（禁止自动激活）|

> ⚠️ 核心红线：非 FULL_SCHEDULE 产出的 PlanVersion 默认 CANDIDATE，禁止自动激活。正式采用直接通过 `PlanVersion.Status = ACTIVE` 表示。
...
排程成功：
      ...
      ScheduleRun.CompletedAt = 当前时间
      PlanVersion.Status     = ACTIVE
      PlanVersion.ActivatedAt = 当前时间
      PlanVersion.ActivatedBy = 'SYSTEM'

排程失败：
      ScheduleRun.Status     = FAILED
      ScheduleRun.ErrorMessage = 错误信息
...
| 步骤 | 负责人 | 动作 |
|------|--------|------|
| ScheduleRun 创建 | 3号位（触发端） | Hangfire / API 触发时同步创建 |
| ScheduleRun 状态更新 | 2号位（排程完成后） | Status/CompletedAt 回填 |
| PlanVersion 创建 | 2号位（排程启动时） | 创建 BUILDING 版本壳 |
| PlanVersion 激活 | 调度器（FULL_SCHEDULE）/ 3号位（其他） | Status→ACTIVE |
| Scenario 创建 | 3号位/业务 | 仿真实场景 |
...
5号位基于 StageDetail 按 StageSeq 排序生成，只记录 `FromFactoryCode <> ToFactoryCode` 的跨厂段。该表只表示结构事实，不判断跨厂模式。2号位搬运到 `APS_BOM_CROSS_FACTORY_EDGE_RAW` 供 Pegging 直接读取。

#### 2.10.4 PeggingSupplyAllocation 与物理 Pegging 表分离

`PeggingSupplyAllocation` 不是候选供给表，只记录已确认可用供给。物理 Pegging 表是 Task-to-Task 血缘表（`UpstreamTaskId/DownstreamTaskId`），在 Task/ShippingTask 生成后由 2号位写入。非 Task 供给写入 `PeggingSupplyAllocation`，不写入物理 Pegging 表。
...
| PeggingSupplyAllocation | 2号位 |
| M库判定索引 | 2号位 |
| 跨厂模式判断 | 2号位（5号位参与业务裁决） |
| 1号位 | 只消费 Task/ShippingTask，不查 MES_ProcessCode_View/Received/PeggingSupplyAllocation |
...
**链路定位**：MES 生产进度汇总数据为 APS 排程引擎提供"已完成量"输入，用于每日全量重算前扣减已完成 Task 数量。此链路与现货库存链路并行独立，**绝不混入 `InventoryBalance`**。

**V1 核心设计决策（写死）**：
- ⚠️ **不接 MES 每条报工明细**：只接 ODS 汇总后的工序级 / 大工艺级进度（避免 MES 报工明细频繁变更污染排程快照）
- ⚠️ **工序识别主字段 = `OperationName`**（MES 工序名称）：不以 MES 工序编码为主匹配字段，因 MES 工序编码不跨大工艺稳定；APS 通过 `OperationName` + `StageCode` 联合识别工序
- ⚠️ **Task / Pegging 全量重算口径**：Task 和 Pegging 随新的 `PlanVersionId` 每日全量重新生成；MES 进度只用于计算当日剩余 Task 数量，**不匹配历史 TaskId**；Pegging 只描述当前 `PlanVersionId` 内供需血缘，不跨版本复用
- ⚠️ **EAM V1 预留**：`EAM_APS_Resource_View` 在 ODS 层预留占位，V1 不读取 EAM 设备故障数据，不生成资源不可用窗口；此占位不影响当前进度快照链路
...
**执行时机**：`ScheduleRun` 必须在 00:40 前创建（最晚 00:38，由 NightlyBatchOrchestrator 在 LLC 计算完成后创建），以提供 `ScheduleRunId` 和 `DataCutoffTime`。三个快照同步 SP 依序执行：00:40（MES工单快照）→ 00:45（工序进度快照）→ 00:50（大工艺进度快照），在 LLC 计算（00:35）之后、ScheduleContext 装载（02:00）之前完成。
...
**1号位消费规则**：
1. 排程引擎优先查 `StageProgressSnapshot`（大工艺粒度，性能优先）按 `ProductionInstructionNo + MaterialCode + StageCode` 取 `RemainingQty`
2. 需工序级精度时才查 `OperationProgressSnapshot`，按 `ProductionInstructionNo + MESWorkOrderNo + MaterialCode + OperationName + StageCode` 取 `RemainingQty`
3. 所有由 Order 驱动生成的 Task 必须透传 `ProductionInstructionNo`；**禁止生成不可追溯到 `ProductionInstructionNo` 的 Task**
...
### 4.1 建议1：白天紧急插单的实时BOM展开优化

#### **4.1.1 问题分析**

**原方案盲区**：
- 80万棵树的繁重准备工作是在夜间00:00完成的
- 若白天新增订单、OrderCanonicalId 无 Workset 缓存、或 RequestedBOMNO 为空但需要实时解析入口时，夜间结果尚未覆盖，APS本地将无对应展开数据

**原方案弥补（v5.0.26 vNext 主路径）**：
- 在1小时/次的订单增量同步程序中检测无 Workset 缓存的新订单（OrderCanonicalId）
- 写入 MES_API_BOM_Request_Detail，以 RequestDetailId 触发 sp_ExpandBOMRealtime_vNext 单条展开
- 将实时展开结果补录进 MES_APS_BOM_Workset_Realtime 缓存

---

#### **4.1.2 优化方案**

> ~~旧 BOMNO 级表结构已废弃~~ — v5.0.33 起 `MES_API_BOM_Request_Realtime` 和 `MES_APS_BOM_Workset_Realtime` 均已升级为 RequestDetail 级结构，**正式 DDL 以 §5.1.5 / §5.1.6 为准**，此处不再展示旧结构，避免误复制。

**ODS库实时展开线程**：
- 独立的SQL Server Agent Job，每5分钟执行一次
- 优先级低于夜间批次展开
- 5分钟内完成单个 RequestDetail 级实时展开
...
**并发控制**：
- 实时展开线程最多同时处理**3个实时 RequestDetail**
- 避免影响MES生产库性能

**超时控制**：
- 单个 RequestDetail 展开超时时间：**5分钟**
- 超时后标记为FAILED，记录错误日志

**重试机制**：
- 失败的实时 RequestDetail 自动重试**3次**
- 3次失败后人工介入
...
> v5.0.33：补全辅助字段（`ParentProcRefCode` / `ChildProcRefCode` / `ChildSourceHintCode`）、回填结果字段（`ChildRequiredStageCode` / `ChildRequiredFactory`）及追溯锚点（`RequestDetailId`）
...
CREATE TABLE MES_APS_BOM_Workset_Realtime (
    ...
    Quantity               DECIMAL(18,6)   NOT NULL,
    Level                  INT             NOT NULL,
    ParentProcRefCode      NVARCHAR(20)    NULL,      -- 透传自 MES_BOM_View（工序码）
    ChildProcRefCode       NVARCHAR(20)    NULL,      -- 透传自 MES_BOM_View（工序码）
    ChildSourceHintCode    NVARCHAR(10)    NULL,      -- 透传自 MES_BOM_View（Produce字段）
    ...
    RequestDetailId        BIGINT          NULL,      -- v5.0.33: 订单粒度追溯锚点
    CreatedAt              DATETIME2       NOT NULL DEFAULT GETDATE()
);
...
CREATE INDEX IX_Realtime_RequestDetail
ON MES_APS_BOM_Workset_Realtime(RequestDetailId, BOMNO);
```
...
**接口名称**：`sp_RequestRealtimeBOMExpansion`

**输入参数**：
- `@BOMNO`：BOM编号
- `@Priority`：优先级（0=普通，1=紧急）
...
EXEC sp_RequestRealtimeBOMExpansion 
    @BOMNO = 'BOM-NEW-001',
    @Priority = 1,
...
#### **风险1：白天新增订单或无 Workset 缓存的实时展开盲区**

**问题**：
- 80万棵树的繁重准备工作是在夜间00:00完成的
- 若白天新增订单、无 Workset 缓存、或 RequestedBOMNO 为空但需要实时解析入口时，触发实时 RequestDetail 级展开

**弥补措施（v5.0.26 vNext 主路径）**：
- ✅ 在1小时/次的订单增量同步程序中检测新增订单或无 Workset 缓存的 OrderCanonicalId
- ✅ 写入 MES_API_BOM_Request_Detail，以 RequestDetailId 触发实时 RequestDetail 级展开
- ✅ 将实时展开结果补录进 MES_APS_BOM_Workset_Realtime 缓存
- ✅ 5分钟内完成实时展开
...
**文档结束。当前基线版本 v1.33（2026-06-23），完整版本历史以前文各版变更说明为准。**
