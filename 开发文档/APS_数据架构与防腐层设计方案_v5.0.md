...
## v1.33 变更说明（2026-06-23 规则与参数引擎接入运行编排）

- 🆕 **§2.8.6 规则与参数引擎接入运行编排**：阐明 RuleSetVersion/ParameterSetVersion/StrategyProfileVersion 策略包体系
- 🆕 **ScheduleRun.StrategyProfileVersionId**：本次运行采用的策略包版本；00:38 创建时绑定，02:00 加载到 ScheduleContext
- 📌 **追溯链**：StrategyProfileVersion → RuleSetVersion + ParameterSetVersion
...
- **《APS_数据库字段说明文档_v5.0》**（当前 v5.0.45）：数据库表结构与字段定义
- **《APS_数据库表结构设计_v5.0.sql》**（当前 v5.0.45）：DDL脚本，数据库表结构
