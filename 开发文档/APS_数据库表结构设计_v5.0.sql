...
--   - sp_SyncMasterData: @SourceType 枚举**不**含 'ProcessCode'（ProcessCodeDict 是 APS 自维护字典，不参与自动同步）
...
-- 🔄 v5.0.16 定位**翻转**：v5.0.15 错位定为「ERP 工序对照表的 ODS 物理镜像」；本版改为：
--    **APS 自维护的 ODS 增强工序字典**——业务字段是 ERP/MES 语义来源，但本表不参与任何自动同步
-- 消费边界：**仅 ODS 内部消费**（5 号位 material_stage_chain 推导、2 号位 MaterialStageDeptContext 组装、委外 ShippingTask 生成器、运维诊断、BI 报表）
...
-- ⚠️ 维护方式：APS 系统管理员通过 UI/SQL 人工维护；不存在自动同步骨架
-- 维护流程：管理员录入条目 → 0 号位审批（含 StageCode 归属确认）→ IsActive=1 启用
-- 字段变更（含 StageCode 修改）必须经 0 号位审批，且**绝不允许**任何自动化同步过程覆盖此表数据
...
CREATE VIEW MES_ProcessCode_View AS
SELECT
    ProcessCode,
    ProcessName,
    FactoryCode,
