...
**基于：DDL v5.0.45；相关文档：演变总表 v3.29 / 防腐层 v1.33**  
...
**业务用途**：APS 自维护的「工序对照增强字典」；业务字段语义来源 ERP/MES，但本表**不参与任何自动同步**。

**🔄 v5.0.16 定位翻转说明**：
- v5.0.15 错位定为「ERP 工序对照表的 ODS 物理镜像/缓存」并配 `sp_SyncMasterData(@SourceType='ProcessCode')` 自动同步分支 —— **错位**
- v5.0.16 改为 **「APS 自维护的 ODS 增强工序字典」**：APS 系统管理员人工维护 + 0 号位审批；不存在自动同步机制
...
**维护责任**：APS 系统管理员 + 0 号位审批；**不参与** `sp_SyncMasterData` 自动同步流程；任何字段变更（含 StageCode 修改）必须经 0 号位审批
...
- 字段变更（含 StageCode 修改）必须经 0 号位审批；**绝不允许**任何自动化同步过程覆盖此表数据
